...
CCD_example_tib = indel.data %>% 
  ...
  mutate(freqCut_mean = freqCut_mean / (1- freqCut_mean)) %>%
...
TIF_hits = CCD_tib %>% filter(!is.na(editratio_slope_plot) & 
...
CCD_tib %>% filter(feature == "H3K36me3" & drug_conc == "EPZ-6438 10 µM") %>% pull(editratio_slope_plot)
```
...
FC_tib = ratios_tib_fc %>% dplyr::select(IPR, drug_conc, log2.fc.editratio, log2.fc.ratio, all_of(chromatin.features.short)) %>% ungroup()

for (i in c("BMN 673 10 µM", "Apicidin 100 nM")){
  drug_conc_dt <- filter(FC_tib, drug_conc == i)
  # Run a model per drug_conc
  set.seed(1)
  chrom_formula = reformulate(chromatin.features.short, response = "log2.fc.editratio")
  ...
  combined.dt <- tibble(measured = drug_conc_dt %>% pull("log2.fc.editratio"), 
...
# adj_p.value_KO_model %>% filter(drug_conc %in% c("BMN 673 10 µM", "Apicidin 100 nM"), var == "log2.fc.editratio") %>% knitr::kable()
# |var             |drug_conc       |   p.value|     p.adj|
# |:---------------|:---------------|---------:|---------:|
# |log2.fc.editratio |BMN 673 10 µM   | 0.0846856| 0.1072685|
# |log2.fc.editratio |Apicidin 100 nM | 0.0000799| 0.0006183|
```
...
# Highlight top 4 strongest hits + and - TIF. In the end we kept only LMNB1, but let's keep this code for flexibility.
ratios_tib_fc %>% 
