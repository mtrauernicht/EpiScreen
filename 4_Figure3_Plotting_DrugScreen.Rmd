...
## 3D K27me3 top CCDs
```{r 3C H3K27me3 synergies, fig.width= 6, fig.height= 6}
rank_dt = CCD_tib %>% 
...
all_compounds_epistasis_TIF %>% 
  filter(!between(H3K27me3, -0.02, .02)) %>%
  rownames()
```
...
## 3D m5C top CCDs
```{r 3C m5C synergies, fig.width= 6, fig.height= 6}
rank_dt = CCD_tib %>% 
  left_join(rownames_to_column(target, var = "drug_conc")) %>%
...
# Samples to highlight in the heatmap S3C
all_compounds_epistasis_TIF %>% 
  filter(!between(m5C, -0.02, .02)) %>%
  rownames()
```

## 3F K27me3 and CTCF correlation
```{r 3F K37me3 vs CTCF CCD correlation, fig.width= 10, fig.height= 5}
...
## S3B Validation of HDACi in the pool
...
vorino.dt %>% mutate(state = ifelse(late_replicating == 1 & 
  ...
  ggplot(., aes(log2_FC_TIF, fill = state)) +
                                      ...
                                      H3K27me3 == 0, "Triple Heterochromatin", ifelse(
                                        late_replicating == 0 & 
                                          ...
                                          H3K27me3 == 0, "Euchromatin", ifelse(late_replicating == 0 &  
                                                                                 ...
                                                                                 H3K27me3 == 1, "H3K27me3", "other-heterochromatin")))) %>%
  mutate(state = factor(state, levels = c("Euchromatin", "other-heterochromatin", "H3K27me3", "Triple Heterochromatin"))) %>%
  ggplot(., aes(log2_FC_TIF, color = state)) +
  stat_ecdf() +
...
GSK.dt = GSK_ratios %>%
  filter(!is.na(diff_TIF)) %>%
  dplyr::select(barcode, diff_TIF, log2_FC_TIF, efficiency_GSK126, efficiency_DMSO_GSK, domains) %>%
  distinct(barcode, .keep_all = TRUE) %>%
  data.table()
