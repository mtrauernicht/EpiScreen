...
## 3B Plot HDACi effects on different chromatin types
```{r 3B HDACi in different clusters}
...
```{r mock data for scaling}
target[nrow(target) + 1,] <- c("Scaling_ctrl", "DNA-PK", 0.25)
```
...
ggplot(TIF_synergy_cluster,
       aes(x = cluster_HDAC, y = H3K27me3)) +
  geom_boxplot(outlier.shape = NA) +
  geom_quasirandom() +
  geom_signif(comparisons = combn(sort(unique(TIF_synergy_cluster$cluster_HDAC)), 2, simplify = F), 
              map_signif_level=sigFunc, 
              step_increase = 0.1)
```

<<<<<<< HEAD
## Filtering out chromatin-interacting compounds
```{r}
# Make matrix with drugs from selected target
TIF_synergy_matrix = all_compounds_epistasis_TIF %>% 
  #filter(abs(H3K27me3) > 0.1) 
  mutate(abs_value = abs(H3K27me3)) %>%
  arrange(desc(abs_value)) %>%
  slice_head(n = 15) %>%
  dplyr::select(-abs_value)
...
pheatmap(TIF_synergy_matrix[, TRIP_chrom_clust$labels],
                 border_color = F,
                 cellwidth = 8,
                 cellheight = 8,
                 cluster_rows = cluster_rows,
                 ...
                 clustering_method = "ward.D2",
                 main = "Chromatin synergies - top hits",
                 color = colors,
                 annotation_colors = annotation_colors_scr, 
                 annotation_legend = T)
=======
## 3D K27me3 CCDs
```{r 3C K27me3 CCDs, fig.width= 10, fig.height= 8}
# Filter the CDD table for only compounds with a CCD on H3K27me3
only_H3K327me3 = all_compounds_epistasis_TIF %>% 
  filter(!between(H3K27me3, -0.01, .01)) # %>%
  # filter(H3K27me3 != 0)

# change these numbers according to where you want to set the cutoff
quant_breaks <- quantile(only_H3K327me3, c(.01,.99), na.rm = T)
palette_length <- 100
breaks <- c(seq(quant_breaks[1], 0, length.out=ceiling(palette_length/2) + 1), 
            seq(quant_breaks[2]/palette_length, quant_breaks[2], length.out=floor(palette_length/2)))
...
pheatmap(only_H3K327me3[, TRIP_chrom_clust$labels],
         border_color = F,
         cellwidth = 8,
         cellheight = 8,
         breaks = breaks,
         ...
         clustering_method = "ward.D",
         annotation_row = target_ctrl,
         main = "TIF CCD with H3K27me3",
         color = colors,
         annotation_colors = annotation_colors_scr, 
         annotation_legend = T)
>>>>>>> 1acb8ebe3c7962e83a4d0e46e056309665c6c79b
...
=======
## 3E m5C CCD
```{r 3E DNA methylation CCD, fig.width= 10, fig.height= 8}
only_m5C = all_compounds_epistasis_TIF %>% 
  filter(!between(m5C, -0.03, .03)) # %>%
  # filter(H3K27me3 != 0)

# change these numbers according to where you want to set the cutoff
quant_breaks <- quantile(only_m5C, c(.01,.99), na.rm = T)
palette_length <- 100
breaks <- c(seq(quant_breaks[1], 0, length.out=ceiling(palette_length/2) + 1), 
            seq(quant_breaks[2]/palette_length, quant_breaks[2], length.out=floor(palette_length/2)))
colors = colorRampPalette(slope.colors %>% pull(color))(length(breaks))
...
pheatmap(only_m5C[, TRIP_chrom_clust$labels],
         border_color = F,
         cellwidth = 8,
         cellheight = 8,
         breaks = breaks,
...
## 3E m5C CCD
```{r 3E CTCF CCD, fig.width= 5, fig.height= 5}
only_CTCF =  all_compounds_epistasis_TIF %>% 
  filter(!between(CTCF, -0.02, .02) ) # %>%
  # filter(H3K27me3 != 0)

# change these numbers according to where you want to set the cutoff
quant_breaks <- quantile(only_CTCF, c(.01,.99), na.rm = T)
palette_length <- 100
breaks <- c(seq(quant_breaks[1], 0, length.out=ceiling(palette_length/2) + 1), 
            seq(quant_breaks[2]/palette_length, quant_breaks[2], length.out=floor(palette_length/2)))
colors = colorRampPalette(slope.colors %>% pull(color))(length(breaks))
...
pheatmap(only_CTCF[, TRIP_chrom_clust$labels],
         border_color = F,
         cellwidth = 8,
         cellheight = 8,
         breaks = breaks,
...
## 3F K27me3 and CTCF correlation
```{r 3F K37me3 vs CTCF CCD correlation, fig.width= 5, fig.height= 5}
...
## S3A
```{r S3A Synergies chromatin - TIF all, fig.width= 10, fig.height= 20}
# change these numbers according to where you want to set the cutoff
...
p2 = plot_comb_grid_beeswarm(GSK.dt, opt_df,
                        domain_levels, 'log2_FC_TIF', min_count = 10, y_plabel=c(-1.5, 1.5))
plot_grid(p1, p2, labels = c("TIF HDACi", "TIF EZH2i"))
...
GSK.dt = GSK_ratios %>%
  filter(!is.na(diff_TIF)) %>%
  dplyr::select(barcode, diff_TIF, log2_FC_MMEJ_NHEJ, ratio_MMEJ_NHEJ, log2_FC_TIF, efficiency_GSK126, efficiency_DMSO_GSK, domains) %>%
  distinct(barcode, .keep_all = TRUE) %>%
  data.table()
...
p1 = plot_comb_grid_beeswarm(vorino.dt, opt_df,
                        domain_levels, 'log2_FC_TIF', min_count = 10, y_plabel=c(-0.3, 0.3))
p2 = plot_comb_grid_beeswarm(GSK.dt, opt_df,
                        domain_levels, 'log2_FC_TIF', min_count = 10, y_plabel=c(-0.3, 0.3))
...
ggplot(KDM6_KD, aes(siRNA, log2_FC_MMEJ_NHEJ, fill = replicate)) + 
  geom_bar(stat = "identity", position = "dodge") +
  facet_grid(. ~ IPR)

ggplot(KDM6_KD, aes(siRNA, log2_FC_TIF, fill = replicate)) + 
  geom_bar(stat = "identity", position = "dodge") +
  facet_grid(. ~ IPR)
```
