...
Which drugs are changing MMEJ ratio? Do they act globally or locally? 
```{r warning = FALSE}
...
# Order IPRs by their starting MMEJ ratio
dmso_freq <- indel.data %>%
  filter(drug == "DMSO") %>%
...
clusters_drugs <- cutree(tree = as.dendrogram(clusters_drugs), k = 4)
clusters_drugs <- data.frame(clusters_drugs)
eff.data.heatmap2 <- cbind(eff.data.heatmap2, clusters_drugs)
clusters_drugs <- eff.data.heatmap2 %>%
  distinct(drug, clusters_drugs)
target <- target %>%
  rownames_to_column("drug") %>%
  left_join(clusters_drugs) %>%
  na.omit()
rownames(target) <- target$drug
target <- target %>%
  dplyr::select(target, clusters_drugs)


pheatmap(as.matrix(eff.data.heatmap2 %>%
                       arrange(target) %>%
           ...
           na_col = "white",
           breaks = myBreaks,
           main = 'MMEJ ratio change per IPR',
...
chromatin_effect <- merge(eff.data.heatmap3, chromatin_ipr) %>%
  ...
  distinct(MMEJ_ratio_norm, drug, chromatin, conc_char, target, clusters_drugs)
