...
indel.data <- readRDS("/DATA/projects/DSBrepair/data/R/rs20220131_episcreen/rs20220131_episcreen_ratios.4.RDS")
```
...
dmso_freq <- indel.data %>%
  ...
  distinct() %>%
  mutate(mean_freq_dmso = mean(dmso_eff),
         dif_freq_dmso = log2(dmso_eff/mean_freq_dmso)) %>%
  distinct(dif_freq_dmso, IPR) %>%
  arrange(desc(dif_freq_dmso))
...
## Correlation of mmej ratio changes with starting mmej ratio
...
for (i in unique(corr_dmso$drug_conc)) {
  corr_dmso$corr[corr_dmso$drug_conc == i] <- cor(corr_dmso$dmso_eff[corr_dmso$drug_conc == i],
                                                  corr_dmso$MMEJ_ratio_norm[corr_dmso$drug_conc == i])
}
...
pheatmap(corr_dmso2 %>%
           #filter(target == "HDAC") %>%
           distinct(drug_conc, corr, corr_repli, viab_mean) %>%
           mutate(viab_mean = log2(viab_mean + 0.3)) %>%
...
ggplot(corr_dmso %>% filter(drug_conc == "Trichostatin A (TSA)_100 nM"),
              aes(x = MMEJ_ratio_norm, y = H3K27me3.zscore)) +
  geom_point() +
...
ggplot(corr_dmso,
              aes(x = dmso_eff, y = MMEJ_ratio_norm, color = target)) +
  geom_point() +
...
ggplot(corr_dmso %>% filter(drug_conc == "Fluoro-SAHA_1 µM"),
              aes(x = dmso_eff, y = MMEJ_ratio_norm)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
  stat_cor(method = "pearson")
