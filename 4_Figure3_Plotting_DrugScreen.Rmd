...
eff.data$sign <- "0_No"
eff.data$sign[eff.data$MMEJ_zscore_global_comb > 1 & eff.data$MMEJ_pval_adj_global_comb < 0.05] <- "1_Up"
eff.data$sign[eff.data$MMEJ_zscore_global_comb < -1 & eff.data$MMEJ_pval_adj_global_comb < 0.05] <- "2_Down"
...
eff.data <- eff.data %>%
  ...
  mutate(fraction = (`1_Up` + `2_Down`) / (`0_No` + (`1_Up` + `2_Down`))) %>%
  pivot_longer(`0_No`:`2_Down`, names_to = "sign", values_to = "number")
...
ggplot_custom(eff.data ,
  ...
  scale_fill_manual(values = c(colors_diverse[1],colors_diverse[2], colors_diverse[4]))
```
...
eff.data.heatmap <- indel.data %>%
  ...
  distinct(conc_char, drug, target, IPR, MMEJ_zscore_comb, MMEJratio) %>%
  mutate(MMEJratio = log2(ave(MMEJratio, drug, conc_char, IPR, FUN = mean))) %>%
  distinct()
...
eff.data.heatmap2 <- eff.data.heatmap %>%
  ...
  filter(min < 0.0000000000001)

#eff.data.heatmap2$MMEJratio[eff.data.heatmap2$MMEJ_zscore_comb > 0.001] <- NA
eff.data.heatmap2 <- eff.data.heatmap2 %>%
  dplyr::select(-min, -MMEJ_zscore_comb) %>%
  spread(IPR, MMEJratio)
...
myBreaks <- c(seq(breaks[1], 0, length.out=palette_length))
...
for (i in unique(eff.data.heatmap2$conc_char)) {
  pheatmap(as.matrix(eff.data.heatmap2[eff.data.heatmap2$conc_char == i,] %>%
           ...
           color = colorRampPalette(c(colors_diverse[2], "white"))(palette_length),
...
for (i in unique(indel.data$target)) {
  ...
  x <- indel.data %>%
    ...
    distinct(mean_eff_zscore_comb, MMEJ_zscore_comb, drug, conc_char, target, IPR) %>%
    mutate(drug_conc = paste(drug, conc_char, sep = "_"))
  
  if (length(unique(x$drug_conc)) > 1) {
  p<- ggplot(x ,
       aes(x = MMEJ_zscore_comb, y = mean_eff_zscore_comb)) +
  geom_point() +
...
eff_mmej_corr <- indel.data %>%
  ...
  mutate(mean_eff_pval_adj_global_comb = p.adjust(mean_eff_pval_global_comb, method = 'fdr')) %>%
  filter(mean_eff_pval_adj_global_comb < 0.001) %>%
  distinct(mean_eff_zscore_comb, MMEJ_zscore_comb, drug, conc_char, target, IPR, mean_eff_pval_global_comb, MMEJ_zscore_global_comb) %>%
  mutate(drug_conc = paste(drug, conc_char, sep = "_")) %>%
  mutate(slope = NA, term = NA, p.value = NA)

for (i in unique(eff_mmej_corr$drug_conc)) {
    model.correlation <- lm(formula = mean_eff_zscore_comb ~ MMEJ_zscore_comb, 
...
eff_mmej_corr_heatmap <- eff_mmej_corr %>%
  ...
  #filter(p.value < 0.05) %>%
  dplyr::select(-term, -IPR, -mean_eff_zscore_comb, -MMEJ_zscore_comb) %>%
  distinct()
