...
## 2E HDAC specific
```{r 2E TIF HDAC heatmap, warning = FALSE, fig.width= 12, fig.height=10}
...
GSK_ratios = RSTP2_IndelRatios_Chromatin_2kb %>% 
  filter(replicate == "mean" & drug %in% c("DMSO_GSK", "GSK126") & plasmid == "LBR2" & barcode %in% barcde_all_reps_gsk) %>%
  ...
  mutate(diff_TIF = efficiency_GSK126 - efficiency_DMSO_GSK,
         ...
         log2_FC_TIF = log2(ratio_TIF),
         diff_MMEJ_MMEJNHEJ = MMEJ_MMEJNHEJ_GSK126 - MMEJ_MMEJNHEJ_DMSO_GSK,
         ratio_MMEJ_MMEJNHEJ = MMEJ_MMEJNHEJ_GSK126/MMEJ_MMEJNHEJ_DMSO_GSK,
         log2_FC_MMEJ_MMEJNHEJ = log2(ratio_MMEJ_MMEJNHEJ)) 
  
Chromdata = unique(RSTP2_IndelRatios_Chromatin_2kb[, c(3, 55:88)])

GSK_ratios %<>% left_join(Chromdata , by = "barcode") %>% distinct(barcode, .keep_all = TRUE)
...
## GSK126
chrom.mods.cols <- seq(grep("log2_FC_MMEJ_MMEJNHEJ", names(GSK_ratios))+1, grep("H3K27me3_domain", names(GSK_ratios))-1)
...
GSK.hist.corr <- CorrelationDFMean(GSK_ratios, "log2_FC_MMEJ_MMEJNHEJ", targets = chrom.mods.cols) %>%
  arrange(corr) %>% pull(rowname) %>% factor(.,levels = .)

lims = 1

CorrelationPlotMean(GSK_ratios, "log2_FC_MMEJ_MMEJNHEJ", targets = GSK.hist.corr, title = "with log2 FC balance after GSK126 treatment")
```
...
GSK.dt = GSK_ratios %>%
  filter(!is.na(diff_TIF)) %>%
  dplyr::select(barcode, diff_TIF, log2_FC_MMEJ_MMEJNHEJ, ratio_MMEJ_MMEJNHEJ, log2_FC_TIF, domains) %>%
  distinct(barcode, .keep_all = TRUE) %>%
  data.table()
...
p1 = plot_comb_grid_beeswarm(GSK.dt, opt_df,
                        domain_levels, 'ratio_MMEJ_MMEJNHEJ', min_count = 10, y_plabel=c(-2, 2))
p1 = plot_comb_grid_beeswarm(GSK.dt, opt_df,
                        domain_levels, 'log2_FC_MMEJ_MMEJNHEJ', min_count = 10, y_plabel=c(-1.5, 1.5))
p1 = plot_comb_grid_beeswarm(GSK.dt, opt_df,
...
```{r}
# indel.data %>% filter(!between(zscore_TIF, -3.8906, 3.8906) & conc_char != "control" & viab_reproducibility & cor_TIF_filter & filter_zscore_TIF_comb_rep) %>% distinct(IPR, drug, replicate, target, tech, conc_char, zscore_TIF, zscore_TIF_comb_tech, zscore_TIF_comb_rep) %>% group_by(IPR, drug, conc_char, target, zscore_TIF_comb_rep) %>% count() %>% filter(n >= 6) %>% View()

indel.data %>% filter(!between(zscore_TIF_comb_rep, -3.2905, 3.2905) & conc_char != "control" & viab_reproducibility & cor_TIF_filter & filter_zscore_TIF_comb_rep) %>% distinct(IPR, drug, replicate, target, tech, conc_char, zscore_TIF, zscore_TIF_comb_tech, zscore_TIF_comb_rep) %>% group_by(IPR, drug, conc_char, target) %>% count() %>% filter(n >= 6)
...
# indel.data %>% filter(zscore_TIF_comb_rep >= 3.2905 & conc_char != "control" & viab_reproducibility & cor_TIF_filter & filter_zscore_TIF_comb_rep) %>% distinct(IPR, drug, replicate, target, tech, conc_char, zscore_TIF, zscore_TIF_comb_tech, zscore_TIF_comb_rep) %>% group_by(IPR, drug, conc_char, target) %>% count() %>% filter(n >= 6) %>% View()

indel.data %>% filter(!between(zscore_bal, -1.96, 1.96) & conc_char != "control" & viab_reproducibility & cor_bal_filter & filter_zscore_bal_comb_rep) %>% distinct(IPR, drug, replicate, target, tech, conc_char, zscore_bal, zscore_bal_comb_tech, zscore_bal_comb_rep) %>% group_by(IPR, drug, conc_char, target) %>% count() %>% filter(n >= 6)
```
