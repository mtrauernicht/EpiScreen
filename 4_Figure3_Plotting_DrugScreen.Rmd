...
dmso_bal <- indel.data %>%
  filter(drug == "DMSO") %>%
  dplyr::select('dmso_bal' = MMEJratio, IPR) %>%
  mutate(max_dmso_bal = ave(dmso_bal, IPR, FUN = mean),
         dmso_bal = mean(dmso_bal)) %>%
  mutate(max_dmso_bal = max(max_dmso_bal)) %>%
  #mutate(max_dmso_bal = max_dmso_bal - dmso_bal) %>%
  distinct(dmso_bal, max_dmso_bal)
...
ggplot(indel.data %>%
              ...
              aes(y = -log10(MMEJ_pval_adj_global_comb), x = MMEJratio, 
                  color = ifelse(MMEJ_pval_adj_global_comb < 0.05 & MMEJratio > (dmso_bal$dmso_bal + 0.02), "green", 
                               ifelse(MMEJ_pval_adj_global_comb < 0.05 & MMEJratio < (dmso_bal$dmso_bal - 0.02), "yellow","blue")))) +
  geom_point() +
  geom_hline(linetype = "dashed", yintercept = -log10(0.05)) +
  ...
  scale_color_manual(values = c("#C2C2C2", "#3d405b", "#e07a5f")) +
  facet_wrap(~conc_char) +
  annotate("rect",xmin=(dmso_bal$dmso_bal - 0.02),xmax=(dmso_bal$dmso_bal + 0.02),ymin=-Inf,ymax=Inf, alpha=0.1, fill="black") +
  annotate("rect",xmin=-Inf,xmax=Inf,ymin=-Inf,ymax=-log10(0.05), alpha=0.1, fill="black") +
  coord_flip()
...
eff.data$sign <- "2_No"
eff.data$sign[eff.data$MMEJratio > (dmso_bal$dmso_bal + 0.02) & eff.data$MMEJ_pval_adj_global_comb < 0.05] <- "1_Up"
eff.data$sign[eff.data$MMEJratio < (dmso_bal$dmso_bal - 0.02) & eff.data$MMEJ_pval_adj_global_comb < 0.05] <- "0_Down"
