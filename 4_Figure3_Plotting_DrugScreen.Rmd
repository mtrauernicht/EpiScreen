...
eff.data.heatmap <- eff.data.heatmap %>%
  ...
  mutate(min = ave(MMEJ_zscore_comb, drug, conc_char, FUN = min)) # %>%  filter(min < 0.05)
...
col_order <- data.frame("Epigenetic Compound" = p1$tree_col[["labels"]],
...
pheatmap(pc1%>%
           column_to_rownames("Epigenetic Compound") %>%
           t(),
         border_color = F,
         cellwidth = 8,
         cellheight = 8,
...
col_order <- data.frame("Epigenetic Compound" = p2$tree_col[["labels"]],
...
col_order <- data.frame("Epigenetic Compound" = p3$tree_col[["labels"]],
