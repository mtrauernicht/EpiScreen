...
## Rank plot MMEJ score

```{r}
indel.data %>% 
  filter(conc_char != "control" & pathway_bal_filter & viab_reproducibility & cor_bal_filter) %>%
  distinct(drug, conc_char, zscore_bal_global_comb_rep , target, viab_reproducibility) %>% 
...
bottom_hits = indel.data  %>% 
  ...
  arrange(zscore_bal_global_comb_rep) %>% 
  slice_head(n = 10) %>% 
  distinct(drug, conc_char, target, zscore_bal_global_comb_rep ) %>% 
  mutate(rank = seq(1:n()),
         drug = gsub(" [(].*[)]", "", drug),
         drug_conc = paste(drug, conc_char, sep = " "))
```
...
## Bar plot: effect sizes MMEJ z-score per target group
...
all.samples = readRDS("/DATA/projects/DSBrepair/git/EpiScreen/FACS_set1.RDS")

vadj.G2 = all.samples %>% filter(variable %in% c("G2", "S", "G1")) %>% group_by(drug_conc, ID) %>% summarise(vadj = sum(mean_value)) %>% mutate(variable = "G2")
vadj.S = all.samples %>% filter(variable %in% c("S", "G1")) %>% group_by(drug_conc, ID) %>% summarise(vadj = sum(mean_value)) %>% mutate(variable = "S")
vadj.G1 = all.samples %>% filter(variable %in% c("G1")) %>% group_by(drug_conc, ID) %>% summarise(vadj = sum(mean_value)) %>% mutate(variable = "G1")
