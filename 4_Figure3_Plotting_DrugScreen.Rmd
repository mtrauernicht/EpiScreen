...
# A: MMEJ-z-score per drug
eff.data <- indel.data %>%
  filter(replicate == "E1504", sample == "drug" | drug == "DMSO") %>%
                dplyr::distinct(drug, conc_char, MMEJscore, sample,
                                tech, replicate, target) 
...
eff.data <- eff.data %>%
  mutate(MMEJ_score_mean = ave(MMEJscore, drug, conc_char, FUN = mean),
         MMEJscore = ave(MMEJscore, drug, conc_char, tech, replicate, FUN = mean)) %>%
  unique()
...
for (i in unique(eff.data$drug)) {
  for (j in unique(eff.data$conc_char)) {
    ...
    eff.data$pval[eff.data$drug == i & eff.data$conc_char == j] <- 
      t.test(eff.data$MMEJscore[eff.data$drug == i & eff.data$conc_char == j], 
                            eff.data$MMEJscore[eff.data$drug == "DMSO"])$p.value
    }
  }
}
...
ggplot_custom(eff.data %>%
                ...
                unique(),
              aes(x = MMEJ_score_mean, y = -log10(pval), color = ifelse(pval < 0.05, "red", "black"))) +
  geom_quasirandom() +
  facet_wrap(~conc_char)
```
*Figure 3A: Compound-induced change in MMEJ score of all tested compounds. Mean MMEJ z-score per drug & concentration vs. p-value from t-test.* 

Conclusion: Yes, there are a lot of interesting hits at all tested concentrations. 

---

## B: MMEJ z-score heatmap: drug vs genomic location
...
eff.data.heatmap <- indel.data %>%
  ...
  distinct(conc_char, drug, IPR, MMEJ_zscore)



eff.data.heatmap2 <- eff.data.heatmap %>%
  mutate(mod = Mod(MMEJ_zscore)) %>%
  mutate(max = ave(mod, drug, conc_char, FUN = max)) %>%
  dplyr::select(-mod) %>%
  spread(IPR, MMEJ_zscore) %>%
  group_by(conc_char) %>%
  top_n(15, max) %>%
  ungroup() %>%
  dplyr::select(-max)
...
eff.data.heatmap <- indel.data %>%
  filter(target == "Epigenetic Reader Domain", replicate == "E1504") %>%
  distinct(conc_char, drug, IPR, MMEJ_zscore) %>%
  mutate(mod = Mod(MMEJ_zscore)) %>%
  mutate(max = ave(mod, drug, conc_char, FUN = max)) %>%
  filter(max > 3 | max < -3) %>%
...
pheatmap(as.matrix(eff.data.heatmap %>%
           ...
           cellheight = 12,
           breaks = breaks,
           main = paste('MMEJ z_score'),
           annotation_row = chromatin,
           color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))(100),
           annotation_colors = annotation_colors_scr, annotation_legend = T)
```
*Figure 3B: Heatmap of MMEJ z-score per IPR for 15 most potent compounds per concentration.*
