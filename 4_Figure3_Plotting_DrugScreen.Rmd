...
eff.data.heatmap2 <- eff.data.heatmap2[,c("drug","conc_char", chromatin_ord$IPR)]

breaks <- quantile(unlist(eff.data.heatmap2 %>% dplyr::select(contains('IPR'))), c(.02,.99)) # change these numbers according to where you want to set the cutoff
palette_length <- 100

myBreaks <- c(seq(breaks[1], -2.58, length.out=ceiling(palette_length/2) + 1), 
              seq(2.58, breaks[2], length.out=floor(palette_length/2)))
...
ggplot(indel.data %>%
         filter(sample == "drug") %>%
         filter(Mod(mean_eff_zscore_global_comb) > 2.58) %>%
...
ggplot(indel.data %>%
         filter(target == "HDAC") %>%
         filter(Mod(mean_eff_zscore_global_comb) > 2.58) %>%
         distinct(mean_eff_zscore_comb, MMEJ_zscore_comb, drug, conc_char, IPR),
       aes(x = MMEJ_zscore_comb, y = mean_eff_zscore_comb)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~drug) +
  ggtitle("efficiency vs. mmej-ratio for HDAC inhibitors per IPR")

eff_mmej_corr <- indel.data %>%
  filter(Mod(mean_eff_zscore_global_comb) > 2.58) %>% 
  distinct(mean_eff_zscore_comb, MMEJ_zscore_comb, target, drug, conc_char, IPR) %>%
...
eff_mmej_corr_heatmap <- eff_mmej_corr %>%
  ...
  #filter(p.value < 0.05) %>%
  dplyr::select(-mean_eff_zscore_comb, -MMEJ_zscore_comb, -term) %>%
  distinct() %>%
  spread(IPR, slope)

eff_mmej_corr_heatmap <- eff_mmej_corr_heatmap[,c("drug_conc", chromatin_ord$IPR)]

palette_length <- 100

myBreaks <- c(seq(-1,1,0.02))

pheatmap(eff_mmej_corr_heatmap %>%
         ...
         cluster_rows = F,
         cluster_cols = F,
         main = paste('correlation: efficiency vs. mmej-score'),
         annotation_row = chromatin,
         color = colorRampPalette(c("#e07a5f", "#e5e5e5", "#3d405b"))(palette_length),
...
ggplot_custom(eff_mmej_corr_heatmap %>%
                distinct(target, drug_conc, slope, p.value) %>%
                filter(p.value < 0.05),
              aes(x = target, y = slope, color = -log10(p.value))) +
  geom_quasirandom() +
