...
indel.data <- readRDS("/DATA/projects/DSBrepair/data/R/mt20230921_episcreen/mt20230921_freq_zscores_0.RDS") 
...
effect_of_compound_tib = filtered_drug_pvals %>%
  ...
  mutate(TIF_signif = case_when(TIF_up_signif <= 1 & TIF_down_signif > 1 ~ "TIF_down", #Annotate how the compounds affects TIF
...
dmso_ratios_tib <- indel.data %>%
  filter(drug == "DMSO") %>%
  distinct(freqCut_mean, barcode, rep) %>%
  mutate(DMSO.freqCut.log2 = log2(freqCut_mean)) %>%
  dplyr::select(-freqCut_mean) %>%
  distinct()
...
epistasis_tib %<>% 
  ...
  mutate(indelrate_slope_plot = case_when(
      effect_on_indelrate & TIF_signif == "TIF_down" & slope < 0 ~ slope, 
      TIF_signif == "TIF_down" & slope > 0 ~ as.double(NA), 
      effect_on_indelrate & TIF_signif == "TIF_up" & slope > 0 ~ slope, 
      TIF_signif == "TIF_up" & slope < 0 ~ as.double(NA), 
      effect_on_indelrate & TIF_signif == "TIF_both" ~ slope,
      T ~ as.double(NA))) %>%
