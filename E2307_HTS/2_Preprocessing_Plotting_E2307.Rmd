...
## Figure S4B: Chromatin landscape of RPE-1 IPRs
```{r}
ipr_chromatin_matrix <- local_effects_chromatin_ccd %>%
  dplyr::select(barcode, contains("chip"), contains("dam"), contains("late")) %>%
  dplyr::select(-dam_LMNB2) %>%
  distinct() %>%
  column_to_rownames("barcode")

### Colors
quant_breaks <- quantile(ipr_chromatin_matrix, c(.01,.99), na.rm = T) # change these numbers according to where you want to set the cutoff
palette_length <- 100
...
pheatmap(t(ipr_chromatin_matrix), 
         color = colors, 
         border_color = NA,
         cellwidth = 8,
         cellheight = 8,
