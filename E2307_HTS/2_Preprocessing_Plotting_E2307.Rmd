...
ggplot(mutations.prefilter.dt.top10.bc %>%
         distinct(barcode, median_barcode_count) %>%
         filter(median_barcode_count > 100 & median_barcode_count < 10000),
       aes(x = reorder(barcode, -median_barcode_count), y = median_barcode_count)) +
  geom_bar(stat = "identity") +
  ylab("Average barcode count per sample") +
  theme_pubr(x.text.angle = 90)


mutations.prefilter.dt.top10.bc <- mutations.prefilter.dt.top10.bc %>%
  filter(median_barcode_count > 4000) # random filtering

mutations.prefilter.dt <- mutations.prefilter.dt %>%
  filter(barcode %in% mutations.prefilter.dt.top10.bc$barcode)
...
CCD_tib <- readRDS("/DATA/projects/DSBrepair/data/R/rs20230921_episcreen/mt20230922_CCD_table_1.RDS")
...
local_effects_chromatin_ccd <- local_effects_chromatin %>%
  mutate(log2_freqcut_dif = log2(freqCut_mean) - log2(DMSO_freq_mean))


## Example plots
ggplot(local_effects_chromatin_ccd %>%
         filter(drug %in% c("AT9283", "PCI_100", "Tuba")),
       aes(x = chip_H3K4me1, y = log2_freqcut_dif)) +
  geom_point() +
  geom_smooth(method = "lm") + 
  facet_wrap(~drug) +
  theme_pubr(border = T)
...
ggplot(local_effects_chromatin_ccd %>%
         filter(drug %in% c("AT9283", "PCI_100", "Tuba")),
       aes(x = dam_H3K9me3, y = log2_freqcut_dif)) +
  geom_point() +
  geom_smooth(method = "lm") + 
  facet_wrap(~drug) +
  theme_pubr(border = T)
...
ggplot(local_effects_chromatin_ccd %>%
         filter(drug %in% c("AT9283", "PCI_100", "Tuba")),
       aes(x = dam_LMNB1, y = log2_freqcut_dif)) +
  geom_point() +
  geom_smooth(method = "lm") + 
  facet_wrap(~drug) +
  theme_pubr(border = T)
...
### Load K562 synergies
CCD_tib_k562 <- readRDS("/DATA/projects/DSBrepair/data/R/rs20230424_episcreen/rs20230424_CCD_table_0.RDS") %>%
  filter(drug_conc %in% c("Tubastatin A 10 µM", "PCI-24781 100 nM")) %>%
  dplyr::select("drug" = drug_conc, "k562_slope" = indelrate_slope_plot, feature) %>%
  na.omit() %>%
  filter(!feature %in% c("HDAC3", "CTCF"))

CCD_tib_k562$drug[CCD_tib_k562$drug == "Tubastatin A 10 µM"] <- "Tuba"
CCD_tib_k562$drug[CCD_tib_k562$drug == "PCI-24781 100 nM"] <- "PCI_100"

CCD_tib_rpe <- CCD_tib %>%
  filter(drug != "PCI_33", feature != "dam_LMNB2") %>%
  dplyr::select(drug, indelrate_slope_plot, feature) %>%
  group_by(drug) %>%
  mutate(indelrate_slope_plot = ifelse(indelrate_slope_plot < 0, NA, indelrate_slope_plot),
         onlyNA = sum(is.na(indelrate_slope_plot))) %>% 
