...
ggplot(mutations.prefilter.dt.top10.bc %>%
         ...
         filter(median_barcode_count > 2500),
...
mutations.prefilter.dt.top10.bc <- mutations.prefilter.dt.top10.bc %>%
  filter(median_barcode_count > 2500) # random filtering
...
pval_df <- data.frame("drug" = unique(global_effects$drug), pval = c("","","","","","",""))

for (i in unique(global_effects$drug)) {
...
barcode_numbering <- local_effects %>%
  distinct(barcode) %>%
...
local_effects_chromatin_2 <- local_effects_chromatin %>%
  filter(!drug %in% c("DMSO", "PCI_1")) %>%
  distinct(barcode, drug, freqCut_dif_mean, DMSO_freq_mean, dam_LMNB1)

ggplot(local_effects_chromatin_2,
       aes(x = DMSO_freq_mean, y = freqCut_dif_mean, color = dam_LMNB1)) +
  geom_point() +
  facet_wrap(~drug) +
  theme_pubr(border = T)
```
...
quant_breaks <- c(-0.01586577, 0.01586577)
palette_length <- 100
