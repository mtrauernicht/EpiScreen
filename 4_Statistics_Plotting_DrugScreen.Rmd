...
# Plotting 
ggplot(indel.data.plot[indel.data.plot$Drug == "DMSO",]) + 
  geom_bar(stat= "identity", aes(x=conc, y=mean.ratio/3)) +
...
ggplot(indel.data.plot[indel.data.plot$Drug == "GSK J4 HCl",], aes(x=conc, y=diff))+
  geom_bar(stat= "identity", aes(x=conc, y=mean.diff/3)) +
...
## Perform t-tests
# t-test for controls
indel.data2 <- indel.data[indel.data$Drug == "DMSO" |
                            indel.data$Drug == "Mirin" |
                            indel.data$Drug == "DNA-PKi",]
indel.data2$logratio.drug <- ave(indel.data2$logratio.platenorm, 
                                 indel.data2$classifier, FUN = function(x) mean(x))
indel.data2 <- indel.data2[,c(6,21,27)]
...
indel.data2$logratio.drug <- ave(indel.data2$logratio.platenorm, 
                                 indel.data2$classifier, FUN = function(x) mean(x))
indel.data2 <- indel.data2[,c(7,14,21,27)]
indel.data2 <- unique(indel.data2)
indel.data2$mean <- ave(indel.data2$logratio.drug, indel.data2$Target, indel.data2$conc, FUN = function(x) mean(x))
indel.data2$sd <- ave(indel.data2$logratio.drug, indel.data2$Target, indel.data2$conc, FUN = function(x) sd(x))

for (j in unique(indel.data2$conc)) {
for (i in unique(indel.data2$Target)) {
    x <- indel.data2[indel.data2$Target == "Negative Control" & indel.data2$conc == j,]
    y <- indel.data2[indel.data2$Target == i & indel.data2$conc == j,]
    
    if (nrow(y)>1) {
    indel.data2$p.value[indel.data2$Target == i & indel.data2$conc == j] <- 
      t.test(x$logratio.drug, y$logratio.drug)$p.value
    
    indel.data2$statistic[indel.data2$Target == i & indel.data2$conc == j] <- 
      t.test(x$logratio.drug, y$logratio.drug)$statistic
  
    indel.data2$conf.int1[indel.data2$Target == i & indel.data2$conc == j] <- 
      t.test(x$logratio.drug, y$logratio.drug)$conf.int[1]
  
    indel.data2$conf.int2[indel.data2$Target == i & indel.data2$conc == j] <- 
      t.test(x$logratio.drug, y$logratio.drug)$conf.int[2]
    }
}
}
...
indel.data2$p.value.adjust <- p.adjust(indel.data2$p.value, n = length(indel.data2$Target/3))
indel.data2 <- indel.data2[,c(1,2,6,5,11)]
indel.data2 <- unique(indel.data2)



# t-test for each drug
...
# Adjust p-values for multiple testing
indel.data2 <- indel.data2[,c(1,3,6,7,8)]
indel.data2 <- unique(indel.data2)
...
indel.data2$logratio <- ave(indel.data2$logratio, indel.data2$integration, FUN = function(x) mean(x))
indel.data2 <- indel.data2[,c(20,36,42)]
indel.data2 <- unique(indel.data2)
indel.data2$mean <- ave(indel.data2$logratio, indel.data2$int, FUN = function(x) mean(x))
indel.data2$sd <- ave(indel.data2$logratio, indel.data2$int, FUN = function(x) sd(x))
indel.data2$p.value <- t.test(indel.data2$logratio[indel.data2$int == 1], indel.data2$logratio[indel.data2$int == 2])$p.value
...
ggplot(hdac.dmso, aes(colour = Drug, group = Drug, fill = Drug)) +
  geom_bar(stat="identity", aes(x =  reorder(integration,order),
                                y = mean)) +
