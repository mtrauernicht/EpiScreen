...
sp <- ggscatter(viability.comp.drugs, x = "viability", y = "X", color = "plate.nr",
   add = "reg.line",
...
indel.data2$`Logratio Change`[abs(indel.data2$MMEJ_zscore)>0.5] <- "0.5-1"
indel.data2$`Logratio Change`[abs(indel.data2$MMEJ_zscore)>1] <- ">1"
...
indel.data2 <- indel.data %>% select(Drug, conc, mean.MMEJ_zscore.drug, mean.efficiency_zscore.drug)
...
ggplot(indel.data2, aes(x = mean.MMEJ_zscore.drug, y = mean.efficiency_zscore.drug)) +
  ...
  gghighlight(abs(mean.MMEJ_zscore.drug) > 2, label_key = Drug) + theme_bw() +
...
ggplot(indel.data2, aes(x = mean.MMEJ_zscore.drug, y = mean.efficiency_zscore.drug)) +
  geom_point() +
  xlab("Log2(+1/-7 ratio) Change") + ylab("Cutting Efficiency Change") +
  gghighlight(abs(mean.efficiency_zscore.drug) > 0.05, label_key = Drug) + theme_bw() +
  theme(text = element_text(size = 14)) +
  facet_wrap(~conc, nrow = 1, ncol = 3)
...
ggplot(indel.data2, aes(x = mean.viability.drug, y = mean.efficiency_zscore.drug)) +
  ...
  gghighlight(abs(mean.efficiency_zscore.drug) > 0.1, label_key = Drug) + theme_bw() +
  theme(text = element_text(size = 14)) +
  facet_wrap(~conc, nrow = 1, ncol = 3)
...
ggplot(indel.data2, aes(x = mean.MMEJ_zscore.drug, y = mean.viability.drug)) +
  ...
  gghighlight(abs(mean.MMEJ_zscore.drug) > 1.5, label_key = Drug) + theme_bw() +
  theme(text = element_text(size = 14)) +
  facet_wrap(~conc, nrow = 1, ncol = 3)
...
ggplot(indel.data2, aes(x=conc, y=MMEJ_zscore)) +
  geom_quasirandom() + 
  coord_cartesian(ylim = c(-4, 4)) +  
  gghighlight(abs(MMEJ_zscore) > 2, label_key = Drug) + theme_bw()
  
# The same but only the range between 1 & 2
ggplot(indel.data2[abs(indel.data2$MMEJ_zscore) < 2,], aes(x=conc, y=MMEJ_zscore)) +
  geom_quasirandom() + 
  coord_cartesian(ylim = c(-4, 4)) +  
  gghighlight(abs(MMEJ_zscore) > 1, label_key = Drug) + theme_bw()
