...
## Data analysis
```{r data analysis, echo = FALSE, warning = FALSE}
## Normalize the data
...
ggplot(data = indel.data[indel.data$Drug == c("DMSO","DNA-PKi","Mirin"),], 
       aes(x = logratio.platenorm, fill = Target)) + theme_classic() +
...
ggplot(data = indel.data, 
       aes(x = efficiency.platenorm, fill = Target)) + geom_density()+ theme_classic() +
...
indel.data2 <- indel.data %>% select(Drug, conc, mean.logratio.drug, mean.efficiency.drug)
...
ggplot(indel.data2, aes(x = mean.logratio.drug, y = mean.efficiency.drug)) +
  ...
  gghighlight(abs(mean.logratio.drug) > 2, label_key = Drug) + theme_bw() +
...
ggplot(indel.data2, aes(x = mean.logratio.drug, y = mean.efficiency.drug)) +
  geom_point() +
  xlab("Log2(+1/-7 ratio) Change") + ylab("Cutting Efficiency Change") +
  gghighlight(abs(mean.efficiency.drug) > 0.05, label_key = Drug) + theme_bw() +
  theme(text = element_text(size = 14)) +
  facet_wrap(~conc, nrow = 1, ncol = 3)
...
indel.data2 <- indel.data %>% select(Drug, conc, mean.viability.drug, mean.efficiency.drug) %>% unique()
indel.data2$conc <- factor(indel.data2$conc, levels = c("100nm", "1um", "10um"))

ggplot(indel.data2, aes(x = mean.viability.drug, y = mean.efficiency.drug)) +
  geom_point() +
  xlab("Viability") + ylab("Cutting Efficiency Change") +
  gghighlight(abs(mean.efficiency.drug) > 0.1, label_key = Drug) + theme_bw() +
  theme(text = element_text(size = 14)) +
  facet_wrap(~conc, nrow = 1, ncol = 3)


# Correlation plot mean viability vs. mean ratio
indel.data2 <- indel.data %>% select(Drug, conc, mean.logratio.drug, mean.viability.drug) %>% unique()
indel.data2$conc <- factor(indel.data2$conc, levels = c("100nm", "1um", "10um"))

ggplot(indel.data2, aes(x = mean.logratio.drug, y = mean.viability.drug)) +
  geom_point() +
  xlab("Log2(+1/-7 ratio) Change") + ylab("Viability") +
  gghighlight(abs(mean.logratio.drug) > 1.5, label_key = Drug) + theme_bw() +
  theme(text = element_text(size = 14)) +
  facet_wrap(~conc, nrow = 1, ncol = 3)
...
indel.data.rep1 <- indel.data.rep1 %>% select(barcode, Number, conc, logratio)
indel.data.rep2 <- indel.data.DNAPK[grep("rep2", indel.data.DNAPK$condition),]
indel.data.rep2 <- indel.data.rep2 %>% select(barcode, Number, conc, logratio)
indel.data.rep3 <- indel.data.DNAPK[grep("rep3", indel.data.DNAPK$condition),]
indel.data.rep3 <- indel.data.rep3 %>% select(barcode, Number, conc, logratio)
...
indel.data2 <- indel.data2 %>% select(Drug, conc, logratio.platenorm)
...
ggplot(indel.data2, aes(x=conc, y=logratio.platenorm)) +
  ...
  gghighlight(abs(logratio.platenorm) > 2, label_key = Drug) + theme_bw()
...
ggplot(indel.data2[abs(indel.data2$logratio.platenorm) < 2,], aes(x=conc, y=logratio.platenorm)) +
  geom_quasirandom() + 
  coord_cartesian(ylim = c(-4, 4)) +  
  gghighlight(abs(logratio.platenorm) > 1, label_key = Drug) + theme_bw()
...
ggplot(data = indel.data[indel.data$Drug == "DMSO",]) +
  geom_boxplot(mapping = aes(x = reorder(integration, order), y = efficiency)) + theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1.0, vjust = 0))+
...
for (i in unique(indel.data$Target)) {
  ...
  p <- ggplot(data = data) +
    geom_quasirandom(mapping = aes(x = reorder(integration,order), y = logratio.platenorm, colour = Target), alpha = 0.8, dodge.width = -0.5) + theme_bw() +
    theme(axis.text.x = element_text(angle = 90, hjust = 1.0, vjust = 0))+ 
...
hdac.dmso <- hdac.dmso %>% select(Drug, logratio.platenorm, 
                                                     efficiency.platenorm, order, 
                                                     integration) %>% unique()
