...
```{r}
# Remove not needed data
mapping_mean <- dplyr::select(mapping_mean, ends_with("id"), starts_with("mean"))

# Rename variables 
names(mapping_mean)[2:ncol(mapping_mean)] <- gsub(".*_(.*?_.*?_.*?).txt", "\\1", file.list_mean)
...
# Save heatmap for further use (exclude barcodes that were discarded in episcreen analysis)
setwd("/DATA/usr/m.trauernicht/EpiScreen/epigenetic-screening-on-trip-clone/Results/")
tiff("clone5.tiff", units="in", width=8, height=5, res=300)
pheatmap(as.matrix(mapping_mean[,c(-1,-11)]), annotation_col = LAD_integration, 
         annotation_row = chromatin.mark_type, 
         annotation_colors = annotation_colors,
         color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))(100),
         breaks = breaksList,
...
x <- pheatmap(as.matrix(mapping_mean[,c(-1,-11)]), annotation_col = LAD_integration, 
         annotation_row = chromatin.mark_type, 
         annotation_colors = annotation_colors,
         color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))(100),
         breaks = breaksList,
...
mapping_mean_reduced <- mapping_mean[c(1,3:4,6,13),]

pheatmap(as.matrix(mapping_mean_reduced[,-1]), annotation_col = LAD_integration, 
         annotation_row = chromatin.mark_type, 
         annotation_colors = annotation_colors,
         color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))(100),
         breaks = breaksList,
