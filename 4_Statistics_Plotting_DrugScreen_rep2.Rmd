...
### Compare DMSO indel scores between replicates
...
### Compute indel scores for each unique drug-conc-integration
...
indel.data.rep1 <- indel.data2 %>%
  filter(rep == 'R1') %>%
  select(barcode, drug, conc, 'R1' = MMEJ_zscore)
indel.data.rep2 <- indel.data2 %>%
  filter(rep == 'R2') %>%
  select(barcode, drug, conc, 'R2' = MMEJ_zscore)
indel.data.rep3 <- indel.data2 %>%
  filter(rep == 'R3') %>%
  select(barcode, drug, conc, 'R3' = MMEJ_zscore)

indel.data.rep <- merge(indel.data.rep1, indel.data.rep2, all = T)
indel.data.rep <- merge(indel.data.rep, indel.data.rep3, all = T)

# Correlation matrix plot
correlation.plot.rep <- indel.data.rep %>% select(R1, R2, R3) %>% na.omit()
...
ggplot(indel.data %>%
         ...
         mutate(mean_MMEJscore_int = ave(MMEJscore_norm, drug, 
                                           conc, integration, FUN = function(x) mean(x))) %>%
         select(mean_MMEJscore_int, drug, integration, conc) %>%
         unique() %>%
         mutate(mean_MMEJscore = ave(mean_MMEJscore_int, drug, conc, FUN = function(x) mean(x)),
                sd_MMEJscore = ave(mean_MMEJscore_int, drug, conc, FUN = function(x) sd(x))) %>%
         select(mean_MMEJscore, sd_MMEJscore, drug, conc) %>%
         unique(),
       aes(x = mean_MMEJscore, y = sd_MMEJscore)) +
  geom_point() +
  theme_bw() +
  facet_wrap(~conc)
...
ggplot(indel.data %>%
         filter(drug != "DNA-PKi", target != "Negative Control", target != "MRN") %>%
         mutate(mean_MMEJ_zscore_int = ave(MMEJscore_norm, drug, 
                                           conc, integration, FUN = function(x) mean(x))) %>%
...
ggplot(indel.data %>%
         ...
         mutate(mean_MMEJscore_int = ave(MMEJscore_norm, drug, 
                                           conc, integration, FUN = function(x) mean(x))) %>%
         select(mean_MMEJscore_int, drug, target, integration, conc) %>%
         unique() %>%
         mutate(mean_MMEJscore = ave(mean_MMEJscore_int, drug, conc, FUN = function(x) mean(x)),
                sd_MMEJscore = ave(mean_MMEJscore_int, drug, conc, FUN = function(x) sd(x))) %>%
         select(sd_MMEJscore, drug, conc, target) %>%
         unique(),
       aes(x = target, y = sd_MMEJscore)) +
  geom_quasirandom() +
