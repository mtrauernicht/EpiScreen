...
# indel.data.r1 <- get(load("/DATA/usr/m.trauernicht/projects/EpiScreen/files_scripts/mt20210504_drug_screen_mutations_rep1")) %>%
#   mutate(biorep = "rep1")
indel.data <- get(load("/DATA/projects/DSBrepair/data/R/EpiScreen/rs20210528_drug_screen_mutations_rep2"))

integrations <- readRDS('/DATA/projects/DSBrepair/data/R/rs20200311_domains_clone5.RDS') %>%
  dplyr::select('barcode' = ID, 'domain' = group, IPR)

# Import viability data
viability.comp <- read.csv2("/DATA/usr/m.trauernicht/projects/EpiScreen/files_scripts/mt20200618_viability_rep1-2.csv") %>%
  setnames(".id", "id")
```
...
ggplot(indel.data2 %>%
         filter(group != "PAO") %>%
         mutate(viability = (E1504 + E177) / 2) %>%
         dplyr::select(drug, group, concentration, viability) %>%
         unique(),
       aes(x = "x", y = viability, color = group)) +
...
indel.data$IPR <- factor(indel.data$IPR, levels = c("IPR1", "IPR2", "IPR3", "IPR4", "IPR5",
                                                                    "IPR6", "IPR7", "IPR8", "IPR9", "IPR10",
                                                                    "IPR11", "IPR12", "IPR13", "IPR14", 
                                                                    "IPR15", "IPR16", "IPR17", "IPR18", "IPR19"))
...
indel.data <- indel.data[indel.data$barcode != "CTCTTAATCGCTGCC" & indel.data$barcode != "ACCCCTAAAGGCGCTG",]


# ACCCCTAAAGGCGCTG should be removed
...
ggplot(indel.data %>% 
         dplyr::select(MMEJscore, condition, IPR, drug, replicate) %>%
         filter(drug == "DMSO") %>%
...
indel.data.DMSO.rep <- indel.data %>% 
  dplyr::select(MMEJscore, condition_2, IPR, drug, replicate, plate) %>%
  filter(drug == "DMSO") %>%
  dcast(condition_2 + IPR + drug + plate ~ replicate, value.var = "MMEJscore")
...
indel.data.DMSO.rep <- indel.data %>% 
  dplyr::select(freqCut, condition_2, IPR, drug, replicate, plate) %>%
  filter(drug == "DMSO") %>%
  dcast(condition_2 + IPR + drug + plate ~ replicate, value.var = "freqCut")

sp <- ggscatter(indel.data.DMSO.rep, x = "E177", y = "E1504",
                size = 2, add = "reg.line",
   add.params = list(color = colors_diverse[5], fill = "lightgray"), title = "MMEJscore rep 1 vs. rep 2",
   conf.int = TRUE, ylab = "rep 2", xlab = "rep 1")

sp + stat_cor(method = "pearson", label.x = 0.6, label.y = 0.9) + theme_classic()
```
...
indel.data <- merge(indel.data, dmso.mmej.score)
...
indel.data <- indel.data %>%
  mutate(MMEJ_zscore_mean_bio = ave(MMEJ_zscore, IPR, drug, concentration, replicate, FUN = function(x) mean(x)),
         MMEJ_zscore_mean = ave(MMEJ_zscore_mean_bio, IPR, drug, concentration, FUN = function(x) mean(x)))

# Do the same for the normalized efficiency
indel.data <- indel.data %>%
  mutate(freqCut_norm_mean_bio = ave(freqCut_norm, IPR, drug, concentration, replicate, FUN = function(x) mean(x)),
         freqCut_norm_mean = ave(freqCut_norm_mean_bio, IPR, drug, concentration, FUN = function(x) mean(x)))
...
indel.data <- indel.data %>%
  mutate(MMEJ_zscore_mean_bio_drug = ave(MMEJ_zscore_mean_bio, drug, concentration, replicate, FUN = function(x) mean(x)),
         MMEJ_zscore_mean_drug = ave(MMEJ_zscore_mean, drug, concentration, FUN = function(x) mean(x)),
         freqCut_norm_mean_bio_drug = ave(freqCut_norm_mean_bio, drug, concentration, replicate, FUN = function(x) mean(x)),
         freqCut_norm_mean_drug = ave(freqCut_norm_mean_bio, drug, concentration, FUN = function(x) mean(x)))
```
...
ggplot(data = indel.data %>%
         dplyr::select(MMEJ_zscore_mean_bio, sample, drug, concentration, IPR, replicate) %>%
         unique(), 
...
indel.data2$zscore_change[abs(indel.data2$MMEJ_zscore_mean)>1] <- "1-2"
indel.data2$zscore_change[abs(indel.data2$MMEJ_zscore_mean)>2] <- ">2"
