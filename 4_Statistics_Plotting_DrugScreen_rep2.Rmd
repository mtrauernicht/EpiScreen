...
# S_A: Correlate viability data from the 2 replicates, highlight controls
ggplot_custom(viability.data %>%
...
# C: Cutting efficiency per drug - vulcano plot?
eff.data <- indel.data %>%
                dplyr::distinct(drug, freqCut_norm_mean_drug, freqCut_norm_split, conc_char, sample,
                                tech, replicate) %>%
...
# D: MMEJ-z-score per drug
eff.data <- indel.data %>%
                dplyr::distinct(drug, conc_char, MMEJ_zscore_drug_comb, MMEJscore, sample,
                                tech, replicate) 

dmso.mmej.score <- indel.data %>%
  filter(drug == "DMSO") %>%
...
eff.data <- merge(eff.data, dmso.mmej.score)

eff.data <- eff.data %>%
  mutate(MMEJ_zscore = (MMEJscore - mean_DMSO) / sd_DMSO) %>%
  mutate(MMEJ_zscore = ave(MMEJ_zscore, replicate, drug, tech, FUN = mean)) %>%
  dplyr::select(-MMEJscore) %>%
  unique()
...
for (i in unique(eff.data$drug)) {
  for (j in unique(eff.data$conc_char)) {
    if (nrow(eff.data[eff.data$drug == i & eff.data$conc_char == j]) >= 2) {
    eff.data$pval[eff.data$drug == i & eff.data$conc_char == j] <- 
      t.test(eff.data$MMEJ_zscore[eff.data$drug == i & eff.data$conc_char == j], 
                            eff.data$MMEJ_zscore[eff.data$drug == "DMSO"])$p.value
    }
  }
}
...
ggplot_custom(eff.data %>%
                filter(sample == "drug") %>%
                dplyr::select(-MMEJ_zscore) %>%
                unique(),
              aes(x = MMEJ_zscore_drug_comb, y = -log10(pval), color = ifelse(pval < 0.05, "red", "black"))) +
  geom_quasirandom() +
  facet_wrap(~conc_char)
...
techn_repl <- indel.data %>%
  filter(sample == "drug") %>%
  dplyr::distinct(freqCut_norm_split, drug, tech, replicate, conc_char, sample) %>%
  mutate(freqCut_norm_split = ave(freqCut_norm_split, drug, tech, replicate, conc_char, FUN = mean)) %>%
  unique() %>%
  mutate(rep = paste(replicate, tech, sep = "_")) %>%
  dplyr::select(-replicate, -tech) %>%
  spread(rep, freqCut_norm_split) %>%
  na.omit()
...
# S_D: Correlation between biological replicates MMEJ score
...
# S_E: Correlation between biological replicates efficiency
biol_repl <- indel.data %>%
  filter(sample == "drug") %>%
  dplyr::distinct(freqCut_norm_drug, drug, replicate, conc_char, sample) %>%
  spread(replicate, freqCut_norm_drug) %>%
  na.omit()
...
```{r}
# A: Correlation of efficiency of DMSO samples of all genomic locations between biological replicates
ggplot_custom(indel.data %>%
         filter(drug == "DMSO") %>%
         dplyr::distinct(freqCut, replicate, barcode, tech, plate, conc_char, well) %>%
...
# B: Cutting efficiency in DMSO per IPR
...
ggplot(indel.data2 %>%
         ...
         dplyr::distinct(freqCut, IPR, tech, plate, conc_char, drug, sample) %>%
           mutate(freqCut = ave(freqCut, IPR, tech, plate, conc_char, drug, FUN = mean)),
       aes(x = IPR, y = freqCut)) +
  geom_quasirandom_rast(aes(color = sample), dodge.width = 0.75) +
  scale_color_manual(values = c(colors_drugs[2], colors_drugs[6], colors_drugs[7], colors_drugs[4]))


ggplot(indel.data2 %>%
         filter(sample != "DNA-PKi (NHEJ control)", sample != "Mirin (MMEJ control)") %>%
         dplyr::distinct(MMEJscore, IPR, tech, plate, conc_char, drug, sample) %>%
           mutate(freqCut = ave(MMEJscore, IPR, tech, plate, conc_char, drug, FUN = mean)),
...
ggplot_custom(data = indel.data[indel.data$drug == c("DMSO","DNA-PKi","Mirin"),], 
...
ggplot_custom(data = indel.data %>%
                filter(sample != "DNA-PKi (NHEJ control)", sample != "Mirin (MMEJ control)", sample != "DMSO (neg control)", conc_char == "100 nM") %>%
                  dplyr::select(freqCut_norm_mean_drug, sample, drug, conc_char, target) %>%
                  unique() %>%
                mutate(mean_eff_target = ave(freqCut_norm_mean_drug, target, FUN = mean)), 
              aes(y = log2(freqCut_norm_mean_drug), x = reorder(target, -mean_eff_target))) + 
  geom_quasirandom() +
    geom_boxplot(position = position_dodge(0.75), alpha = 0.4) +
    xlab("") + ylab("log2(cutting efficiency)") +
