...
ggplot_custom <- function(...) ggplot2::ggplot_custom(...) + 
...
ggplot_custom(viability.data2 %>%
         filter(group != "PAO", group != "DMSO") %>%
         mutate(viab_norm = (E1504 + E177) / 2) %>%
         dplyr::select(drug, group, conc_char, viab_norm) %>%
         mutate(viab_norm = ave(viab_norm, drug, conc_char, FUN = function(x) mean(x))) %>%
         unique(),
       aes(x = conc_char, y = viab_norm, color = ifelse(viab_norm < 0.45, "red", colors_diverse[1]))) +
  geom_quasirandom() +
...
ggplot_custom(viability.data2, aes(x = target, y = log2(dif), color = changed)) +
  geom_quasirandom() +
...
ggplot_custom(viability.data2, aes(x = E1504_1, y = E177_1)) +
  geom_point() +
  geom_smooth(method = "lm", color = colors_diverse[1]) +
  geom_segment(x = 0.5, xend = 0.5, y = 0.5, yend = 1.2, linetype = "dashed") +
  geom_segment(x = 0.5, xend = 1.2, y = 0.5, yend = 0.5, linetype = "dashed") 
...
ggplot_custom(data = indel.data %>%
                filter(sample != "DNA-PKi (NHEJ control)", sample != "Mirin (MMEJ control)") %>%
                  dplyr::select(MMEJ_zscore_mean_drug, sample, drug, conc_char) %>%
                  unique(), 
              aes(y = MMEJ_zscore_mean_drug, x = conc_char, color = sample)) + 
    geom_quasirandom(dodge.width = 0.75) +
    geom_boxplot(position = position_dodge(0.75), alpha = 0.4) +
    xlab("") + ylab("MMEJ z-score") +
    scale_color_manual(values = c("#65BFA4", "#B3B3B3"))
