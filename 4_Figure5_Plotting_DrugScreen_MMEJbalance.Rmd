...
balance.data.heatmap <- indel.data %>%
  ...
  mutate(MMEJ_log2_FC_mean = ave(MMEJ_log2_FC, IPR, FUN = function(x) mean(x, na.rm = TRUE)),
         ...
         adj_pvalue = p.adjust(p_value, method = 'holm'),
...
```{r S2A TIF heatmaps, warning = FALSE, fig.width = 8, fig.height = 12}
...
```{r 5B Top TIF boosting, fig.width = 8, fig.height = 8}
rank_dt = indel.data %>% 
  ...
  mutate(viab_mean = ave(viab_mean, drug, conc_char, FUN = mean),
         ...
         adj_pval_bal = p.adjust(p_val_bal, method = 'holm')) %>%
  distinct() %>%
  na.omit() %>%
  mutate(rank = seq(1:n()),
         drug = gsub(" [(].*[)]", "", drug),
...
```{r 5E Vorinostat pool, fig.width = 12, fig.height = 10}
...
## 5E: Vorinostat MMEJ FC per chromatin group
Aim: Present a detailed view of the Vorinostat pool data
```{r 5E Vorino upset pool, fig.width = 10, fig.height = 6}
p1 = plot_comb_grid_beeswarm(vorino.dt, opt_df,
                             domain_levels, 'log2_FC_MMEJ_NHEJ', min_count = 10, y_plabel=c(-2.8, 2.8))
...
```{r S5A Cell cycle FACS, fig.width = 8, fig.height = 4}
...
### Heatmaps
```{r S5B CCD heatmap, fig.width= 14, fig.height= 14}
...
## S5C K27me3 top CCDs
```{r S5C strongest effect in H3K27me3, fig.width= 6, fig.height= 6}
rank_dt = CCD_tib %>% 
...
breaks <- quantile(unlist(balance.data.heatmap2 %>% 
                            dplyr::select(contains('IPR'))), c(.01,.91), na.rm = T) # change these numbers according to where you want to set the cutoff
palette_length <- 100
...
```{r S5C Vorino TIF FC Correlations, fig.width = 6, fig.height = 8}
## Vorinostat
