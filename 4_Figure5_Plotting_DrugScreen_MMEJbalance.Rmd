...
```{r 5A MMEJ FC heatmap, warning = FALSE, fig.width = 8, fig.height = 12}
...
ggplot(decitabine_nu7441_df %>%
  ...
  geom_errorbar(aes(ymin=percentage_mean-percentage_sd, ymax=percentage_mean+percentage_sd), width=.2,
                 position=position_dodge(.75))  +
  scale_fill_manual("legend", values = c(Decitabine_ssODN = "#D69C81", NU7441_ssODN = "#779ed7", Decitabine_NU7441_ssODN = "black", DMSO_ssODN = "grey70")) + 
  scale_color_manual("legend", values = c(Decitabine_ssODN = "#D69C81", NU7441_ssODN = "#779ed7", Decitabine_NU7441_ssODN = "black", DMSO_ssODN = "grey70")) + 
  #scale_y_continuous(name="frequency", labels = percent) +
...
```{r 5F Validations vs screen in HDACi and EZH2i, fig.width=8, fig.height=8}
vorinostat_validation = indel.data.validation %>% 
  filter(replicate == "mean" & plasmid == "LBR2", drug %in% c("DMSO", "Vorinostat"), sum_bc_reads > 1000)%>% 
  dplyr::select(barcode, drug, MMEJ_NHEJ, efficiency, all_of(chromatin_cols)) %>%
  distinct() 
...
vorino_screen_K27 = indel.data %>% 
  filter((grepl("Vorinostat", drug) & conc_char == "1 µM") | 
           (grepl("3-Deazane", drug) & conc_char == "1 µM")) %>% 
...
## 5x CCD Pool
```{r S5x CCD pool filtered, fig.height= 6, fig.width= 8}
# Load pool data
...
all_compounds_epistasis_TIF_HDM = CCD_tib %>%
  #mutate(drug_conc = paste(drug, conc_char, sep = " ")) %>%
  dplyr::select(drug_conc, ratio_slope_plot, feature) %>%
...
validation_epistasis_bal <- rbind(validation_epistasis_bal, all_compounds_epistasis_TIF_HDM)
