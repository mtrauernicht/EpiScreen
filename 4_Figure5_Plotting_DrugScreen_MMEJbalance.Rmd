...
viab_drug = indel.data %>% filter(drug != "PAO") %>% 
  ...
  dplyr::summarise(viab = mean(viab_mean))
...
NHEJboost_drug_conc = number_drugs %>% 
  # Add filtering for significant increases
  filter(adj_pvalue < 0.01 & zscore_bal_comb_rep_temp < -1) %>% 
  # unique the drugs
  distinct(drug_conc) %>% 
...
balance.clustering <- balance.data.heatmap %>%
  filter(drug_conc %in% top_50_hits$drug_conc) %>%
  mutate(mean_eff_drug = ave(MMEJ_log2_FC_mean, drug, IPR, FUN = mean),
         drug = gsub(" [(].*[)]", "", drug)) %>%
  distinct(mean_eff_drug, drug, IPR) %>%
  spread(IPR, mean_eff_drug) %>%
  column_to_rownames(var = "drug")

balance.clustering.dist =  dist(balance.clustering, method = "manhattan")

balance.clustering.clust = hclust(balance.clustering.dist, method = "ward.D2")

drug_order_grouped = balance.data.heatmap %>% distinct(drug, conc_char, drug_conc) %>%
  mutate(conc_char = factor(conc_char, levels = c("100 nM", "1 µM", "10 µM"))) %>%
  arrange(conc_char) %>%
  arrange(factor(drug, levels = labels(balance.clustering.clust))) %>%
  pull(drug_conc)
...
plt1 = pheatmap(balance.data.matrix[drug_order_grouped , clone5_map_ipr$labels] %>%
                  na.omit(),
...
MMEJboost = number_drugs %>% 
  ...
  # unique the drugs
  distinct(drug, drug_conc, adj_pvalue, zscore_bal_global_comb_rep_temp) 
...
paste("Of the 160 epigenetic compounds significantly altering the MMEJ:NHEJ ratio globally,", 
      length(unique(MMEJboost$drug)), "boosted the ratio and", length(unique(NHEJboost$drug)), "decreased the ratio.")

# Export data table S10
export_ratios <- indel.data %>%
    mutate(drug = gsub(" [(].*[)]", "", drug),
           drug_conc = paste(drug, conc_char, sep = " ")) %>%
  filter(!drug %in% c("DNA-PKi", "Mirin", "PAO"), conc_char != "control") %>%
  distinct(drug_conc, IPR, freqMMEJ, freqNHEJ) %>%
  na.omit() %>%
  mutate(freqMMEJ = ave(freqMMEJ, drug_conc, IPR, FUN = function(x) mean(x, na.rm = T)),
         freqNHEJ = ave(freqNHEJ, drug_conc, IPR, FUN = function(x) mean(x, na.rm = T))) %>%
  distinct()

ratios_dmso <- export_ratios %>%
  filter(drug_conc == "DMSO 1 µM") %>%
  dplyr::select(IPR, "dmsoMMEJ" = freqMMEJ, "dmsoNHEJ" = freqNHEJ)

export_ratios <- export_ratios %>%
  filter(drug_conc != "DMSO 1 µM") %>%
  left_join(ratios_dmso) %>%
  mutate(difMMEJ = freqMMEJ - dmsoMMEJ,
         difNHEJ = freqNHEJ - dmsoNHEJ) %>%
  distinct(drug_conc, IPR, difMMEJ, difNHEJ)

number_drugs_exp <- indel.data %>%
  #filter(!target %in% c("Aurora Kinase", "JAK", "PARP", "PIM", "HIF")) %>%
  mutate(drug = gsub(" [(].*[)]", "", drug),
         drug_conc = paste(drug, conc_char, sep = " ")) %>%
  filter(pathway_bal_filter == TRUE & conc_char != "control") %>%
  distinct(drug_conc, IPR, zscore_bal_comb_rep, MMEJ_log2_FC) %>%
  na.omit() %>%
  mutate(zscore_bal_comb_rep = ave(zscore_bal_comb_rep, drug_conc, IPR, FUN = function(x) mean(x, na.rm = T))) %>%
  mutate(MMEJ_log2_FC = ave(MMEJ_log2_FC, drug_conc, IPR, FUN = function(x) mean(x, na.rm = T))) %>%
  distinct()
           
export_ratios2 <- export_ratios %>%
  left_join(number_drugs_exp)

export_ratios3 <- export_ratios2 %>%
  mutate(sign_effect = ifelse(drug_conc %in% MMEJboost$drug_conc, "up", "unchanged")) %>%
  mutate(sign_effect = ifelse(drug_conc %in% NHEJboost$drug_conc, "down", sign_effect))
  
ggplot(export_ratios3 %>% filter(sign_effect == "down"), aes(x = difMMEJ, y = difNHEJ)) + 
  geom_point() +
  theme_pubr()
  
write.csv(export_ratios3, file = "data/mmejfreq_s10.csv", row.names = FALSE)
```
...
decitabine_nu7441_all <- read_csv("files_scripts/decitabine_synergy_indels.csv") %>%
...
ggplot(vorino_k27.plot %>%
  ...
  scale_color_manual(values = c("HDACi" = "#157834", "EZH2i" = "#FDCDAC"))
```
...
# Import flow cytometry data - this data is derived from 3_Processing_FACS.Rmd
all.samples = read_csv2("files_scripts/mt20220802_facs_all_drugs.csv")
...
# Show only drugs used in the screen with a significant effect.
drugs_keep = number_drugs %>% 
  # Add filtering for significant increases
  filter(adj_pvalue < 0.01 & drug_conc %in% drugs_tworeps) %>%
  distinct(drug_conc) %>%
  pull()



# How many drug/concentrations were tested?
length(drugs_keep)

# Add control again
drugs_keep = c(drugs_keep, "DMSO 10 µM")


exps.facs.G2 = all.samples %>% 
  mutate(drug_conc = paste(drug, conc, sep = " ")) %>% 
  filter(variable == "G2" & drug_conc %in% drugs_keep) %>% 
  distinct(drug_conc, value, rep)

exps.facs.G2 %>% t_test(., value ~ drug_conc, ref.group = "DMSO 10 µM", p.adjust.method = "BH") %>% filter(p.adj < 0.05)

exps.facs.G1 = all.samples %>%
  mutate(drug_conc = paste(drug, conc, sep = " ")) %>% 
  filter(variable == "G1" & drug_conc %in% drugs_keep) %>% 
  distinct(drug_conc, value, rep)

exps.facs.G1 %>% t_test(., value ~ drug_conc, ref.group = "DMSO 10 µM", p.adjust.method = "BH") %>% filter(p.adj < 0.05)

exps.facs.S = all.samples %>%
  mutate(drug_conc = paste(drug, conc, sep = " ")) %>% 
  filter(variable == "S" & drug_conc %in% drugs_keep) %>% 
  distinct(drug_conc, value, rep)

exps.facs.S %>% t_test(., value ~ drug_conc, ref.group = "DMSO 10 µM", p.adjust.method = "BH") %>% filter(p.adj < 0.05)
...
all.samples <- all.samples %>%
  ...
  filter(drug_conc %in% drugs_keep) %>%
...
vadj.G2 = all.samples %>% filter(variable %in% c("G2", "S", "G1")) %>% 
  group_by(drug_conc_exp) %>% summarise(vadj = sum(mean_value)) %>% mutate(variable = "G2")
vadj.S = all.samples %>% filter(variable %in% c("S", "G1")) %>% 
  group_by(drug_conc_exp) %>% summarise(vadj = sum(mean_value)) %>% mutate(variable = "S")
vadj.G1 = all.samples %>% filter(variable %in% c("G1")) %>% 
  group_by(drug_conc_exp) %>% summarise(vadj = sum(mean_value)) %>% mutate(variable = "G1")
...
all.samples.plot = all.samples  %>%
  ...
  mutate(variable = factor(variable, levels = c("G2", "S", "G1")),
         target = case_when(drug == "DMSO" ~ "Vehicle",
                            T ~ target)) %>%
  na.omit()
...
all.samples.plot <- all.samples.plot %>% 
  filter(drug != "PAO") %>%
  mutate(drug_conc = factor(drug_conc, levels = c("DMSO 10 µM", drugs_order$drug_conc)))
...
clust_CCD = CCD_tib %>%
  ...
  mutate(ratio_slope_plot = ave(ratio_slope_plot, drug, feature, 
...
### Comparing Pool & Clone CCDs of Vorinostat
```{r}
file = list.files(in.dir, pattern = "CCD_validation_table", full.names = T) %>% tail(n = 1)
vorinostat_pool_tib <- readRDS(file) %>%
  filter(drug == "Vorinostat") %>%
  dplyr::select(drug, "pool_ratio_slope" = ratio_slope_plot, feature)

vorinostat_clone_tib <- CCD_tib %>%
  filter(drug_conc == "Vorinostat 1 µM") %>%
  dplyr::select("drug" = drug_conc, "clone_ratio_slope" = ratio_slope_plot, feature) %>%
  mutate(drug = "Vorinostat")

vorinostat_tib <- merge(vorinostat_clone_tib, vorinostat_pool_tib, by = c("drug", "feature"), all = T) %>%
  mutate(pool_ratio_slope = ifelse(is.na(pool_ratio_slope), 0, pool_ratio_slope)) %>%
  mutate(clone_ratio_slope = ifelse(is.na(clone_ratio_slope), 0, clone_ratio_slope)) 

vorinostat_tib <- vorinostat_tib %>% 
           dplyr::select(-drug) %>%
           column_to_rownames("feature") %>%
           t()

pheatmap(vorinostat_tib[, TRIP_chrom_clust$labels],
         cluster_rows = F,
         cluster_cols = TRIP_chrom_clust,
         border_color = F,
         cellwidth = 8,
...
## S5F Aurora Kinase effects (discussion)

```{r S5F Aurora Kinase}
AurKi = read.table("files_scripts/rs20220711_AurK_inhibitors.txt", header = T, sep = "\t") %>% mutate(group = case_when(AurB == 0 & AurA == 1 ~ "AurK-A", AurB == 1 & AurA == 0 ~ "AurK-B", T ~ "PanAurK"))


data2 = indel.data %>% 
  filter(target == "Aurora Kinase", viab_reproducibility &
           indel_freq_filter &  pathway_bal_filter) %>% 
  mutate(drug = gsub(" [(].*[)]", "", drug),
         drug_conc = paste(drug, conc_char, sep = " "))  %>% 
  group_by(drug_conc) %>%
  mutate(MMEJ_log2_FC_mean = ave(MMEJ_log2_FC, FUN = function(x) mean(x, na.rm = TRUE)),
         freqCut_diff_mean = ave(freqCut_diff_mean, FUN = function(x) mean(x, na.rm = TRUE))) %>% 
  distinct(drug, conc_char, drug_conc, freqCut_diff_mean, MMEJ_log2_FC_mean) %>% 
  left_join(AurKi) %>%
  ungroup()

data2 %>% 
  filter(conc_char == "100 nM") %>% 
  ggplot(., aes(freqCut_diff_mean, MMEJ_log2_FC_mean, color = group, label = drug)) + 
  geom_point(size = 2) + 
  theme_bw() + 
  geom_hline(yintercept = 0, linetype = 2) + 
  geom_vline(xintercept = 0, linetype = 2) + 
  ...
  xlab("ΔTIF") + 
  ylab("MMEJ:NHEJ ratio FC (log2)") + 
  ggtitle("100 nM") +
  geom_label_repel()

data2 %>% 
  filter(conc_char == "1 µM") %>% 
  ggplot(., aes(freqCut_diff_mean, MMEJ_log2_FC_mean, color = group, label = drug)) + 
  geom_point(size = 2) + 
  theme_bw() + 
  geom_hline(yintercept = 0, linetype = 2) + 
