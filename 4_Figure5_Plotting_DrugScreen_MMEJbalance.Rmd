...
```{r 5A MMEJ FC heatmap, warning = FALSE, fig.width = 10, fig.height = 12}
...
```{r 5B Top TIF boosting, fig.width = 4, fig.height = 4}
rank_dt = indel.data %>% 
...
```{r 5E Dec NU7441 combination TIDE, fig.width = 6, fig.height = 6}
decitabine_nu7441_all <- read_csv("/DATA/usr/m.trauernicht/projects/EpiScreen/data/decitabine_synergy_indels.csv") %>%
  ...
  mutate(pathway = as.factor(case_when(mutation == -7 ~ "MMEJ",
                                       mutation == 1 ~ "NHEJ",
                                       mutation == 2 ~ "SSTR", 
                                       T ~ "TIF")),
         pathway = factor(pathway, levels = c("TIF", "NHEJ", "MMEJ", "SSTR")),
...
decitabine_nu7441_df = decitabine_nu7441_all %>%
  mutate(percentage_mean = ave(percentage, pathway, condition, ssODN, FUN = mean),
         percentage_sd = ave(percentage, pathway, condition, ssODN, FUN = sd)) %>%
  distinct(pathway, condition, percentage_mean, percentage_sd, ssODN)
...
decitabine_nu7441_all %>%
  filter(ssODN) %>%
  ggboxplot(., x = "condition",
            y = "percentage",
            color = "condition", 
            add = "jitter", 
            palette = colors_DecNu, 
            facet.by = "pathway") +
...
```{r S5A MMEJ boosting FC heatmap, warning = FALSE, fig.width = 10, fig.height = 10}
# pdf(paste0(figure_out, "rs20220623_indelfrequency_heatmap_allconc.pdf"), width=20, height=8)
balance.data.matrix = balance.data.heatmap2.clusters %>%
  arrange(target_all) %>%
  column_to_rownames('drug_conc') %>%
...
all_compounds_epistasis_TIF = CCD_tib %>%
  ...
  mutate(onlyNA = sum(is.na(ratio_slope_plot)),
         drug_conc = gsub("control", "1 µM", drug_conc, perl = TRUE)) %>% 
  filter(onlyNA != 25) %>%
...
```{r S5E Vorino TIF FC Correlations, fig.width = 6, fig.height = 8}
## Vorinostat
