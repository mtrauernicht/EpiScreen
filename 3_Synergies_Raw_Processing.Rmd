...
# How many samples have a significant effect on the TIF and the balance per concentration?
filtered_drug_conc %>% group_by(conc_char) %>% summarise(TIF = sum(effect_on_indelrate), Bal = sum(effect_on_ratio))
# And overall? (remove the control samples)
filtered_drug_conc %>% filter(conc_char != "control") %>% summarise(TIF = sum(effect_on_indelrate), Bal = sum(effect_on_ratio))
...
ratios_tib_fc <- ratios_tib %>% 
  ...
  distinct(IPR, drug, conc_char, target, log2.fc.ratio, log2.fc.editratio) %>%
...
epistasis_tib %<>% 
  ...
  distinct(# These are p values for a significant CCD by PCR
    signif.pcr.ratio, signif.pcr.editratio,
    drug_conc, feature, 
    ratio_slope, editratio_slope, # The slopes
  ...
  mutate(
    ...
    editratio_slope_plot = case_when(
      ...
      effect_on_indelrate & signif.pcr.editratio & TIF_signif == "TIF_both" ~ 
        editratio_slope,
      T ~ as.double(NA))) %>%
  distinct(drug_conc, feature,
           editratio_slope, editratio_slope_plot, 
           ratio_slope, ratio_slope_plot)
