...
filtered_drug_pvals = indel.data %>%
  mutate(drug = case_when(conc_char == "control" & plate %in% c(1:6) ~ 
                            paste(drug, drug_plate, well, "1", sep = " "), 
                          conc_char == "control" & plate %in% c(7:12) ~ 
                            paste(drug, drug_plate, well, "2", sep = " "),
                          conc_char == "control" & plate %in% c(13:18) ~ 
...
effect_of_compound_tib = filtered_drug_pvals %>%
  ...
  dplyr::summarise(#Summarise number of significant barcodes per compound for balance
    ...
    #Annotate which pathway the compound boosts
    TIF_up_signif = sum(zscore_TIF_comb_rep_temp > 1.96)) %>% 
  mutate(bal_signif = case_when(NHEJ_signif >= 1 & MMEJ_signif >= 1 ~ "both", 
         ...
         TIF_signif = case_when(TIF_down_signif >= 1 & TIF_up_signif >= 1 ~ "TIF_both",
                                ...
                                T ~ "no_effect")) %>%
  mutate(drug_conc = paste(drug, conc_char, sep = " ")) %>%
  # Semi-join the filtered drug conc to keep only the significant ones after p adjustment.
...
ratios_tib <- indel.data %>%
  ...
  dplyr::select(-MMEJratio, -MMEJratio_min) %>%
  distinct()
...
ratios_tib_fc <- ratios_tib %>% 
  mutate(log2.fc.ratio = MMEJratio.log2 - DMSO.ratio.log2, 
         ...
         log2.fc.editratio = ave(log2.fc.editratio, IPR, drug_conc, FUN = function(x) mean(x, na.rm = TRUE)), 
         ...
         diff.freqCut = freqCut - DMSO.freqCut,
         diff.freqCut = ave(diff.freqCut, IPR, drug_conc, FUN = function(x) mean(x, na.rm = TRUE))) %>%
  distinct(IPR, drug, conc_char, target, log2.fc.freqCut, log2.fc.ratio, log2.fc.editratio, diff.freqCut) %>%
...
slope.protein.features <- tibble(drug_conc = NA, feature = NA,  term = NA, 
                                 ...
                                 indelratediff = NA, p.value.indelratediff = NA, 
...
for (i in unique(ratios_tib_fc$drug_conc)) {
  for (j in chromatin.features.short) {
    ...
    # Indel rates raw
    model.indelrate <- lm(formula = diff.freqCut ~ unlist(model_tib[j]), 
                          data = model_tib) %>% 
      tidy()
    ...
    slope.protein.features <- slope.protein.features %>% 
      add_row(drug_conc = i, 
              ...
              p.value.indelrate = model.indelrate.log2 %>% 
                pull(p.value),
              # TIF diff
              indelratediff = model.indelrate %>% 
                pull(estimate), 
              p.value.indelratediff = model.indelrate %>% 
                pull(p.value),
              # TIF ratio
...
```{r}
CCD_tib = ratios_tib_fc %>% 
  dplyr::select(IPR, drug_conc, log2.fc.freqCut, diff.freqCut, 
                log2.fc.ratio, log2.fc.editratio, 
                all_of(chromatin.features.short)) %>% 
  ungroup()

#Create an empty dt with CCDs of DDR proteins
drug_conc_CCDs_dt <- tibble(var = NA, drug_conc = NA, num_comp = NA, 
                            r.squared = NA, adj.r.squared = NA,p.value = NA)
# chrom_formula = as.formula(paste("y ~ ", paste(chromatin.features, collapse= "+")))

for (i in unique(CCD_tib$drug_conc)){
  for (j in c("log2.fc.freqCut", "diff.freqCut", "log2.fc.editratio", "log2.fc.ratio")) {
...
signif_pcr_hits = adj_p.value_KO_model %>% filter(p.adj < pval_signif) %>% 
  ...
  mutate(val = TRUE,
         ...
         var = gsub("diff.freqCut", "signif.pcr.freqCutdiff", var)) %>%
...
TIFdiff_CCDs = adj_p.value_KO_model %>% 
  filter(var == "diff.freqCut" & 
           p.adj < pval_signif &
           !grepl("DNA-PK|DMSO|Mirin", drug_conc)) %>%
  nrow()

TIFratio_CCDs = adj_p.value_KO_model %>% 
...
paste("Of the 160 drugs,", TIFratio_CCDs,"had a significant TIF for the balance and", bal_CCDs,"had a significant CCD for the balance")
...
epistasis_tib = slope.protein.features %>% 
  distinct(drug_conc, term, feature, indelrate, indelratediff, editratio, ratio) %>% 
  pivot_wider(., names_from = term, values_from = c(indelrate, indelratediff,
                                                    editratio, ratio)) %>%
...
epistasis_tib %<>% 
  ...
  distinct(# These are p values for a significant CCD by PCR
    signif.pcr.ratio, signif.pcr.editratio, signif.pcr.freqCut, signif.pcr.freqCutdiff, 
    drug_conc, feature, 
    indelrate_slope, indelratediff_slope, ratio_slope, editratio_slope, # The slopes
  ...
  mutate(
    ratio_slope_plot = case_when(
      effect_on_ratio & signif.pcr.ratio & bal_signif == "MMEJ" & ratio_slope > 0 ~ 
        ratio_slope,
      bal_signif == "MMEJ" & ratio_slope < 0 ~ 
        as.double(NA), 
      effect_on_ratio & signif.pcr.ratio & bal_signif == "NHEJ" & ratio_slope < 0 ~ 
        ratio_slope,
      bal_signif == "NHEJ" & ratio_slope > 0 ~ as.double(NA), 
      effect_on_ratio & signif.pcr.ratio & bal_signif == "bal_both" ~ 
        ratio_slope,
      T ~ as.double(NA)),
    editratio_slope_plot = case_when(
      effect_on_indelrate & signif.pcr.editratio & TIF_signif == "TIF_down" & editratio_slope < 0 ~
        editratio_slope, 
      TIF_signif == "TIF_down" & editratio_slope < 0 ~ as.double(NA), 
      effect_on_indelrate & signif.pcr.editratio & TIF_signif == "TIF_up" & editratio_slope > 0 ~ 
        editratio_slope, 
      TIF_signif == "TIF_up" & editratio_slope > 0 ~ as.double(NA), 
      effect_on_indelrate & signif.pcr.editratio & TIF_signif == "TIF_both" ~ 
        editratio_slope,
      T ~ as.double(NA)),
    indelrate_slope_plot = case_when(
      effect_on_indelrate & signif.pcr.freqCut & TIF_signif == "TIF_down" & 
        indelrate_slope < 0 ~ indelrate_slope, 
      TIF_signif == "TIF_down" & indelrate_slope > 0 ~ as.double(NA), 
      effect_on_indelrate & signif.pcr.freqCut & TIF_signif == "TIF_up" & 
        indelrate_slope > 0 ~ indelrate_slope, 
      ...
      T ~ as.double(NA)),
    indelratediff_slope_plot = case_when(
      effect_on_indelrate & signif.pcr.freqCutdiff & 
        TIF_signif == "TIF_down" & indelratediff_slope < 0 ~ indelratediff_slope, 
      TIF_signif == "TIF_down" & indelratediff_slope > 0 ~ as.double(NA), 
      effect_on_indelrate & signif.pcr.freqCutdiff & 
        TIF_signif == "TIF_up" & indelratediff_slope > 0 ~ indelratediff_slope, 
      TIF_signif == "TIF_up" & indelratediff_slope < 0 ~ as.double(NA), 
      effect_on_indelrate & signif.pcr.freqCutdiff & TIF_signif == "TIF_both" ~ 
        indelratediff_slope,
      T ~ as.double(NA))) %>%
  distinct(drug_conc, feature, indelrate_slope, indelrate_slope_plot, 
           indelratediff_slope, indelratediff_slope_plot, 
           editratio_slope, editratio_slope_plot, ratio_slope, ratio_slope_plot)
...
TIFdiff_CCDs = epistasis_tib %>% 
  filter(!grepl("DNA-PK|control", drug_conc)) %>% 
  filter(!is.na(indelratediff_slope_plot)) %>%
  distinct(drug_conc) %>%
  nrow()
...
paste("Of the 480 drug:concentration combinations,", TIF_CCDs,"had a significant CCD for the TIF, ", TIFdiff_CCDs, "for TIF diff and", bal_CCDs,"had a significant CCD for the balance")
...
filename <- SetFileName("CCD_table_raw", initials = initials, extension = "RDS")
