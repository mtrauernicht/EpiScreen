...
mean_sd_fits <- tibble(term = NA, estimate = NA, std.error = NA, IPR = NA, rep = NA)

# First prepare a table with TIF in DMSO, well, IPR, replicate, tech and plate. Then group by tech and replicate.
dmso.TIF.score <- indel.data %>%
  filter(drug == "DMSO") %>%
  dplyr::select('DMSO' = freqCut, IPR, replicate, tech) %>%
  mutate(rep = paste(replicate, tech, sep = " ")) %>%
  distinct(IPR, replicate, tech, DMSO, rep)

#Function to fit normal distribution through data
for (i in unique(dmso.TIF.score$rep)) {
  for (x in unique(dmso.TIF.score$IPR)) {
    single_wt_ratio <- filter(dmso.TIF.score, rep == i & IPR == x) %>% pull(DMSO)
    mean_sd_fits_row <- fitdistr(single_wt_ratio, "normal") %>% tidy()
    mean_sd_fits <- mean_sd_fits %>% add_row(mean_sd_fits_row %>% mutate(IPR = x, rep = i))
  }
}
...
dmso.TIF.score <- mean_sd_fits %>% filter(!is.na(term)) %>%
  ...
  dplyr::select(IPR, replicate, tech, mean_DMSO = mean, sd_DMSO = sd)
```
...
TIF_zscore_calc = indel.data %>% 
  ...
  mutate(drug = ifelse(drug == "DMSO" & plate %in% c(1,2,7,8,13,14), paste(drug, drug_plate, well, "1", sep = " "), 
                       ifelse(drug == "DMSO" & plate %in% c(3,4,9,10,15,16), paste(drug, drug_plate, well, "2", sep = " "),
                              ifelse(drug == "DMSO" & plate %in% c(5,6,11,12,17,18), paste(drug, drug_plate, well, "3", sep = " "),drug)))) %>%
  distinct(IPR, replicate, tech, drug, freqCut, conc_char) %>% 
  left_join(dmso.TIF.score)
```

### Calculate the TIF z-scores per IPR
...
mean.TIF.zscore.comb = TIF.zscore %>% distinct(drug, conc_char, IPR, replicate, zscore_TIF) %>%
  mutate(ID = paste(drug, conc_char, IPR, sep = " ")) %>%
...
TIF.zscore %<>%
  dplyr::select(IPR, replicate, tech, drug, conc_char, 
                zscore_TIF, 
...
# First prepare a table with MMEJ ratio in DMSO, well, IPR, replicate, tech and plate. Then group by tech and replicate.
dmso.bal.score <- indel.data %>%
  filter(drug == "DMSO") %>%
  dplyr::select('DMSO' = MMEJratio, IPR, replicate, tech) %>%
  mutate(rep = paste(replicate, tech, sep = " ")) %>%
  distinct(IPR, replicate, tech, DMSO, rep)

#Function to fit normal distribution through data
for (i in unique(dmso.bal.score$rep)) {
  for (x in unique(dmso.bal.score$IPR)) {
    single_wt_ratio <- filter(dmso.bal.score, rep == i & IPR == x) %>% pull(DMSO)
    mean_sd_fits_row <- fitdistr(single_wt_ratio, "normal") %>% tidy()
    mean_sd_fits <- mean_sd_fits %>% add_row(mean_sd_fits_row %>% mutate(IPR = x, rep = i))
  }
}
...
bal_zscore_calc = indel.data %>%
   mutate(drug = ifelse(drug == "DMSO", paste(drug, plate, well, sep = " "), drug)) %>%
  filter(!is.na(MMEJratio)) %>%
  distinct(IPR, replicate, tech, drug, MMEJratio, conc_char) %>% 
  left_join(dmso.bal.score)
```


### Calculate the balance z-scores per IPR
...
# Combine the z-scores of both screens
mean.bal.zscore.comb = bal.zscore %>% distinct(drug, conc_char, IPR, replicate, zscore_bal) %>%
  mutate(ID = paste(drug, conc_char, IPR, sep = " ")) %>%
...
bal.zscore %<>% left_join(mean.bal.zscore.comb) %>% 
  dplyr::select(IPR, replicate, tech, drug, conc_char, MMEJ_FC, MMEJ_log2_FC,
                zscore_bal, 
...
cols_to_replace_1 = TIF.zscore %>% dplyr::select(-IPR, -replicate, -tech, -drug, -conc_char) %>% colnames()
cols_to_replace_2 = bal.zscore %>% dplyr::select(-IPR, -replicate, -tech, -drug, -conc_char) %>% colnames()
cols_remove = c(cols_to_replace_1, cols_to_replace_2)
indel.data2 <- indel.data %>% 
  # distinct(MMEJratio, freqCut, drug, conc_char, target, IPR, drug_plate, well, tech, replicate) %>%
  mutate(drug = ifelse(drug == "DMSO" & plate %in% c(1,2,7,8,13,14), paste(drug, drug_plate, well, "1", sep = " "), 
                       ifelse(drug == "DMSO" & plate %in% c(3,4,9,10,15,16), paste(drug, drug_plate, well, "2", sep = " "),
                              ifelse(drug == "DMSO" & plate %in% c(5,6,11,12,17,18), paste(drug, drug_plate, well, "3", sep = " "),drug)))) %>%
...
filtered_drug_conc = indel.data2 %>%
