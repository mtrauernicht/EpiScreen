...
indel.data <- get(load("mt20190129_indel.data"))
findhits.data <- indel.data

outlier.df <- get(load("mt20190129_outlier.df"))

mapping.data <- get(load("rs20190116_RSTP2.2000.Chromatin.RData"))

clone.list <- read.delim("mt20181121_barcodes_vs_clones.txt")
```
...
findhits.data[grep("AGAAAATAATATGACG|AGGGCGTAAAATATTT|ATACTATATTTAACGG|GCTAACATCACGAATC", findhits.data$barcode),]$chr.state <- "open"
...
outlier.df[grep("AGAAAATAATATGACG|AGGGCGTAAAATATTT|ATACTATATTTAACGG|GCTAACATCACGAATC", outlier.df$barcode),]$chr.state <- "open"
...
open.df <- open.df[!duplicated(open.df$logratio.mean),]
permissive.df <- outlier.df[grep("permissive",outlier.df$chr.state),]
permissive.df <- permissive.df[!duplicated(permissive.df$logratio.mean),]
closed.df <- outlier.df[grep("closed",outlier.df$chr.state),]
closed.df <- closed.df[!duplicated(closed.df$logratio.mean),]
...
outlier.open.df <- open.df[-grep(paste(x1, collapse = "|"), open.df$Drug),]
outlier.open.df <- outlier.open.df[-grep(paste(x2, collapse = "|"), outlier.open.df$Drug),]
outlier.permissive.df <- permissive.df[-grep(paste(x2, collapse = "|"), permissive.df$Drug),]
outlier.permissive.df <- outlier.permissive.df[-grep(paste(x3, collapse = "|"), outlier.permissive.df$Drug),]
outlier.closed.df <- closed.df[-grep(paste(x1, collapse = "|"), closed.df$Drug),]
outlier.closed.df <- outlier.closed.df[-grep(paste(x3, collapse = "|"), outlier.closed.df$Drug),]
...
mapping.data <- mapping.data[,c(1,77:99,101)]

# Remove duplicate rows
mapping.data <- mapping.data[!duplicated(mapping.data), ]

# Filter out clone 5 barcodes
clone.list <- clone.list[grep("clone 5", clone.list$Clone),]
mapping.data <- merge(clone.list, mapping.data, by.x = "Barcode", by.y = "barcode")
```
