...
mapping_mean_long <- get(load("mt20190214_mapping_mean_long"))
```
...
annotate.bc2 <-
  mapping_mean_long[grep("H3K27me3",mapping_mean_long$variable),]

annotate.bc <- dcast(annotate.bc, barcode ~ variable, value.var = "value")
annotate.bc2 <- dcast(annotate.bc2, barcode ~ variable, value.var = "mean")
annotate.bc[is.na(annotate.bc)] <- 0
setnames(annotate.bc2, old = "H3K27me3", new = "H3K27me3_mean")
annotate.bc <- merge(annotate.bc, annotate.bc2)
...
annotation_colors = list(
  ...
  H3K27me3_mean = brewer.pal(9, "PuBu"),
...
pheatmap(as.matrix(indel.data.med.target), annotation_col = annotate.bc, fontsize_row = 6,
         color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdGy")))(length(breaksList)), 
         breaks = breaksList, annotation_legend = F,cellheight = 6,
...
pheatmap(as.matrix(indel.data.low.target), annotation_col = annotate.bc, fontsize_row = 6,
         color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdGy")))(length(breaksList)), 
         breaks = breaksList, annotation_legend = F,cellheight = 6,
         annotation_colors = annotation_colors, main = "log2(+1/-7 indel ratio) - 
         100nM of drugs added")
...
pheatmap(as.matrix(indel.data.high.drug), fontsize_col = 5, fontsize_row = 5, annotation_col = target, cellheight = 8,
...
for (i in unique(data2$Target)) {
  ...
  p <- pheatmap(as.matrix(data[,1:18]), annotation_col = annotate.bc, 
         color = colorRampPalette(rev(brewer.pal(n = 7, name = "RdGy")))(100), annotation_legend = F,
