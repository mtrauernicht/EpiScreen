...
colors = diverge_hcl(length(breaks), palette = "Cork")
...
hdac2_3 <- TIF_synergy_cluster %>%
  ...
  left_join(target.HDAC %>% rownames_to_column("drug_conc")) %>%
...
for(i in unique(CCD_tib$feature)) {
  rank_dt = CCD_tib %>% 
    ...
    filter(feature == i, !is.na(indelrate_slope)) %>%
    distinct() %>%
    arrange(desc(indelrate_slope))
  ...
  if(nrow(rank_dt) > 1) {
    ...
    p1 = rank_dt %>% 
      ...
      theme(legend.position = "none",
            ...
            axis.ticks.x=element_blank())+
      # add horizontal line for 0.01 slope cutoff which is highlighted in the heatmap
      geom_vline(xintercept = rank_dt[rank_dt$indelrate_slope  < -0.01, ]$rank[1] - 0.5)  +
      geom_vline(xintercept = max(rank_dt[rank_dt$indelrate_slope  > 0.01, ]$rank) + 0.5)
    
    
    p2 = rank_dt %>% 
      filter(!between(rank, 16, maxrank-15)) %>%
      mutate(rank = seq(1:n())) %>% 
      ggplot(., aes(x = rank, y = indelrate_slope, fill = target)) +
      geom_bar(stat = "identity") + 
      scale_x_reverse() +
      coord_flip() +
      ylab("Synergy score") +
...
## 4C m5C top CCDs
```{r 4C m5C synergies, fig.width = 8, fig.height = 6}
rank_dt = CCD_tib %>% 
  left_join(rownames_to_column(target, var = "drug_conc")) %>%
...
# # change these numbers according to where you want to set the cutoff
quant_breaks <- quantile(TIF_synergy_matrix_DNMT, c(.20,.80), na.rm = T)
max_quant = max(abs(quant_breaks))
palette_length <- 100
breaks <- c(seq(-max_quant, 0, length.out=ceiling(palette_length/2) + 1),
            seq(max_quant/palette_length, max_quant, length.out=floor(palette_length/2)))
...
p = pheatmap(TIF_synergy_matrix_DNMT[, TRIP_chrom_clust$labels],
             ...
             cluster_cols = TRIP_chrom_clust,
             clustering_distance_rows = "correlation",
             main = paste("Chromatin synergies - TIF DNMT cluster"),
             color = colors,
             annotation_colors = annotation_colors_scr, 
             annotation_legend = T)
```
...
ratios_tib_fc %>% 
  filter(drug_conc %in% c("Vorinostat 1 µM", "EPZ-6438 10 µM", "GSK J4 HCl 10 µM"))  %>% 
