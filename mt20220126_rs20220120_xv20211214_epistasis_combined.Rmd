...
indel.data <- readRDS("/DATA/projects/DSBrepair/data/R/rs20220216_episcreen/rs20220216_episcreen_ratios.RDS")
...
ratios_tib_dist %<>% mutate(drug_conc = paste(drug, conc_char, sep = "_"))
...
slope.ratio.features.list <- slope.features.tib %>% 
  ...
  select(-count_feature_ratio)  %>% 
  group_by(conc) %>% 
  group_split(., .keep = FALSE) %>% 
  set_names(unlist(group_keys(slope.features.tib.filt)))   
...
# clone5_map = hclust(dist(t(all_compounds_epistasis)))
...
# giveNAs = which(is.na(as.matrix(dist(all_compounds_epistasis))),arr.ind=TRUE)
# unique(giveNAs)
# 
# tab = sort(table(c(giveNAs)),decreasing=TRUE)
# checkNA = sapply(1:length(tab),function(i){
# sum(is.na(as.matrix(dist(all_compounds_epistasis[-as.numeric(names(tab[1:i])),]))))
# })
# rmv = names(tab)[1:min(which(checkNA==0))]

quant_breaks <- quantile(all_compounds_epistasis, c(.01,.99), na.rm = T) # change these numbers according to where you want to set the cutoff
palette_length <- 100
breaks <- c(seq(quant_breaks[1], 0, length.out=ceiling(palette_length/2) + 1), 
              seq(quant_breaks[2]/palette_length, quant_breaks[2], length.out=floor(palette_length/2)))
colors = colorRampPalette(slope.colors %>% pull(color))(length(breaks))
...
pdf(paste0(figure_out, "rs20220208_epistasis_pathwayratio_all.pdf"), width=10, height=20)

pheatmap(all_compounds_epistasis[, clone5_map$labels],
         border_color = F,
         cellwidth = 8,
         cellheight = 8,
...
ritios_tib_epi_effects = indel.data %>% 
  mutate(drug_conc = paste(drug, conc_char, sep = " ")) %>% 
  filter(drug_conc %in% rownames(all_compounds_epistasis) & 
         drug != "DNA-PKi") %>%
...
clone5_map_ipr = hclust(dist(clone5.matrix))
...
pdf(paste0(figure_out, "rs20220208_zscorepathwayratio_all.pdf"), width=10, height=20)
...
pheatmap(mmejzscore_matrix[, clone5_map_ipr$labels],
         ...
         breaks = breaks,
         annotation_row = target,
         main = "MMEJ z-scores of the epistasis hits",
         color = colors,
         annotation_colors = annotation_colors_scr, annotation_legend = T)
...
pdf(paste0(figure_out, "rs20220208_epistasis_indelrate_all.pdf"), width=10, height=20)
pheatmap(all_compounds_epistasis[, clone5_map$labels],
         border_color = F,
         cellwidth = 8,
         cellheight = 8,
...
ritios_tib_epi_effects = indel.data %>% 
  ...
  filter(drug_conc %in% rownames(all_compounds_epistasis), 
         drug != "DNA-PKi") %>%
  distinct(IPR, mean_eff_zscore_comb, drug_conc, drug, conc_char) 

mmejzscore_matrix = ritios_tib_epi_effects %>%
  distinct(drug_conc, IPR, mean_eff_zscore_comb) %>% 
...
pdf(paste0(figure_out, "rs20220208_zscoreindelrate_all.pdf"), width=10, height=20)
pheatmap(mmejzscore_matrix[, clone5_map_ipr$labels],
         border_color = F,
         cellwidth = 8,
         cellheight = 8,
         ...
         breaks = breaks,
         annotation_row = target,
         main = "indel rate z-scores of the epistasis hits",
         color = colors,
         annotation_colors = annotation_colors_scr, annotation_legend = T,
...
pdf(paste0(figure_out, "rs20220210_synergy_indelrate_all.pdf"), width=10, height=20)
pheatmap(all_compounds_indelrate_synergy[, clone5_map$labels],
         border_color = F,
         cellwidth = 8,
         cellheight = 8,
         breaks = breaks,
...
```{r all concentrations indel rate z-score}
ritios_tib_epi_effects = screen_ratios %>% 
  mutate(drug_conc = paste(drug, conc_char, sep = " ")) %>% 
  filter(drug_conc %in% rownames(all_compounds_indelrate_synergy), 
         drug != "DNA-PKi") %>%
  distinct(IPR, mean_eff_zscore_comb, drug_conc, drug, conc_char) 
...
quant_breaks <- quantile(mmejzscore_matrix, c(.01,.99), na.rm = T) # change these numbers according to where you want to set the cutoff
palette_length <- 1000
breaks <- breaks <- c(seq(quant_breaks[1], 
                          0, 
                          length.out=ceiling(palette_length/2) + 1), 
                      seq(quant_breaks[2]/palette_length,  
                          quant_breaks[2], 
...
pdf(paste0(figure_out, "rs20220210_zscoreindelrate_all.pdf"), width=10, height=20)
pheatmap(mmejzscore_matrix[, clone5_map_ipr$labels],
         border_color = F,
         cellwidth = 8,
         cellheight = 8,
         ...
         breaks = breaks,
         annotation_row = target,
         main = "indel rate z-scores of the synergy hits",
         color = colors,
         annotation_colors = annotation_colors_scr, annotation_legend = T,
         annotation_col = domains_c5,
         cutree_rows = 8)
