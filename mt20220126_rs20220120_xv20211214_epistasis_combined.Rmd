  ...
  mutate(x_intercept_indelrate = ifelse(indelrate_effect == "mixed" & p.value.indelrate < 0.05, 
         ...
         orientation_indelrate = ifelse(indelrate_slope < 0 & indelrate_effect == "indel_rate_decrease" | 
                                        ...
                                        ifelse(indelrate_slope < 0 & indelrate_effect == "indel_rate_increase" | 
                                         ...
                                         "complex")),
         indelrate_slope_plot = ifelse(orientation_indelrate == "divergent" | orientation_indelrate == "complex" & x_intercept_indelrate > 1, indelrate_slope, NA),
         orientation_ratio = ifelse(ratio_slope < 0 & ratio_effect == "NHEJ_increase" | 
                                        ...
                                        ifelse(ratio_slope < 0 & ratio_effect == "MMEJ_increase" | 
                                         ...
                                         "complex")),
         ratio_slope_plot = ifelse(orientation_ratio == "divergent" | orientation_ratio == "complex" & x_intercept_ratio > 1, ratio_slope, NA))  


###WIP###\

# no positive intercept(x)
...
ggplot(ratios_tib %>% 
         filter(drug %in% c("DNA-PKi 10 µM", "Nexturastat A", "SGC-CBP30")),
       aes(H3K27me3.zscore
           ,log2.dist.ratio, color = drug)) + 
  geom_point() +
  stat_cor(method = "spearman") + 
...
```{r filter checking}
hits = epistasis_tib %>% 
  distinct(feature, drug_conc, indelrate_slope_plot) %>% 
  filter(!is.na(indelrate_slope_plot)) %>% 
  mutate(drug_conc_feature = paste(drug_conc, feature, sep = "_")) %>% 
  pull(drug_conc_feature)

ratios_tib %>% 
  ungroup() %>%
  select(barcode, drug, conc_char, target, all_of(chromatin.features), log2.dist.freqCut) %>%
  pivot_longer(cols = all_of(chromatin.features), names_to = "feature", values_to = "zscore") %>%
  mutate(drug_conc_feature = paste(drug, conc_char, feature, sep = "_")) %>%
  filter(drug_conc_feature %in% hits) %>%
  select(-drug_conc_feature) %>%
  ggplot(., aes(zscore
                ,log2.dist.freqCut, color = target)) + 
  geom_point() +
  stat_cor(method = "spearman") + 
  geom_smooth(method = "lm") + 
  theme_bw() +
  facet_wrap(feature ~ conc_char)

hits = epistasis_tib %>% 
  distinct(feature, drug_conc, ratio_slope_plot) %>% 
...
ratios_tib %>% 
  ungroup() %>%
  select(barcode, drug, conc_char, target, all_of(chromatin.features), log2.dist.ratio) %>%
  pivot_longer(cols = all_of(chromatin.features), names_to = "feature", values_to = "zscore") %>%
  mutate(drug_conc_feature = paste(drug, conc_char, feature, sep = "_")) %>%
  filter(drug_conc_feature %in% hits) %>%
  select(-drug_conc_feature) %>%
  ggplot(., aes(zscore
                ,log2.dist.ratio, color = target)) + 
  geom_point() +
  stat_cor(method = "spearman") + 
  geom_smooth(method = "lm") + 
  theme_bw() +
...
slope.features.tib <-   epistasis_tib %>% 
  select(drug_conc, ratio_slope_plot, indelrate_slope_plot, feature) %>%
  group_by(drug_conc) %>%
...
slope.ratio.features.list <- slope.features.tib %>% 
  select(drug, conc, ratio_slope_plot, feature, count_ratio) %>%
...
slope.rate.features.list <- slope.features.tib %>% 
  select(drug, conc, indelrate_slope_plot, feature, count_rate) %>%
  filter(count_rate != 25) %>%
  select(-count_rate) %>%
  group_by(conc) %>%
  group_split(.keep = FALSE) 

slope.ratio.features.scale.dcast.1um = slope.ratio.features.list[[1]] %>%
  reshape2::dcast(drug ~ feature, value.var = "ratio_slope_plot") %>% 
  column_to_rownames(var = "drug")

slope.ratio.features.scale.dcast.10um = slope.ratio.features.list[[2]] %>%
  reshape2::dcast(drug ~ feature, value.var = "ratio_slope_plot") %>% 
  column_to_rownames(var = "drug")


slope.ratio.features.scale.dcast.100nm = slope.ratio.features.list[[3]] %>%
  reshape2::dcast(drug ~ feature, value.var = "ratio_slope_plot") %>% 
  column_to_rownames(var = "drug")

slope.indelrate.features.scale.dcast.1um = slope.rate.features.list[[1]] %>%
  reshape2::dcast(drug ~ feature, value.var = "indelrate_slope_plot") %>% 
  column_to_rownames(var = "drug")


slope.indelrate.features.scale.dcast.10um = slope.rate.features.list[[2]] %>%
  reshape2::dcast(drug ~ feature, value.var = "indelrate_slope_plot") %>% 
  column_to_rownames(var = "drug")


slope.indelrate.features.scale.dcast.100nm = slope.rate.features.list[[3]] %>%
  reshape2::dcast(drug ~ feature, value.var = "indelrate_slope_plot") %>% 
  column_to_rownames(var = "drug")
```
...
max_slope = round(max(c(max(slope.ratio.features.scale.dcast.100nm), abs(min(slope.ratio.features.scale.dcast.100nm)))), 2)
breaks <- seq(-max_slope, max_slope, 0.001)
colors = colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))(length(breaks))
colors[which(breaks == 0)-1] = "gray"
...
max_slope = round(max(c(max(slope.ratio.features.scale.dcast.1um), abs(min(slope.ratio.features.scale.dcast.1um)))), 2)
breaks <- seq(-max_slope, max_slope, 0.001)
colors = colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))(length(breaks))
colors[which(breaks == 0)-1] = "gray"
...
max_slope = round(max(c(max(slope.ratio.features.scale.dcast.10um), abs(min(slope.ratio.features.scale.dcast.10um)))), 2)
breaks <- seq(-max_slope, max_slope, 0.001)
colors = colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))(length(breaks))
colors[which(breaks == 0)-1] = "gray"
...
```{r}
max_slope = round(max(c(max(slope.indelrate.features.scale.dcast.100nm), abs(min(slope.indelrate.features.scale.dcast.100nm)))), 2)
breaks <- seq(-max_slope, max_slope, 0.001)
colors = colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))(length(breaks))
colors[which(breaks == 0)-1] = "gray"
...
max_slope = round(max(c(max(slope.indelrate.features.scale.dcast.1um), abs(min(slope.indelrate.features.scale.dcast.1um)))), 2)
breaks <- seq(-max_slope, max_slope, 0.001)
colors = colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))(length(breaks))
colors[which(breaks == 0)-1] = "gray"
...
max_slope = round(max(c(max(slope.indelrate.features.scale.dcast.10um), abs(min(slope.indelrate.features.scale.dcast.10um)))), 2)
breaks <- seq(-max_slope, max_slope, 0.001)
colors = colorRampPalette(rev(brewer.pal(n = 7, name = "RdBu")))(length(breaks))
colors[which(breaks == 0)-1] = "gray"
