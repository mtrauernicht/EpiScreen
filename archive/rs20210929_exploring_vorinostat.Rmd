...
barcde_all_reps = validations_ratios %>% 
  filter(replicate != "mean" & plasmid == "LBR2" & pool %in% c("A", "B")) %>% 
...
vorinostat_reps_ratios = validations_ratios %>% 
  filter(replicate != "mean" & plasmid == "LBR2", barcode %in% barcde_all_reps & pool %in% c("A", "B")) %>% 
...
```{r}
vorinostat_ratios_signif_subsample = validations_ratios %>% filter(replicate != "mean" & plasmid == "LBR2" & barcode %in% sample(signif_bcs, 10) & pool %in% c("A", "B")) %>% group_by(barcode)
vorinostat_ratios_signif = validations_ratios %>% filter(replicate != "mean" & plasmid == "LBR2" & barcode %in% signif_bcs & pool %in% c("A", "B")) %>% group_by(barcode)
