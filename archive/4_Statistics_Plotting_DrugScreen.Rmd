---
title: "Plotting main results of the drug screen"
...
# Drug screen DSB repair - main results
...
indel.data <- RSTP2_2000_indels.df %>%
  select(Number, Drug, Target, `IC50..µM.`, `.id`, V2, X.7, X0, X1, rep, 
         plate, conc, ratio, read.count, read.count1.7, efficiency, MMEJ_score)
indel.data.all <- RSTP2_2000_indels.df %>% 
  select(Number, Drug, Target, `IC50..µM.`, `.id`, V2, X.7, X.6, X.5, X.4,
...
viability.comp2 <- merge(viability.comp, indel.data) %>% 
  setnames(old = c("V2", "X"), new = c("rn", "viability_rep2")) %>% 
  mutate(plate.id = gsub("(.*plate[1,2]).*", "\\1", .id),
         viability = ave(viability, plate.id, FUN = function(x) x/max(x)),
         viability_rep2 = ave(viability_rep2, plate.id, FUN = function(x) x/max(x)))


viability.comp2$group <- "drug"
viability.comp2$group[viability.comp2$Drug == "DMSO"] <- "DMSO"
viability.comp2$group[viability.comp2$Drug == "PAO"] <- "PAO"

viability.comp2$conc <- factor(viability.comp2$conc, levels = c("100nm", "1um", "10um"))

sp <- ggscatter(viability.comp2, x = "viability", y = "viability_rep2",
...
viability.comp.rep <- merge(viability.comp, indel.data) %>% 
  setnames(old = "X", new = "viability_rep2") %>% 
  mutate(viability = ave(viability, FUN = function(x) x/max(x)),
         viability_rep2 = ave(viability_rep2, FUN = function(x) x/max(x)),
         dif = viability / viability_rep2) %>%
  select(dif, Drug, Target, conc, viability, viability_rep2) %>%
  unique()

viability.comp.rep$changed <- "no"
viability.comp.rep$changed[viability.comp.rep$dif >= 2] <- "yes"
viability.comp.rep$conc <- factor(viability.comp.rep$conc, levels = c("100nm", "1um", "10um"))

ggplot(viability.comp.rep, aes(x = Target, y = log2(dif))) +
  geom_quasirandom() +
  theme_bw() +
...
ggplot(data = all.indel.data[all.indel.data$Drug == c("PAO","DMSO"),], aes(x = read.count, fill = Drug, group = Drug)) + 
  geom_density() + 
...
indel.data <- indel.data[indel.data$barcode != "ACCCCTAAAGGCGCTG" &
                           ...
                           indel.data$barcode != "TGTCCCTTAGTACTTT",]
...
# Add plate id
indel.data$plate.id <- gsub("(.*plate[1,2]).*", "\\1", indel.data$condition)
...
indel.data <- indel.data %>% 
  mutate(logratio = ave(ratio, FUN = function(x) log2(x)), # log2(+1/-7)ratio
         viability = ave(viability, plate.id, FUN = function(x) x/max(x)), # relative viability
...
for (i in unique(indel.data$plate.id)) {
  ...
  dmso.mean <- mean(indel.data$logratio[indel.data$Drug == 
                        "DMSO" & indel.data$plate.id == 
                        i & indel.data$barcode == j])
  indel.data$logratio.platenorm[indel.data$plate.id == i & indel.data$barcode == j] <- 
    indel.data$logratio[indel.data$plate.id == i & indel.data$barcode == j] -
    dmso.mean
  }
}
...
for (i in unique(indel.data$plate.id)) {
  ...
  dmso.eff.mean <- mean(indel.data$efficiency[indel.data$Drug == 
                        "DMSO" & indel.data$plate.id == 
                        i & indel.data$barcode == j])
  indel.data$efficiency.platenorm[indel.data$plate.id == i & indel.data$barcode == j] <-
    indel.data$efficiency[indel.data$plate.id == i & indel.data$barcode == j] -
    dmso.eff.mean
  }
}
...
for (i in unique(indel.data$plate.id)) {
  ...
  dmso.MMEJ_score <- mean(indel.data$MMEJ_score[indel.data$Drug == 
                        "DMSO" & indel.data$plate.id == 
                        i & indel.data$barcode == j])
  indel.data$MMEJ_score_norm[indel.data$plate.id == i & indel.data$barcode == j] <-
    indel.data$MMEJ_score[indel.data$plate.id == i & indel.data$barcode == j] /
    dmso.MMEJ_score
  }
}
...
for (i in unique(indel.data$Drug)) {
  for (j in unique(indel.data$conc)) {
    indel.data$MMEJ_zscore[indel.data$Drug == i & indel.data$conc == j] <-
      (indel.data$MMEJ_score[indel.data$Drug == i & indel.data$conc == j] -
      mean(indel.data$MMEJ_score[indel.data$Drug == "DMSO" & indel.data$conc == j])) / 
      sd(indel.data$MMEJ_score[indel.data$Drug == "DMSO" & indel.data$conc == j])
  }
} 
...
for (i in unique(indel.data$Drug)) {
  for (j in unique(indel.data$conc)) {
    indel.data$efficiency_zscore[indel.data$Drug == i & indel.data$conc == j] <-
      (indel.data$efficiency[indel.data$Drug == i & indel.data$conc == j] -
      mean(indel.data$efficiency[indel.data$Drug == "DMSO" & indel.data$conc == j])) / 
      sd(indel.data$efficiency[indel.data$Drug == "DMSO" & indel.data$conc == j])
  }
} 
...
ggplot(data = indel.data[indel.data$Drug == c("DMSO","DNA-PKi","Mirin"),], 
  ...
  geom_density() + xlab("MMEJ z-score") 
...
indel.data$group <- "drug"
indel.data$group[indel.data$Drug == "DMSO"] <- "DMSO"
indel.data$conc <- factor(indel.data$conc, levels = c("100nm", "1um", "10um"))

ggplot(data = indel.data %>%
         filter(Drug != "DNA-PKi") %>%
         mutate(MMEJ_score_drug = ave(MMEJ_score, condition, FUN = function(x) mean(x))) %>%
         select(MMEJ_score_drug, group, condition, conc) %>%
         unique(), 
       aes(y = MMEJ_score_drug, x = 'x', color = group)) + theme_bw() +
  geom_quasirandom(dodge.width = 0.75) +
...
ggplot(data = indel.data %>%
         filter(Drug != "DNA-PKi") %>%
         mutate(efficiency_drug = ave(efficiency, condition, FUN = function(x) mean(x))) %>%
         select(efficiency_drug, group, condition, conc) %>%
         unique(), 
       aes(y = efficiency_drug, x = 'x', color = group)) + theme_bw() +
  geom_quasirandom(dodge.width = 0.75) +
  geom_boxplot(position = position_dodge(0.75), alpha = 0.4) +
  xlab("") + ylab("Cutting Efficiency") +
  scale_color_manual(values = c("#2a9d8f", "black")) +
  facet_wrap(~conc)
...
indel.data2 <- indel.data
indel.data2$zscore_change <- "<2"
indel.data2$zscore_change[abs(indel.data2$MMEJ_zscore)>2] <- "2-3"
indel.data2$zscore_change[abs(indel.data2$MMEJ_zscore)>3] <- ">3"
...
ggplot(indel.data2, aes(x = mean.viability.drug, y = mean.efficiency_zscore.drug)) +
  geom_point(color = ifelse(indel.data2$mean.efficiency_zscore.drug > 2 | 
                                         indel.data2$mean.efficiency_zscore.drug < -2, "black", "grey")) +
...
indel.data2 <- indel.data %>% 
  select(Drug, conc, mean.MMEJ_zscore.drug, mean.viability.drug) %>% 
  filter(Drug != "DNA-PKi") %>% 
  unique()
indel.data2$conc <- factor(indel.data2$conc, levels = c("100nm", "1um", "10um"))

ggplot(indel.data2, aes(x = mean.MMEJ_zscore.drug, y = mean.viability.drug)) +
  geom_point(color = ifelse(indel.data2$mean.MMEJ_zscore.drug > 2 | 
                                         indel.data2$mean.MMEJ_zscore.drug < -2, "black", "grey")) +
...
indel.data.rep1 <- indel.data2 %>%
  filter(rep == 1) %>%
  select(barcode, Drug, conc, 'rep1' = MMEJ_zscore)
indel.data.rep2 <- indel.data2 %>%
  filter(rep == 2) %>%
  select(barcode, Drug, conc, 'rep2' = MMEJ_zscore)
indel.data.rep3 <- indel.data2 %>%
  filter(rep == 3) %>%
  select(barcode, Drug, conc, 'rep3' = MMEJ_zscore)
...
\newpage
## Visualizaton: Identifying locally vs. globally acting drugs
```{r visualization_global-vs-local, out.width= "80%", fig.align= "center", echo = FALSE, warning = FALSE}
...
ggplot(indel.data %>%
         ...
         mutate(mean_MMEJ_score_int = ave(MMEJ_score_norm, Drug, 
                                           conc, integration, FUN = function(x) mean(x))) %>%
         select(mean_MMEJ_score_int, Drug, integration, conc) %>%
         unique() %>%
         mutate(mean_MMEJ_score = ave(mean_MMEJ_score_int, Drug, conc, FUN = function(x) mean(x)),
                sd_MMEJ_score = ave(mean_MMEJ_score_int, Drug, conc, FUN = function(x) sd(x))) %>%
         select(mean_MMEJ_score, sd_MMEJ_score, Drug, conc) %>%
         unique(),
       aes(x = mean_MMEJ_score, y = sd_MMEJ_score)) +
  geom_point() +
  theme_bw() +
  facet_wrap(~conc)
...
ggplot(indel.data %>%
         filter(Drug != "DNA-PKi", Target != "Negative Control", Target != "MRN") %>%
         mutate(mean_MMEJ_zscore_int = ave(MMEJ_score_norm, Drug, 
                                           conc, integration, FUN = function(x) mean(x))) %>%
         select(mean_MMEJ_zscore_int, Drug, Target, integration, conc) %>%
         unique() %>%
         mutate(mean_MMEJ_zscore = ave(mean_MMEJ_zscore_int, Drug, conc, FUN = function(x) mean(x)),
                sd_MMEJ_zscore = ave(mean_MMEJ_zscore_int, Drug, conc, FUN = function(x) sd(x)),
                sd_percentage = 100* (sd_MMEJ_zscore / mean_MMEJ_zscore)) %>%
         select(sd_percentage, Drug, conc, Target) %>%
         unique(),
...
ggplot(indel.data %>%
         ...
         mutate(mean_MMEJ_score_int = ave(MMEJ_score_norm, Drug, 
                                           conc, integration, FUN = function(x) mean(x))) %>%
         select(mean_MMEJ_score_int, Drug, Target, integration, conc) %>%
         unique() %>%
         mutate(mean_MMEJ_score = ave(mean_MMEJ_score_int, Drug, conc, FUN = function(x) mean(x)),
                sd_MMEJ_score = ave(mean_MMEJ_score_int, Drug, conc, FUN = function(x) sd(x))) %>%
         select(sd_MMEJ_score, Drug, conc, Target) %>%
         unique(),
       aes(x = Target, y = sd_MMEJ_score)) +
  geom_quasirandom() +
...
ggplot(indel.data %>%
         filter(Drug != "DNA-PKi", Target != "Negative Control", Target != "MRN") %>%
         mutate(mean_efficiency_score_int = ave(efficiency.platenorm, Drug, 
                                           conc, integration, FUN = function(x) mean(x))) %>%
         select(mean_efficiency_score_int, Drug, Target, integration, conc) %>%
         unique() %>%
         mutate(mean_efficiency_score = ave(mean_efficiency_score_int, Drug, conc, FUN = function(x) mean(x)),
                sd_efficiency_score = ave(mean_efficiency_score_int, Drug, conc, FUN = function(x) sd(x))) %>%
         select(sd_efficiency_score, Drug, conc, Target) %>%
         unique(),
       aes(x = Target, y = sd_efficiency_score)) +
  geom_quasirandom() +
