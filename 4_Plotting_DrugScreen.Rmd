...
RSTP2_2000_indels.df <- get(load("mt20190222_RSTP2_2000_indels.df"))
...
ggplot(data = all.indel.data[all.indel.data$Drug == c("PAO","DMSO"),], aes(x = read.count, fill = Drug, group = Drug)) + geom_density() + facet_wrap(~rn, nrow = 6)+ labs(title = "read count before cutting (PAO) and after cutting (DMSO)")
...
ggplot(freq, aes(Var1, value, fill = variable)) +
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 1.0, vjust = 0))+ 
  ylab("outlier datapoints") + xlab("barcode")+ labs(title = "relative amount of outlier datapoints per barcode")
