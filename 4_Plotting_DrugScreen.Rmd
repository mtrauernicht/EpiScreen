...
for (i in 1:18) {
  dmso.mean <- mean(indel.data$logratio[indel.data$Drug == "DMSO" & indel.data$plate.nr == i])
  indel.data$logratio.platenorm[indel.data$plate.nr == i] <- indel.data$logratio[indel.data$plate.nr == i] - dmso.mean
}



# t-test
indel.data$barcode <- as.character(indel.data$barcode)
indel.data$classifier <- gsub("_rep[0-9]", "\\1", indel.data$condition)
indel.data$classi.bc <- paste(indel.data$barcode,indel.data$classifier)
indel.data$classifier.bc <- do.call(paste, c(indel.data[c("classifier", "barcode")], sep = "_")) 

ppvalue <- replicate(0, NULL)
for (i in unique(indel.data$classi.bc)){
  x <- indel.data[indel.data$Drug == "DMSO",]
  y <- indel.data[indel.data$classi.bc == i,]
  ppvalue[[i]] <- t.test(x$logratio.platenorm, y$logratio.platenorm)
}

t.list.p.value <- sapply(ppvalue, '[[', "p.value")
t.list.statistic <- sapply(ppvalue, '[[', "statistic")
t.list.conf.int <- sapply(ppvalue, '[', "conf.int")
...
# t outlier df (based on 1-tail 95% confidence interval, df=10)
t.outlier.df <- indel.data[abs(indel.data$t) > 1.8125,]
t.outlier.df <- outlier.df[-grep("DNA-PK", t.outlier.df$Target),]
...
ggplot(data = indel.data) +
  geom_quasirandom(mapping = aes(x = barcode, y = logratio.mean, 
...
ggplot(data = indel.data) +
  geom_quasirandom(mapping = aes(x = barcode, y = logratio.comb, colour = Target)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1.0, vjust = 0))+ 
  ylab("log2(+1/-7 ratio)") + xlab("Barcode") + labs(fill="titel")

# ggplot specifying the drug ratio
ggplot(data = indel.data, aes(x = barcode, y = logratio.comb, colour = conc)) +
  geom_quasirandom() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1.0, vjust = 0)) + 
