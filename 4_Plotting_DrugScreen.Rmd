...
for (i in 1:18) {
  ...
  indel.data$logratio.platenorm[indel.data$plate.nr == i & indel.data$barcode == j] <- 
    ...
    dmso.mean
  print(indel.data$logratio.platenorm)
  }
}
...
for (i in 1:18) {
  ...
  indel.data$efficiency.platenorm[indel.data$plate.nr == i & indel.data$barcode == j] <-
    ...
    dmso.eff.mean
  print(indel.data$efficiency.platenorm)
  }
}
...
for (i in unique(indel.data$Target)) {
  ...
  p <- ggplot(data = data) +
    geom_quasirandom(mapping = aes(x = logratio.mean, y = efficiency, colour = Target, size = chr.state)) +
...
ggplot(data = indel.data, aes(x=mean.viability, y=mean.logratio.target)) +
  geom_point() + geom_smooth(method = lm) + 
  stat_smooth_func(geom="text",method="lm",hjust=0,parse=TRUE)
