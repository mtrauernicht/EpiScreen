...
barcodesum <- lapply(all.norm.mutations.df.list, ReadsPerBarcode)

# I want to cut off the mutations below -60, these are very rare and not really needed
# for the plotting of the data.

# We start by giving the range we want to remove.
###lowerindels <- as.character(c(-120:-30, 4:20))

# Apply the RemoveLargeDeletions function on all the dataframes in the list.
###all.norm.mutations.df.list <- lapply(all.norm.mutations.df.list, RemoveLargeDeletions)

# Let's keep the total mutations list also.
all.norm.mutations.df.list.large <- all.norm.mutations.df.list
all.norm.mutations.df.list <- lapply(all.norm.mutations.df.list, SumLargeIndels, maxshift = 15)
...
sum.list <- lapply(all.norm.mutations.df.list, OverallIndels)
large.sum.list <- lapply(all.norm.mutations.df.list.large, OverallIndels) ##what is the difference

# We can plot all these
###OverallIndelPlot(sum.list)
OverallIndelPlot(all.norm.mutations.df.list)

# To create a PDF of these figures, for the small mutations.
setwd("/DATA/projects/DSBrepair/results/")
filename <- SetFileName("_Overall_IndelSpectra.pdf", "rs")
pdf(filename, width = 11, height = 8)
OverallIndelPlot(all.norm.mutations.df.list)
dev.off()

# Same for large mutations
filename <- SetFileName("_Overall_IndelSpectra_large_mutations.pdf", "rs")
pdf(filename, width = 11, height = 8)
OverallIndelPlot(all.norm.mutations.df.list.large)
dev.off()
```
...
### Plotting the indel spectrum for each barcode in one specifig sample.
...
# Preview of a few barodes in a sample.
names(all.norm.mutations.df.list)
# Name the sample:
plot_sample <- c("indel_sum_AB_LBR2_64h")
####### Function that creates the PDF ######
# Generate file name with date and sample
if (plot_sample %in% names(all.norm.mutations.df.list)) {
  # Date is called at the beginning of the script.
  filename <- paste("rs", Date, "Overall_IndelSpectra" , plot_sample, ".pdf", sep = "_")
  ...
  SampleIndelSpectra(j=all.norm.mutations.df.list[[plot_sample]], name=plot_sample, barcodes.no = 200)
...
## Plotting the indel sprectra for one integration in all the samples.
Here we plot one barcode in all the samples that it was found.
...
# Preview :
##BarcodeIndelSpectrum("TATGGCTGTCGGGTAG.B")
#BarcodeIndelSpectrum(indellist = all.norm.mutations.df.list, i="CCGGGGACGTATGCAC.B")

# To make a PDF of these plots
filename <- SetFileName("CCCACCCAGCCCACGT_A_LBR2.pdf", "rs")
pdf(filename, width = 11, height = 8)
#BarcodeIndelSpectrum("TATGGCTGTCGGGTAG.B")
BarcodeIndelSpectrum(indellist = all.norm.mutations.df.list, i="CCCACCCAGCCCACGT.A")
dev.off()
```
...
# Group experiments
```{r sample names}
samples_names <- unique(names(all.norm.mutations.df.list))
LBR2_samples <- grep("LBR2_-|LBR2_DMSO$|LBR2_ODNOT_DMSO|LBR2_t64$|LBR2_DMSO_t64$|LBR2_siNT", samples_names, value=TRUE)

LBR2.A_samples <- grep("LBR2_-|LBR2_DMSO$|LBR2_ODNOT_DMSO|LBR2_t64$|LBR2_DMSO_t64$|LBR2_siNT", grep("_A_", samples_names, value=TRUE), value=TRUE)
LBR2.B_samples <- grep("LBR2_-|LBR2_DMSO$|LBR2_ODNOT_DMSO|LBR2_t64$|LBR2_DMSO_t64$|LBR2_siNT", grep("_B_", samples_names, value=TRUE), value=TRUE)

LBR2_NU7441_samples <- grep("LBR2_NU7441$|LBR2_ODNOT_NU7441|LBR2_NU7441_t64$", samples_names, value=TRUE)


LBR2_ODN3_samples <- grep("LBR2_ssODN3_DMSO_t64$|LBR2_ODN3_DMSO$", samples_names, value=TRUE)
LBR2_ODN3_NU7441_samples <- grep("LBR2_ssODN3_NU7441_t64$|LBR2_ODN3_NU7441$", samples_names, value=TRUE)

LBR1_samples <- grep("LBR1_-|LBR1_DMSO$|LBR1_ODNOT_DMSO|LBR1_t64$|LBR1_DMSO_t64$|LBR1_siNT", samples_names, value=TRUE)
...
mutations.regular.list <- all.norm.mutations.df.list[LBR2_samples]
names(mutations.regular.list)
dat <- do.call(rbind, setNames(mutations.regular.list, NULL)) # This is a big data.frame with all data 
repl <- length(mutations.regular.list)
...
reads.mut.barcode[reads.mut.barcode$barcode=="AAAAGGTTATCTCGGT.A", -1] == colSums(dat[dat$barcode=="AAAAGGTTATCTCGGT.A", -1])
...
Freq.LBR2_samples <- FrequencyReadsMut(all.norm.mutations.df.list, LBR2_samples)
BeeswarmIndelSpectrum(Freq.LBR2_samples)

Freq.LBR2.NU7441_samples <- FrequencyReadsMut(all.norm.mutations.df.list, LBR2_NU7441_samples)
BeeswarmIndelSpectrum(Freq.LBR2.NU7441_samples)

Freq.LBR2_ODN3_samples <- FrequencyReadsMut(all.norm.mutations.df.list, LBR2_ODN3_samples)
BeeswarmIndelSpectrum(Freq.LBR2_ODN3_samples)

Freq.LBR2_ODN3_NU7441_samples <- FrequencyReadsMut(all.norm.mutations.df.list, LBR2_ODN3_NU7441_samples)
BeeswarmIndelSpectrum(Freq.LBR2_ODN3_NU7441_samples)
dim(Freq.LBR2_ODN3_NU7441_samples)

Freq.LBR1_samples <- FrequencyReadsMut(all.norm.mutations.df.list, LBR1_samples)
BeeswarmIndelSpectrum(Freq.LBR1_samples)
