...
global_effects = indel.data %>%
  ...
  mutate(drug = gsub(" [(].*[)]", "", drug),
         drug_conc = paste(drug, conc_char, sep = " ")) %>%
  left_join(mean_viab_table) %>%
  filter(mean_TIF_global_pval_adj_comb < 0.01 & !(freqCut_diff_global_mean < -0.05 & mean_viab < 60)) %>%
...
## 2A Ranking 
```{r 2A Top TIF boosting, fig.width = 4, fig.height = 4}
rank_dt = indel.data %>% 
...
## 2B-C Zoom on rank
```{r 2BC Zoom on rank, fig.width = 8, fig.height = 4}
# Boosting effect
p1 = rank_dt %>% 
  filter(rank <= 75) %>%
  mutate(# n = ifelse(is.na(n), 1, 2),
