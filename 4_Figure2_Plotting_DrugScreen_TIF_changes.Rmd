...
eff.data <- indel.data.freq %>%
  ...
  mutate(mean_eff_pval_comb = 2*pnorm(q = abs(zscore_TIF_comb_rep), lower.tail = FALSE),
         mean_eff_pval_adj_comb = p.adjust(mean_eff_pval_comb, method = 'holm'),
...
global_effects = indel.data %>%
  ...
  mutate(mean_TIF_global_pval_comb = 2*pnorm(q = abs(zscore_TIF_global_comb_rep), lower.tail = FALSE),
         mean_TIF_global_pval_adj_comb = p.adjust(mean_TIF_global_pval_comb, method = 'holm'))  %>% 
  mutate(drug = gsub(" [(].*[)]", "", drug),
         drug_conc = paste(drug, conc_char, sep = " ")) %>%
  left_join(mean_viab_table) %>%
  filter(mean_TIF_global_pval_adj_comb < 0.01 & !(freqCut_diff_global_mean < -0.05 & mean_viab < 60) & viab_reproducibility == T & 
           indel_freq_filter == T, filter_zscore_TIF_global_comb_rep) %>%
  distinct(drug, drug_conc, conc_char, target, mean_TIF_global_pval_adj_comb, mean_viab) 
...
all_TIF = indel.data %>% 
  ...
  filter(filter_zscore_TIF_comb_rep & 
           ...
           # !between(zscore_TIF_comb_rep, -1.96, 1.96) & 
...
knitr::kable(all_TIF)
paste("Compounds that significantly (p holm adj < 0.01) changed the TIF in all 18 sites in both replicates: ", paste(all_TIF$drug_conc, collapse = ", "))
...
paste("Compounds that significantly (p holm adj < 0.01) changed the TIF in all 16 of the 18 sites in both replicates: ", paste(comp_near_all_TIF$drug_conc, collapse = ", "))
```
...
top_50_hits = eff.data.heatmap %>% 
  ...
  # slice(1:70) #%>%
  # filter(
  #   target %in% kinase_andco_targets,
  #   # target %in% chromatin_targets,
  # )
