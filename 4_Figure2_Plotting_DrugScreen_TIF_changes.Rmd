...
## Select outdir
out.dir = paste0("./figures/", Date, "_Bas_way/")
dir.create(out.dir)
in.dir = list.dirs(path = "/DATA/projects/DSBrepair/data/R") %>% .[grepl("episcreen_Bas_way", .)] %>% tail(n = 1)
...
## 2A: Heatmap of TIF differences {.tabset}
...
### Plot the heatmap
```{r 2A TIF heatmap, warning = FALSE, fig.width = 10, fig.height = 25}
...
## 2B Ranking 
```{r 2B Top TIF boosting, fig.width = 4, fig.height = 4}
rank_dt = indel.data %>% 
  filter(!drug %in% c("DMSO", "Mirin", "DNA-PKi") & indel_freq_filter & viab_reproducibility  & !is.na(zscore_TIF_global_comb_rep)  & cor_TIF_filter) %>%
...
rank_dt %>% 
  ...
  mutate(# n = ifelse(is.na(n), 1, 2),
    col = ifelse(!between(rank, 15, maxrank-8) & 
...
## 2C-D Zoom on rank
```{r 2CD Zoom on rank, fig.width = 8, fig.height = 4}
# Boosting effect
p1 = rank_dt %>% 
  filter(rank <= 75) %>%
  # left_join(near_all_TIF) %>%
