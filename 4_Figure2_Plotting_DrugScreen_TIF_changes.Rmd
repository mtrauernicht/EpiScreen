...
eff.data.heatmap <- eff.data %>%
  ...
  ungroup() %>%
  distinct(drug_conc, target, freqCut_diff_mean, IPR, min_score)
...
eff.clustering.dist = dist(eff.clustering, method = "manhattan")
...
plt1 = pheatmap(eff.data.heatmap2[eff.clustering.clust$labels , clone5_map_ipr$labels],
...
all_compounds_lowerTIF = indel.data %>% 
  filter(conc_char != "control" & indel_freq_filter & viab_reproducibility) %>% 
  mutate(drug = gsub(" [(].*[)]", "", drug),
         drug_conc = paste(drug, conc_char, sep = " ")) %>% 
