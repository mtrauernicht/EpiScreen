...
## Select outdir
out.dir = paste0("./figures_new_filter/", Date, "/")
dir.create(out.dir)
in.dir = list.dirs(path = "/DATA/projects/DSBrepair/data/R") %>% .[grepl("episcreen_new_filter", .)] %>% tail(n = 1)
...
all_TIF = indel.data %>% 
  distinct(drug, conc_char, replicate, zscore_TIF_comb_tech, filter_zscore_TIF_comb_rep, target, 
...
eff.data <- indel.data %>%
  ...
  filter(!(viab_mean < .60 & zscore_TIF_global_comb_rep < -2.54)) %>%  # we can choose to display all drugs that have significant global hits
