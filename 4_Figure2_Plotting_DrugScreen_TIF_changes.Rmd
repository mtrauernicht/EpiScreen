...
indel.data.freq <- indel.data %>%
  ...
  filter(!(freqCut_diff_global < -0.05 & viab_mean < max_viab_TIF_down)) %>%
...
mean_viab_table = indel.data %>% distinct(drug, conc_char, replicate, viab_mean, target ) %>%
  ...
  summarise(mean_viab = mean(viab_mean),
...
global_effects = indel.data %>%
  ...
  filter(mean_TIF_global_pval_adj_comb < 0.01 & 
           !(freqCut_diff_global_mean < -0.03 & mean_viab < max_viab_TIF_down) & 
...
comp_near_all_TIF = near_all_TIF %>% # At least 15 sites
  ...
  dplyr::count() %>% 
  left_join(mean_viab_table) %>%
  filter(n == 2 & mean_viab > max_viab_TIF_down) # Both replicates

near_all_TIF %>%
  left_join(mean_viab_table) %>% 
  filter(mean_viab > max_viab_TIF_down) %>%
  knitr::kable()
...
rank_dt = indel.data %>% 
  ...
  distinct(drug, conc_char, zscore_TIF_global_comb_rep , 
           target, viab_mean, viab_reproducibility, filter_zscore_TIF_global_comb_rep) %>% 
  mutate(viab_mean = ave(viab_mean, drug, conc_char, FUN = mean),
         ...
         mean_eff_pval_adj_comb = p.adjust(mean_eff_pval_comb, method = 'BH')) %>%
  distinct() %>%
  filter(!(viab_mean < max_viab_TIF_down & zscore_TIF_global_comb_rep < -2.54)) %>% 
...
p1 = rank_dt %>% 
  ...
  mutate(# n = ifelse(is.na(n), 1, 2),
    col = ifelse(filter_zscore_TIF_global_comb_rep & 
                   !(viab_mean < max_viab_TIF_down & zscore_TIF_global_comb_rep < -2.54) & 
                   mean_eff_pval_adj_comb < 0.01,
                 "black", "gray20")
  ) %>%
  # filter(rank < 100 ) %>%
...
p3 = rank_dt %>% 
  ...
  # left_join(near_all_TIF) %>%
  mutate(# n = ifelse(is.na(n), 1, 2),
    col = case_when(rank > (maxrank-30) & 
                   filter_zscore_TIF_global_comb_rep & 
                   !(viab_mean < .70 & zscore_TIF_global_comb_rep < -2.54) & 
...
eff.data <- indel.data %>%
  ...
  filter(!(viab_mean < max_viab_TIF_down & zscore_TIF_global_comb_rep < -1.96)) %>%  # we can choose to display all drugs that have significant global hits
...
mean_data_drug = indel.data %>%
  ...
  distinct(conc_char, drug, target, IPR, zscore_TIF_comb_rep, 
           freqCut_diff_mean, filter_zscore_TIF_comb_rep) %>%
...
target_all <- target_set_drugs %>% 
  ...
  # left_join(welch_tib) %>% # We don't use the Welch test finally.
...
rownames(cont_table) = c("AurK", "DNMT", "ERD", "HDAC", "HDM", "HMT", "JAK", "PARP", "PIM", "Sirtuin")
...
### Plot the heatmap
```{r 2D TIF heatmap, warning = FALSE, fig.width = 10, fig.height = 16}
...
decrease_hits = rank_dt %>% filter(filter_zscore_TIF_global_comb_rep & viab_reproducibility &
                   mean_eff_pval_adj_comb < 0.01 & zscore_TIF_global_comb_rep < 0 &
                     !(viab_mean < max_viab_TIF_down & zscore_TIF_global_comb_rep < -1.96)) %>% 
  distinct(target, drug) %>%
  group_by(target) %>%
  count() %>%
  dplyr::rename(effect = n) %>%
...
rownames(cont_table) = c("AurK", "ERD", "HDM", "other")
...
```{r S2A lowered TIF, fig.width = 10, fig.height = 5}
p1 = indel.data %>% mutate(drug = gsub(" [(].*[)]", "", drug),
                           drug_conc = paste(drug, conc_char, sep = " ")) %>% 
  filter(conc_char != "control" & !is.na(freqCut_diff_global_mean)) %>%
  distinct(zscore_TIF_global_comb_rep, viab_mean, 
           replicate, drug_conc, target, freqCut_diff_global_mean) %>%
  group_by(drug_conc, target) %>%
  summarise(viab_mean = mean(viab_mean),
            freqCut_diff_global_mean = mean(freqCut_diff_global_mean)) %>%
  ggplot(., aes(viab_mean, freqCut_diff_global_mean)) +
  geom_point(size = 2) + 
...
p2 = indel.data %>% mutate(drug = gsub(" [(].*[)]", "", drug),
                           drug_conc = paste(drug, conc_char, sep = " ")) %>% 
  filter(conc_char != "control" & !is.na(freqCut_diff_global_mean)) %>%
  distinct(zscore_TIF_global_comb_rep, viab_mean, 
           replicate, drug_conc, target, freqCut_diff_global_mean) %>%
  ...
  summarise(viab_mean = mean(viab_mean),
            freqCut_diff_global_mean = mean(freqCut_diff_global_mean)) %>%
  filter(!(viab_mean < max_viab_TIF_down & freqCut_diff_global_mean < 0)) %>%
  ggplot(., aes(viab_mean, freqCut_diff_global_mean)) +
  geom_point(size = 2) + 
  geom_point(data = . %>% filter(drug_conc %in% bottom_hits$drug_conc),
             aes(viab_mean, freqCut_diff_global_mean, color = target), size = 3) +
  stat_cor(method = "pearson",
           label.y = -.17,
           label.x = .6) +
  ...
  scale_color_manual(values = 
                       annotation_colors_scr$target[names(annotation_colors_scr$target) %in%
                                                      unique(bottom_hits$target)]) +
  theme_classic2(base_size = 16) +
  geom_vline(xintercept = max_viab_TIF_down, color = "red", linetype="dotted") +
  annotate("rect",
           xmin = 0, xmax = max_viab_TIF_down,
           ymin = -Inf, ymax = 0, 
           alpha = 0.05, fill = "red") +
  ylab("Global TIF change") +
  xlab("Viability compared to control") +
...
indel.data %>% mutate(drug = gsub(" [(].*[)]", "", drug),
  ...
  filter(conc_char != "control" & !is.na(freqCut_diff_global_mean)) %>%
  distinct(zscore_TIF_global_comb_rep, viab_mean, 
           replicate, drug_conc, target, drug, conc_char, freqCut_diff_global_mean) %>%
  group_by(drug_conc, target, drug, conc_char) %>%
  summarise(viab_mean = mean(viab_mean),
            freqCut_diff_global_mean = mean(freqCut_diff_global_mean)) %>%
  filter((viab_mean < max_viab_TIF_down & freqCut_diff_global_mean < 0)) %>%
  dplyr::select(drug, conc_char, target, viab_mean, freqCut_diff_global_mean) %>%
  write_excel_csv(., "DecreaseTIF_Filter_S2A.csv")
```
## S2B gRNA chromatin

```{r S2B gRNA chromatin}
# Feature clustering based on whole TRIP pool
gRNA_chrom_tib <- readRDS('./rs20230216_gRNA_valid_chip_Zscore.RDS') %>% 
# Remove HDAC1 and 2 as they do not reach higher than the 80th percentile of the HDAC 
  filter(pool == "episcreen_gRNA")  
  
gRNA_chrom = gRNA_chrom_tib %>% dplyr::select(-binsize, -pool) %>% column_to_rownames("ID")
...
# make clustering with some column rotation for better visualisation
gRNA_chrom_clust = hclust(dist(t(TRIP_chrom)), method = "ward.D")
...
feature_fill = c(insulator="#a6b5a3", repressive="#304441", euchromatin="#cda4cc",
                 transcribing="#d68d7d", HDAC="#aac9e0", accessibility='#cebc85',
                 methylation='#7dc98f')

order_vec=rownames(clustering)[nrow(clustering):1]

### Colors
quant_breaks <- quantile(gRNA_chrom, c(.01,.99), na.rm = T) # change these numbers according to where you want to set the cutoff
palette_length <- 100
...
pheatmap(t(gRNA_chrom), 
         cluster_rows = gRNA_chrom_clust,
         cluster_cols = F,
...
```{r LBR2 validation with the drugs in clone 5}
...
LBR2.validations = indel.data.validation %>% filter(exp_ID == "E2223")


ggplot(LBR2.validations, aes(drug, efficiency, color = drug)) +
  geom_jitter() + 
  # facet_wrap(. ~ barcode) + 
...
stat.test <- LBR2.validations %>%
  mutate(barcode = gsub(".B", "", barcode)) %>%
  left_join(indel.data %>% distinct(barcode, chromatin)) %>%
  group_by(chromatin) %>%
  pairwise_t_test(
    efficiency ~ drug, paired = TRUE,
    ref.group = "DMSOnG",
    p.adjust.method = "BH"
    ) %>%
  dplyr::select(-df) # Remove details
stat.test
```
...
### ChIP analysis
```{r ChIP, fig.width= 6, fig.height=4, echo = FALSE, eval=FALSE}
## Import computed scores (from Seb)
h3k27ac_files <- list.files(path = "/DATA/usr/m.trauernicht/projects/EpiScreen/data/chip_scores/", 
                            pattern ="density.normalized.scores_H3K27ac_.*",
                            full.names = T)

# Read all files using lapply and fread
h3k27ac_files <- lapply(h3k27ac_files, fread)
h3k27ac_all <- bind_rows(h3k27ac_files) %>%
  dplyr::select(chr, start, sample, group, "signal" = mean.by.region) %>%
  mutate(start = start + 1) %>%
  mutate(integration = paste("chr", chr, "_", start, sep = "")) %>%
  mutate(chromatin_mark = "H3K27ac") %>%
  mutate(radius = gsub("H3K27ac ", "", group)) %>%
  mutate(sample = gsub("H3K27ac_", "", sample)) %>%
  distinct(integration, sample, chromatin_mark, signal, radius)

h3k27me3_files <- list.files(path = "/DATA/usr/m.trauernicht/projects/EpiScreen/data/chip_scores/", 
                            pattern ="density.normalized.scores_H3K27me3_.*",
                            full.names = T)

# Read all files using lapply and fread
h3k27me3_files <- lapply(h3k27me3_files, fread)
h3k27me3_all <- bind_rows(h3k27me3_files) %>%
  dplyr::select(chr, start, sample, group, "signal" = mean.by.region) %>%
  mutate(start = start + 1) %>%
  mutate(integration = paste("chr", chr, "_", start, sep = "")) %>%
  mutate(chromatin_mark = "H3K27me3") %>%
  mutate(radius = gsub("H3K27me3 ", "", group)) %>%
  mutate(sample = gsub("H3K27me3_", "", sample)) %>%
  distinct(integration, sample, chromatin_mark, signal, radius)

chromatin_scores <- rbind(h3k27ac_all, h3k27me3_all)

## Import IPR data
ipr_data <- read_table("/DATA/projects/DSBrepair/Drug_screen_rev2/HDAC_ChIP/data/IPR_value/xv20230608_coordinates_clone5.txt") %>%
  mutate(integration = paste(chr, start, sep = "_")) %>%
  dplyr::select("barcode" = IPR_barcode, integration)

bc_ipr <- read_table("/DATA/usr/m.trauernicht/projects/EpiScreen/data/bc_ipr_chromatin.txt")

ipr_data <- ipr_data %>%
  left_join(bc_ipr)

## Load efficiency changes
eff_changes <- eff.data.heatmap %>%
  filter(drug_conc %in% c("PCI-24781 100 nM", "Tubastatin A 10 µM")) %>%
  dplyr::select(IPR, "sample" = drug, freqCut_diff_mean) %>%
  mutate(sample = ifelse(sample == "PCI-24781", "PCI", "TubA"))

## Combine data
chromatin_scores <- chromatin_scores %>%
  left_join(ipr_data) %>%
  left_join(eff_changes)

## Compute DMSO scores and drug-induced changes
dmso_chromatin_scores <- chromatin_scores %>%
  filter(sample == "DMSO") %>%
  mutate(dmso_signal =  signal) %>%
  dplyr::select(-signal, -freqCut_diff_mean, -sample)

chromatin_scores <- chromatin_scores %>%
  filter(sample != "DMSO") %>%
  left_join(dmso_chromatin_scores) %>%
  mutate(chromatin_fc = (signal) / (dmso_signal))


## Display chromatin change per window size, separate per chromatin mark and drug
for (i in unique(chromatin_scores$IPR)) {
  p <- ggplot(chromatin_scores %>%
                filter(IPR == i) %>%
                mutate(radius = gsub(" bp", "", radius)) %>%
                mutate(radius = as.numeric(radius)),
       aes(x = radius, y = chromatin_fc, group = IPR)) +
    geom_hline(yintercept = 1, lty = 2) +    
    geom_point() +
    geom_line() +
    facet_grid(sample ~ chromatin_mark) +
...
## Plot changes in chromatin signal vs. changes in TIF
for (i in unique(chromatin_scores$radius)) {
  p <- ggplot(chromatin_scores %>%
                filter(chromatin_mark == "H3K27me3", radius == i),
       aes(x = freqCut_diff_mean, y = chromatin_fc)) +
    geom_hline(yintercept = 1, lty = 2) +
    geom_point(aes(size = dmso_signal)) +
    facet_wrap(~ sample) +
    theme_pubr(border = T) +
    ggtitle(i)
...
for (i in unique(chromatin_scores$radius)) {
  p <- ggplot(chromatin_scores %>%
                filter(chromatin_mark == "H3K27ac", radius == i),
       aes(x = freqCut_diff_mean, y = chromatin_fc)) +
    geom_hline(yintercept = 1, lty = 2) +
    geom_point(aes(size = dmso_signal)) +
    facet_wrap(~ sample) +
