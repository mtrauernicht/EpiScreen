...
target <- indel.data[indel.data$target != c("Negative Control", "MRN", "DNA-PK"),] %>% 
  dplyr::select(drug, target) %>% unique() %>% remove_rownames %>% 
  column_to_rownames(var="drug")

# Add type of chr. mark (repressive or active)
type <- mapping_mean_long %>% dplyr::select(variable, type) %>% unique() %>%
  remove_rownames %>% column_to_rownames(var="variable")

# Add annotation of barcodes (based on clustering in 0_Clone5_Annotation.Rmd)
# barcode.annotation <- readRDS('/DATA/projects/DSBrepair/data/R/rs20200311_domains_clone5.RDS') %>%
#   dplyr::select('domain' = group, IPR) %>%
#   column_to_rownames("IPR")
# Add annotation of barcodes (based on clustering in 0_Clone5_Annotation.Rmd)
barcode.annotation <- unique(as.data.frame(indel.data.high$IPR))
...
barcode.annotation <- barcode.annotation %>% remove_rownames() %>%
  column_to_rownames("indel.data.high$IPR")
