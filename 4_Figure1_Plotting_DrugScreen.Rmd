...
ggplot(indel.data2 %>%
         filter(sample != "DNA-PKi (NHEJ control)", sample != "Mirin (MMEJ control)") %>%
         dplyr::distinct(MMEJscore, IPR, drug, conc_char, tech, sample, replicate) %>%
         mutate(freqCut = ave(MMEJscore, IPR, drug, conc_char, tech, replicate)),
...
biol_repl <- indel.data %>%
  filter(sample == "drug") %>%
  dplyr::distinct(MMEJ_zscore_global, drug, replicate, conc_char, sample) %>%
  spread(replicate, MMEJ_zscore_global) %>%
  na.omit()
