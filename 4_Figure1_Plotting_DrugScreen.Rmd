...
mutation.data <- readRDS(file.muts) %>% filter(IPR %in% c("IPR7", "IPR5"))
...
label_ratio <- filter(indel.data, IPR %in% c("IPR5", "IPR7"), drug == "DMSO") %>% 
  group_by(IPR) %>%
  summarise(MMEJratio = mean(MMEJratio),
            freqCut = mean(freqCut)) %>%
  mutate(label_efficiency = paste0("Total indel frequency = ", round(freqCut*100, 1),"%"), 
         label_ratio = paste0("MMEJ ratio = ", round(MMEJratio, 2)),
         label = IPR)
...
```{r 1DE TIF, warning = FALSE, fig.height=6, fig.width=3} 
...
CTRL.data = indel.data %>%
 filter(sample == "DMSO (neg control)" & indel_freq_filter == T & IPR %in% c("IPR7", "IPR5")) %>% 
...
indel.data2 <- indel.data %>%
 filter(sample %in% c("Epigenetic Compound") & indel_freq_filter == T & IPR %in% c("IPR7", "IPR5")) %>%
  distinct(freqCut, IPR, drug, conc_char, tech, sample, replicate, 
