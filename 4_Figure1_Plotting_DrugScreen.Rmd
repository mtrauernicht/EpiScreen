...
![Scheme of the experimental setup](figures/rs20220624_figure1A.png)
...
# C: Viability filtering
indel.data %>% 
  mutate(viab = ave(viab_mean, conc_char, drug)) %>% # average the two replicates for plotting
...
indel.data2 <- indel.data %>%
 filter(sample %in% c("Epigenetic Compound", "DMSO (neg control)") & indel_freq_filter == T) %>%
  distinct(freqCut, IPR, drug, conc_char, tech, sample, replicate, 
           barcode, MMEJratio, MMEJ_zscore_comb, mean_eff_zscore_comb, well, plate) %>%
  mutate(freqCut = ave(freqCut, IPR, drug, conc_char, tech, well, plate, replicate), 
         MMEJratio = ave(MMEJratio, IPR, drug, conc_char, tech, well, plate, replicate),
         sample_conc = ifelse(sample == "Epigenetic Compound", as.character(conc_char), as.character(sample))) %>%
  distinct()
         
sample_order <- c("DMSO (neg control)", "100 nM", "1 µM", "10 µM")
indel.data2$sample_conc <- factor(indel.data2$sample_conc, levels = sample_order)
...
plt1 = indel.data2 %>%
  mutate(freqCut = ave(freqCut, drug, conc_char, well, plate, IPR, FUN = mean)) %>%
  ...
  scale_color_manual(values = c("DMSO (neg control)" = colors_drugs[2], "100 nM" = colors_drugs[7], "1 µM" = colors_drugs[4], "10 µM" = "#000000")) +
...
plt2 = 
  indel.data2 %>%
  distinct(mean_eff_zscore_comb, drug, conc_char, IPR, chromatin, well, plate, sample_conc) %>%
  ggplot(.,
         aes(x = IPR, y = mean_eff_zscore_comb)) +
  geom_quasirandom_rast(aes(color = sample_conc), dodge.width = 0.75, raster.dpi = 600) +
...
plt1 = 
  indel.data2 %>%
        distinct(MMEJratio , drug, conc_char, IPR, chromatin, sample_conc) %>%
          mutate(MMEJratio = ave(MMEJratio, drug, conc_char, IPR, FUN = mean)) %>%
          distinct() %>% 
...
```{r control distribution of indel rates across wells and replicates, warning = FALSE, fig.height=6, fig.width=8}
ggplot(indel.data %>%
         filter(drug == "DMSO") %>%
...
```{r control distribution of MMEJ ratio across wells and replicates, warning = FALSE, fig.height=4, fig.width=4}
ggplot(indel.data %>%
         filter(drug == "DMSO") %>%
...
## S_#: Comparison between technical replicates of efficiency in DMSO

```{r comparison for mean indel rate across replicates, warning = FALSE, fig.height=4, fig.width=4}
ggplot(indel.data %>%
         filter(drug == "DMSO") %>%
