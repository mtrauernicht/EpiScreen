...
plot_grid(plt1, plt2, labels = "AUTO", nrow = 2)
```
...
pheatmap(chrom_matrix, color = colors, breaks = breaks.chip, annotation_row=clustering)

group_colors = group_colors[names(group_colors) %in% unique(domain_matrix$group)]
...
```{r DMSO NHEJ ratios in both replicates}
biol_repl <- indel.data %>%
  filter(sample == "DMSO (neg control)", pathway_bal_filter == T) %>%
  dplyr::distinct(NHEJratio, drug, replicate, conc_char, sample, barcode, tech, well, plate) %>%
  na.omit()

ggplot(biol_repl,
       aes(x = replicate, y = NHEJratio)) +
  geom_quasirandom() +
  stat_cor(method = "pearson") + facet_wrap(. ~ barcode)
```
