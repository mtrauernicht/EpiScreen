...
```{r Fig2B chromideograms, fig.height=5, fig.width=6}
p1 = ggplot() +
...
```{r 1DE TIF, warning = FALSE, fig.height=5, fig.width=6} 
...
```{r 1F MMEJ ratio, warning = FALSE, fig.height=5, fig.width=6}
...
techn_repl <- indel.data %>%
  ...
  dplyr::distinct(MMEJratio_norm, drug, tech, replicate, conc_char, sample) %>%
  mutate(MMEJratio_norm = log2(ave(MMEJratio_norm, drug, tech, replicate, conc_char, FUN = mean))) %>%
  unique() %>%
  mutate(rep = paste(replicate, tech, sep = "_")) %>%
  dplyr::select(-replicate, -tech) %>%
  spread(rep, MMEJratio_norm) %>%
  na.omit()
...
techn_repl <- indel.data %>%
  filter(sample == "Epigenetic Compound" & indel_freq_filter == T & viab_reproducibility == T) %>%
...
biol_repl <- indel.data %>%
  filter(sample == "Epigenetic Compound" & viab_reproducibility == TRUE) %>%
  dplyr::distinct(filter_zscore_bal_global_comb_rep, filter_zscore_TIF_global_comb_rep, filter_zscore_TIF_global_comb_rep, filter_zscore_bal_global_comb_rep, drug, replicate, conc_char, sample, ) %>%
  pivot_wider(names_from = replicate, values_from =  c(filter_zscore_bal_global_comb_rep, filter_zscore_TIF_global_comb_rep)) %>%
  na.omit()

p1 = ggplot_custom(biol_repl,
       aes(x = filter_zscore_TIF_global_comb_rep_E1504, y = filter_zscore_TIF_global_comb_rep_E177)) +
  geom_point() +
...
p2 = ggplot_custom(biol_repl,
       aes(x = filter_zscore_bal_global_comb_rep_E1504, y = filter_zscore_bal_global_comb_rep_E177)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
  stat_cor(method = "pearson") +
...
p3 = ggplot_custom(
  filter(biol_repl, filter_zscore_TIF_global_comb_rep),
       aes(x = filter_zscore_TIF_global_comb_rep_E1504, y = filter_zscore_TIF_global_comb_rep_E177)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
...
p4 = ggplot_custom(
  filter(biol_repl, filter_zscore_bal_global_comb_rep),
       aes(x = filter_zscore_bal_global_comb_rep_E1504, y = filter_zscore_bal_global_comb_rep_E177)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
...
biol_repl <- indel.data %>%
  filter(sample == "Epigenetic Compound" & viab_reproducibility == TRUE) %>%
  dplyr::distinct(filter_zscore_TIF_global_comb_rep, drug, replicate, conc_char, sample) %>%
  spread(replicate, filter_zscore_TIF_global_comb_rep) %>%
  na.omit()

ggplot_custom(biol_repl,
       aes(x = E1504, y = E177)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
