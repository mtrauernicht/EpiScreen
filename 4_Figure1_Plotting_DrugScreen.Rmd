...
techn_repl <- indel.data %>%
  filter(sample == "drug") %>%
  dplyr::distinct(MMEJratio, drug, tech, replicate, conc_char, sample) %>%
  mutate(MMEJratio = ave(MMEJratio, drug, tech, replicate, conc_char, FUN = mean)) %>%
  unique() %>%
  mutate(rep = paste(replicate, tech, sep = "_")) %>%
  dplyr::select(-replicate, -tech) %>%
  spread(rep, MMEJratio) %>%
  na.omit()
