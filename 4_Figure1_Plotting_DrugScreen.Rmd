...
CTRL.data = indel.data %>%
  ...
  distinct(freqCut, IPR, drug, conc_char, tech, sample, replicate, 
           barcode, MMEJratio, zscore_bal_comb_rep, zscore_bal_comb_tech, zscore_TIF_comb_rep,  plate, well) %>%
...
## S1A: Viability filtering
How many drugs are filtered out because they are too toxic?
```{r S1A Viability filter, warning = FALSE, fig.height = 4, fig.width = 4}
...
```{r S1G control distribution of MMEJ ratio across wells and replicates, warning = FALSE, fig.height=3, fig.width=5}
indel.data %>%
  filter(drug == "DMSO") %>%
...
```{r S1H TIF zscores per IPR, warning = FALSE, fig.height=3, fig.width=8}
...
```{r S1I bal zscore per IPR, warning = FALSE, fig.height=3, fig.width=8}
  ggplot(zscore_dt,
...
## S1J: Correlation between technical replicates MMEJ score

How well does the MMEJ score correlate between the six technical replicates?
```{r S1J normalized MMEJ ratio correlation, warning = FALSE, fig.height=5, fig.width=5}
...
techn_repl <- indel.data %>%
  ...
  mutate(MMEJ_log2_FC = ave(MMEJ_log2_FC, drug, tech, replicate, conc_char, FUN = mean)) %>%
  unique() %>%
...
## S1K: Correlation between technical replicates efficiency
How well does the cutting efficiency correlate between the six technical replicates
```{r S1K normalized TIF correlation, warning = FALSE, fig.height=5, fig.width=5}
...
techn_repl <- indel.data %>%
  ...
  mutate(freqCut_norm = log2(ave(freqCut_norm, drug, conc_char, replicate, tech, FUN = mean))) %>%
  distinct() %>%
  mutate(rep = paste(replicate, tech, sep = "_")) %>%
  dplyr::select(-replicate, -tech) %>%
...
biol_repl_TIF <- indel.data %>%
  filter(sample == "Epigenetic Compound" & 
           ...
           cor_TIF_filter & 
           !is.na(zscore_TIF_global_comb_tech)) %>%
  dplyr::distinct(zscore_TIF_global_comb_tech, 
                  filter_zscore_TIF_global_comb_rep, 
                  drug, replicate, conc_char) %>%
  pivot_wider(names_from = replicate, 
              values_from = zscore_TIF_global_comb_tech) %>%
  na.omit()
...
biol_repl_bal <- indel.data %>%
  filter(sample == "Epigenetic Compound" & 
           viab_reproducibility == TRUE & 
           cor_bal_filter & 
           !is.na(zscore_bal_global_comb_tech)) %>%
  dplyr::distinct(zscore_bal_global_comb_tech, 
                  filter_zscore_bal_global_comb_rep, 
                  drug, replicate, conc_char) %>%
  pivot_wider(names_from = replicate, 
              values_from = zscore_bal_global_comb_tech) %>%
  na.omit()

p1 = ggplot_custom(biol_repl_TIF,
       aes(x = E1504, y = E177)) +
  geom_point() +
...
p2 = ggplot_custom(biol_repl_bal,
       aes(x = E1504, y = E177)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
