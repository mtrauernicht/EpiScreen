...
## Select outdir
# out.dir = paste0("/DATA/projects/DSBrepair/scratch/figures_episcreen/figure1_", Date, "/")
# dir.create(out.dir)
...
ggplot(targets, 
       aes(y = reorder(target, +number), x = number, fill = as.numeric(number))) + 
...
ggplot_custom(viability.data %>%
       ...
       aes(x = conc_char, y = mean_viab)) +
  geom_quasirandom(aes(fill = removed), color = "black", shape = 21, size = 2) +
  scale_fill_manual(values = c(colors_diverse[1], colors_diverse[4])) +
  theme_pubr(border = T)
```

*Figure 1C: Filtering of toxic compounds. Compounds that decreased viability by >75% in more than one technical replicate in one or both biological replicate were removed (marked in yellow).*
...
indel.data2 <- indel.data %>%
  ...
  dplyr::distinct(freqCut, IPR, drug, conc_char, tech, sample, replicate, barcode, MMEJratio, MMEJ_zscore_comb, mean_eff_zscore_comb) %>%
...
indel.data2$sample[indel.data2$sample == "drug" & indel.data2$conc_char == "100 nM"] <- "100 nM"
indel.data2$sample[indel.data2$sample == "drug" & indel.data2$conc_char == "10 µM"] <- "10 µM"
indel.data2$sample[indel.data2$sample == "drug" & indel.data2$conc_char == "1 µM"] <- "1 µM"
...
plt1 = ggplot(indel.data2 %>%
         mutate(freqCut = ave(freqCut, drug, conc_char, IPR, FUN = mean)) %>%
         distinct(freqCut, drug, conc_char, IPR, chromatin, sample),
       aes(x = IPR, y = freqCut*100)) +
  geom_quasirandom_rast(aes(color = sample), dodge.width = 0.75, raster.dpi = 600, size = 0.5) +
...
plt2 = ggplot(indel.data2 %>%
        distinct(MMEJratio , drug, conc_char, IPR, chromatin, sample) %>%
          mutate(MMEJratio = ave(MMEJratio, drug, conc_char, IPR, FUN = mean)) %>%
          distinct(),
       aes(x = IPR, y = MMEJratio*100)) +
  geom_quasirandom_rast(aes(color = sample), dodge.width = 0.75, raster.dpi = 600, size = 0.5) +
...
plt2 = ggplot(indel.data2 %>%
        distinct(MMEJ_zscore_comb, drug, conc_char, IPR, chromatin, sample),
