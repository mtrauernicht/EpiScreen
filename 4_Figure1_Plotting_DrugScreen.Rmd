...
indel.data %>% 
  ...
  mutate(viab = ave(viab_mean, conc_char, drug, FUN = function(x) mean(x, na.rm = TRUE))) %>% # average the two replicates for plotting
  distinct(conc_char, drug, viab, pathway_bal_filter) %>%
  ggplot_custom(.,
              aes(x = conc_char, y = viab)) +
    geom_quasirandom(aes(fill = pathway_bal_filter), color = "black", shape = 21, size = 2) +
    scale_fill_manual(values = c("#FA8D62", colors_diverse[1])) +
    theme_pubr(border = T) +
  xlab("Concentration") +
...
```{r 1DE TIF, warning = FALSE, fig.height=8, fig.width=10} 
...
conc_control_colors = c("DMSO (neg control)" = colors_drugs[2], "100 nM" = colors_drugs[7], "1 µM" = colors_drugs[4], "10 µM" = "#000000")
conc_colors = c("100 nM" = colors_drugs[7], "1 µM" = colors_drugs[4], "10 µM" = "#000000")


# D: Cutting efficiency per IPR
CTRL.data = indel.data %>%
 filter(sample == "DMSO (neg control)" & indel_freq_filter == T) %>% 
...
indel.data2 <- indel.data %>%
 filter(sample %in% c("Epigenetic Compound") & indel_freq_filter == T) %>%
  distinct(freqCut, IPR, drug, conc_char, tech, sample, replicate, 
...
sample_order <- c("DMSO (neg control)", "100 nM", "1 µM", "10 µM")
...
plt2 =  indel.data2 %>%
  ...
  geom_quasirandom_rast(aes(color = sample_conc), dodge.width = 0.75, raster.dpi = 600, size = 0.5) +
  scale_color_manual(values = conc_colors, guide=FALSE) +
  facet_wrap(~chromatin, scales = "free_x", nrow = 1) +
...
plot_grid(plt1, plt2, labels = c("D", "E"), nrow = 2)
```
...
```{r 1FG MMEJ ratio, warning = FALSE, fig.height=8, fig.width=10}
# E: MMEJ score per IPR
plt1 = 
  indel.data2 %>%
...
plot_grid(plt1, plt2, labels = c("F", "G"), nrow = 2)
```
...
indel.data %>%
  filter(sample == "Epigenetic Compound") %>%
  distinct(viab_mean, sample, replicate, conc_char, drug, pathway_bal_filter) %>%
  pivot_wider(names_from = replicate, values_from = viab_mean) %>%
  ggplot_custom(., aes(x = E177, y = E1504)) +
  geom_point(aes(color = pathway_bal_filter), size = .4, stroke = NA) +
  ...
  scale_color_manual(values = c(colors_diverse[4], colors_diverse[1])) +
...
## S1C: Comparison between replicates of efficiency in DMSO

```{r S1C control distribution of indel rates across wells and replicates, warning = FALSE, fig.height=3, fig.width=5}
ggplot(indel.data %>%
         filter(drug == "DMSO") %>%
...
## S1D: Comparison between replicates MMEJ ratio in DMSO

```{r S1D control distribution of MMEJ ratio across wells and replicates, warning = FALSE, fig.height=3, fig.width=5}
ggplot(indel.data %>%
         filter(drug == "DMSO") %>%
...
## S1F: Correlation between technical replicates MMEJ score

How well does the MMEJ score correlate between the six technical replicates?
```{r S1F normalized MMEJ ratio correlation, warning = FALSE, fig.height=5, fig.width=5}
...
## S1G: Correlation between technical replicates efficiency
How well does the cutting efficiency correlate between the six technical replicates
```{r S1G normalized TIF correlation, warning = FALSE, fig.height=5, fig.width=5}
...
```{r S1H MMEJ z-score correlation, warning = FALSE}
...
```{r S1I TIF z-score correlation, warning = FALSE}
...
## S1J: Correlation between viability and TIF
...
```{r S1J TIF correlation with viability, warning=F}
...
## S1K: Correlation between viability and MMEJ ratio
...
```{r S1K MMEJ ratio correlation with viability, warning=F}
