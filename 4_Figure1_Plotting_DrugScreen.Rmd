...
file = list.files(in.dir, pattern = "ratios") %>% tail(n = 1)
indel.data <- readRDS("/DATA/projects/DSBrepair/data/R/rs20220624_episcreen/rs20220624_episcreen_ratios.2.RDS")
...
viability.data$group <- "Epigenetic Compound"
...
ggplot_custom(viability.data %>%
                filter(group == "Epigenetic Compound") %>%
...
indel.data2 <- indel.data %>%
 filter(sample %in% c("Epigenetic Compound", "DMSO (neg control)")) %>%
...
ggplot_custom(viability.data %>%
                filter(group == "Epigenetic Compound"), aes(x = E177, y = E1504)) +
  geom_point(aes(color = removed), size = 1, stroke = NA) +
