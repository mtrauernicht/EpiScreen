...
ggplot(indel.data2 %>%
         ...
         dplyr::distinct(MMEJscore, IPR, tech, plate, conc_char, drug, sample) %>%
           mutate(MMEJscore = ave(MMEJscore, IPR, tech, plate, conc_char, drug, FUN = mean)),
       aes(x = IPR, y = MMEJscore)) +
  geom_violin(aes(fill = sample), position = position_dodge(0.75), draw_quantiles = 0.5, width = 1.5) +
  scale_fill_manual(values = c(colors_drugs[2], colors_drugs[6], colors_drugs[7], colors_drugs[4])) +
  coord_polar()+
  ylim(-0.2,0.55)
