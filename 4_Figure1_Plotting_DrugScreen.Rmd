...
```{r warning = FALSE, fig.height=1.6, fig.width=1}
# B: Drugs per target group
...
ggplot(targets, 
       aes(y = reorder(target, number), x = number)) + 
...
```{r warning = FALSE, fig.height = 1.6, fig.width = 1.6}
# C: Viability filtering
...
ggplot_custom(viability.data %>%
  ...
  scale_fill_manual(values = c(colors_diverse[1], "#FA8D62")) +
  theme_pubr(border = T)
```
