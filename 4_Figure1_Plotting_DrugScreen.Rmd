...
target_color = c(HDAC="#60988D", HAT="#C6D0A8", Sirtuin="#8FCAAC",
...
CTRL.data = indel.data %>%
  filter(sample == "DMSO (neg control)" & indel_freq_filter & IPR %in% IPR_set ) %>% 
  distinct(freqCut, IPR, drug, conc_char, tech, sample, replicate, 
           barcode, MMEJratio, zscore_bal_comb_rep, zscore_TIF_comb_rep,  plate, well) %>%
  ...
  dplyr::select(-well, -tech)

indel.data.iprset <- indel.data %>%
  filter(sample %in% c("Epigenetic Compound") & indel_freq_filter & IPR %in% IPR_set ) %>%
  distinct(freqCut, IPR, drug, conc_char, tech, sample, replicate, 
           barcode, MMEJratio, zscore_bal_comb_rep, zscore_TIF_comb_rep) %>%
  left_join(chromatin_tib) %>%
...
plt1 = indel.data.iprset %>%
  ...
  geom_quasirandom_rast(aes(color = sample_conc), dodge.width = 0.75, raster.dpi = 300, size = 2) +
  scale_color_manual(values = conc_control_colors, guide = "none") +
  facet_wrap(IPR ~ ., nrow = 1) +
  ylim(0,100)  +
  theme_classic_nolines_90() + 
  ylab("Total indel frequency [TIF]")

plt2 =  indel.data.iprset %>%
  distinct(zscore_TIF_comb_rep, drug, conc_char, IPR, chromatin, sample_conc) %>%
  mutate(sample_conc = factor(sample_conc, levels = sample_order),
         IPR = factor(IPR, levels = )) %>%
  ggplot(.,
         aes(x = sample_conc, y = zscore_TIF_comb_rep)) +
  geom_quasirandom_rast(aes(color = sample_conc), dodge.width = 0.75, raster.dpi = 300, size = 2) +
...
plot_grid(plt1, plt2, labels = c("E", "G"), ncol = 2)
```
...
plt2 = indel.data.iprset %>%
  distinct(zscore_bal_comb_rep, drug, conc_char, IPR, chromatin, sample_conc) %>%
    mutate(sample_conc = factor(sample_conc, levels = sample_order)) %>%
  ggplot(.,
         aes(x = sample_conc, y = zscore_bal_comb_rep)) +
  geom_quasirandom_rast(aes(color = sample_conc), dodge.width = 0.75, raster.dpi = 300, size = 2) +
  scale_color_manual(values = conc_colors, guide= "none") +
  facet_wrap(IPR ~ ., nrow = 1) +
  geom_hline(linetype = "dashed", yintercept = 2.58) +
...
plot_grid(plt1, plt2, labels = c("F", "H"), ncol = 2)
```
...
## S1A: Correlate viability data from the 2 replicates
...
```{r S1A viability correlation, warning = FALSE, fig.height=4, fig.width=8}
...
## S1B: Correlation between viability and TIF
...
```{r S1B TIF correlation with viability, warning=F, fig.height=5, fig.width=5}
...
## S1C: Correlation between viability and MMEJ ratio
...
```{r S1C MMEJ ratio correlation with viability, warning=F, fig.height=5, fig.width=5}
...
## S1D: Comparison between technical replicates of efficiency in DMSO

```{r S1D comparison for mean indel rate across replicates, warning = FALSE, fig.height=5, fig.width=3}
indel.data %>%
...
## S1G: Combined TIF z-scores per IPR per concentration
```{r S1G TIF zscores per IPR, fig.height=3, fig.width=8}
...
## S1H: Combined balance z-scores per IPR per concentration
```{r S1H bal zscore per IPR, fig.height=3, fig.width=8}
  ggplot(zscore_dt,
...
## S1I: Correlation between technical replicates MMEJ score
How well does the MMEJ score correlate between the six technical replicates?
```{r S1I normalized MMEJ ratio correlation, warning = FALSE, fig.height=5, fig.width=5}
# B: Correlation between technical replicates MMEJ score
techn_repl <- indel.data %>%
  filter(sample == "Epigenetic Compound" & pathway_bal_filter == T & viab_reproducibility == T) %>%
  dplyr::distinct(MMEJ_log2_FC, drug, tech, replicate, conc_char, sample) %>%
  mutate(MMEJ_log2_FC = ave(MMEJ_log2_FC, drug, tech, replicate, conc_char, FUN = function(x) mean(x, na.rm = TRUE))) %>%
  unique() %>%
...
techn_repl <- indel.data %>%
  ...
  mutate(freqCut_norm = log2(ave(freqCut_norm, drug, conc_char, replicate, tech, FUN = function(x) mean(x, na.rm = TRUE)))) %>%
  distinct() %>%
  mutate(rep = paste(replicate, tech, sep = "_")) %>%
  dplyr::select(-replicate, -tech) %>%
...
ggpairs(techn_repl %>% dplyr::select(contains("E1")),
        ...
        diag = list(continuous = function(data, mapping, ...) {
          ggally_densityDiag(data = data, mapping = mapping, alpha = 0.3, fill = "red")})) +
  xlab("Indel rate change [Log2FC]") +
  ylab("Indel rate change [Log2FC]")
```
...
## S1K-N: Correlation in MMEJ z-score between biological replicates
How well does the MMEJ z-score correlate between the biologcial replicates before combining them?
```{r S1KLMN MMEJ global z-score correlation, warning = FALSE, fig.height=5, fig.width=5}
...
p3 = ggplot(
  filter(biol_repl_TIF, filter_zscore_TIF_global_comb_rep),
       aes(x = E1504, y = E177)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
  stat_cor(method = "pearson") +
  xlab("Screen 2 TIF ratio z-score filtered") +
  ylab("Screen 1 TIF ratio z-score filtered") +
  theme_classic(base_size = 16) +
  geom_hline(yintercept = 1.96, linetype = "dashed") +
  geom_hline(yintercept = -1.96, linetype = "dashed") +
  geom_vline(xintercept = 1.96, linetype = "dashed") +
...
p4 = ggplot(
  filter(biol_repl_bal, filter_zscore_bal_global_comb_rep),
       aes(x = E1504, y = E177)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
  stat_cor(method = "pearson") +
  xlab("Screen 2 MMEJ ratio z-score filtered") +
  ylab("Screen 1 MMEJ ratio z-score filtered") +
  theme_classic(base_size = 16) + 
  geom_hline(yintercept = 1.96, linetype = "dashed") +
  geom_hline(yintercept = -1.96, linetype = "dashed") +
  geom_vline(xintercept = 1.96, linetype = "dashed") +
...
plot_grid(p1, p2, p3, p4, ncol = 2, nrow = 2, labels = c("K", "L", "M", "N"))
```
...
p3 = ggplot(
  filter(biol_repl_TIF, filter_zscore_TIF_comb_rep),
       aes(x = E1504, y = E177)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
...
p4 = ggplot(
  filter(biol_repl_bal, filter_zscore_bal_comb_rep),
       aes(x = E1504, y = E177)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
