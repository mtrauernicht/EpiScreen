...
indel.data <- readRDS("/DATA/usr/m.trauernicht/projects/EpiScreen/files_scripts/mt20220121_episcreen.RDS")
...
ggplot(indel.data2 %>%
         ...
         dplyr::distinct(freqCut, IPR, drug, conc_char, tech, sample, replicate) %>%
...
ggplot(indel.data2 %>%
         filter(sample != "DNA-PKi (NHEJ control)", sample != "Mirin (MMEJ control)") %>%
         dplyr::distinct(MMEJratio, IPR, drug, conc_char, tech, sample, replicate) %>%
...
ggpairs(techn_repl %>% dplyr::select(contains("E1")),
               ...
               lower = list(continuous = function(data, mapping, ...) {
                   ...
                   geom_abline(slope = 1, lty = "dashed", col = "grey") +
...
ggpairs(biol_repl %>% dplyr::select(contains("E1")),
               ...
               diag = list(continuous = function(data, mapping, ...) {
                   ggally_densityDiag(data = data, mapping = mapping, alpha = 0.3, fill = "red")})) +
  xlab("MMEJ ratio (z-score)") +
  ylab("MMEJ ratio (z-score)")
```
