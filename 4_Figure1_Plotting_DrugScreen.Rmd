...
CTRL.data = indel.data %>%
  ...
  distinct(freqCut, IPR, drug, conc_char, tech, sample, replicate, 
           barcode, MMEJratio, zscore_MMEJ_mean_comb, zscore_MMEJ_mean, zscore_TIF_mean_comb,  plate, well) %>%
...
indel.data2 <- indel.data %>%
  ...
  distinct(freqCut, IPR, drug, conc_char, tech, sample, replicate, 
           barcode, MMEJratio, zscore_MMEJ_mean_comb, zscore_TIF_mean_comb) %>%
  left_join(chromatin_tib) %>%
...
plt2 =  indel.data2 %>%
  distinct(zscore_TIF_mean_comb, drug, conc_char, IPR, chromatin, sample_conc) %>%
  mutate(sample_conc = factor(sample_conc, levels = sample_order),
         IPR = factor(IPR, levels = )) %>%
  ggplot(.,
         aes(x = sample_conc, y = zscore_TIF_mean_comb)) +
  geom_quasirandom_rast(aes(color = sample_conc), dodge.width = 0.75, raster.dpi = 300, size = 1) +
...
plt2 = indel.data2 %>%
  distinct(zscore_MMEJ_mean_comb, drug, conc_char, IPR, chromatin, sample_conc) %>%
    mutate(sample_conc = factor(sample_conc, levels = sample_order)) %>%
  ggplot(.,
         aes(x = sample_conc, y = zscore_MMEJ_mean_comb)) +
  geom_quasirandom_rast(aes(color = sample_conc), dodge.width = 0.75, raster.dpi = 600, size = 0.5) +
  scale_color_manual(values = conc_colors, guide= "none") +
  facet_wrap(IPR ~ ., nrow = 1) +
  geom_hline(linetype = "dashed", yintercept = 2.58) +
...
biol_repl <- indel.data %>%
  ...
  dplyr::distinct(zscore_MMEJ_global_mean, zscore_TIF_global_mean, cor_zscore_TIF_filter, cor_zscore_bal_filter, drug, replicate, conc_char, sample, ) %>%
  pivot_wider(names_from = replicate, values_from =  c(zscore_MMEJ_global_mean, zscore_TIF_global_mean)) %>%
  na.omit()

p1 = ggplot_custom(biol_repl,
       aes(x = zscore_TIF_global_mean_E1504, y = zscore_TIF_global_mean_E177)) +
  geom_point() +
...
p2 = ggplot_custom(biol_repl,
       aes(x = zscore_MMEJ_global_mean_E1504, y = zscore_MMEJ_global_mean_E177)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
  stat_cor(method = "pearson") +
...
p3 = ggplot_custom(
  filter(biol_repl, cor_zscore_TIF_filter),
       aes(x = zscore_TIF_global_mean_E1504, y = zscore_TIF_global_mean_E177)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
...
p4 = ggplot_custom(
  filter(biol_repl, cor_zscore_bal_filter),
       aes(x = zscore_MMEJ_global_mean_E1504, y = zscore_MMEJ_global_mean_E177)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
...
biol_repl <- indel.data %>%
  filter(sample == "Epigenetic Compound" & viab_reproducibility == TRUE) %>%
  dplyr::distinct(zscore_TIF_global_mean, drug, replicate, conc_char, sample) %>%
  spread(replicate, zscore_TIF_global_mean) %>%
  na.omit()
