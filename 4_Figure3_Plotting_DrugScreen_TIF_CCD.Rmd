...
CCD_example_tib = indel.data %>% 
  mutate(drug = gsub(" [(].*[)]", "", drug),
         drug_conc = paste(drug, conc_char, sep = " ")) %>%
  filter(drug_conc %in% c("EPZ-6438 10 µM", "Decitabine 1 µM", "Scriptaid 1 µM", "Vorinostat 1 µM", "PCI-24781 100 nM", "DMSO control", "SNS-314 Mesylate 1 µM")) %>%
  ...
  mutate(Vorinostat_log2_FC_TIF = log2(Vorinostat/DMSO),
    Scriptaid_log2_FC_TIF = log2(Scriptaid/DMSO),
...
p2 = ggplot(CCD_example_tib,
            aes(x = LMNB1.zscore, y = PCI_log2_FC_TIF)) +
  geom_point() +
  geom_smooth(method = "lm", color = "#01665e",se = FALSE) +
  stat_cor(method = "pearson") +
  geom_vline(xintercept = 0, linetype = 2) + 
...
colors = colorRampPalette(c("#d5bdaf", "#e5e5e5", "#84a98c"))(palette_length)
