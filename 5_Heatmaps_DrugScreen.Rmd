...
indel.data <- get(load("mt20190510_indel.data"))
...
target <- target[target$Target != c("Negative Control", "MRN", "DNA-PK"),]
...
chip_data <- dcast(chip_data, Var2 ~ Var1, value.var = "mean")
chip_data <- chip_data[chip_data$Var2 != 
                         "ACCCCTAAAGGCGCTG" & chip_data$Var2 != "CTCTTAATCGCTGCC", ]
...
setnames(delay.data, "barcode", "Var2")
...
dnameth.data <- chip_data3[,c(1,17)]
...
# Boxplot
mean.ratio <- indel.data[indel.data$Drug == "GSK J4 HCl" & indel.data$conc == "10um",]
...
```{r}
## In case I want to include the mean.ratio
mean.ratio <- indel.data[indel.data$Drug == "3-Deazaneplanocin A (DZNeP)" & indel.data$conc == "1um",]
mean.ratio <- mean.ratio[,c(1,25)]
mean.ratio$logratio.platenorm <- ave(mean.ratio$logratio.platenorm, mean.ratio$barcode, FUN = function(x) mean(x))
mean.ratio <- unique(mean.ratio)
setnames(mean.ratio, "barcode", "Var2")
...
hdac1.data <- chip_data[,c(1,3)]
hdac.1.data <- cbind(hdac1.data, mean.ratio2)

ggplot(hdac.1.data, aes(x=H3K27me3, y=logratio.platenorm))+
  geom_point()+
  geom_smooth(method = lm) 
```
