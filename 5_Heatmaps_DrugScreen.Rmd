...
indel.data <- get(load("mt20200501_indel.data"))
...
indel.data$mean.eff.bc.drug <- 
  ave(indel.data$efficiency_zscore, 
      indel.data$barcode, indel.data$conc,
      indel.data$Drug, FUN = function(x) mean(x))
...
indel.data.high.drug.ess <- indel.data.high.drug[, colSums(indel.data.high.drug > 3) >= 1 |
                            colSums(indel.data.high.drug < -5) >= 1]
# Keeping the scale in the pheatmap function
myBreaks2 <- c(seq(-10, 0, length.out=ceiling(100/2) + 1), 
              seq(9/100, 9/2, length.out=floor(100/2)))
...
indel.data.med.drug.ess <- indel.data.med.drug[, colSums(indel.data.med.drug > 3) >= 1 |
                            colSums(indel.data.med.drug < -3) >= 1] 
...
indel.data.low.drug.ess <- indel.data.low.drug[, colSums(indel.data.low.drug > 2) >= 1 |
                            colSums(indel.data.low.drug < -3) >= 1]
...
indel.data.high.eff.ess <- indel.data.high.eff[, colSums(indel.data.high.eff > 3) >= 1 |
                            colSums(indel.data.high.eff < -4) >= 1]
...
indel.data.med.eff.ess <- indel.data.med.eff[, colSums(indel.data.med.eff > 3) >= 1 | 
                           colSums(indel.data.med.eff < -5) >= 1]
...
indel.data.low.eff.ess <- indel.data.low.eff[, colSums(indel.data.low.eff > 3) >= 1 |
                           colSums(indel.data.low.eff < -5) >= 1]
...
mean.ratio <- indel.data[indel.data$Drug == "DMSO",] %>% select(barcode, ratio) %>%
...
delay.data <- delay.df[delay.df$clone %in% "5",] %>%
  ...
  select(barcode, replication_timing) %>% unique()
...
## Scatterplot
# DNA methylation
dnameth.data <- as.data.frame(chip_data2) %>% rownames_to_column() %>% select(DNAse, WGBS)
...
# LMNB1
lmnb1.data <- as.data.frame(chip_data2) %>% rownames_to_column() %>% select(LMNB1)
...
mean.ratio <- indel.data[indel.data$Drug == "CTPB" & indel.data$conc == "10um",] %>%
  select(barcode, logratio.platenorm) %>% 
...
mean.ratio <- indel.data[indel.data$Drug == "AT9283" & indel.data$conc == "1um",] %>%
  select(barcode, rep, logratio.platenorm) %>%
...
hdac1.data <- chip_data %>% select(DNAse, WGBS)
...
mean.ratio <- indel.data[indel.data$Drug == "AR-42" | indel.data$Drug == "Vorinostat (SAHA, MK0683)" |
  ...
  select(barcode, Drug, logratio.platenorm) %>%
...
# Scatterplot
mean.ratio <- rownames_to_column(as.data.frame(mean.ratio2))
h3k27.data <- chip_data %>% select(H3K27me3)
...
mc1293.indel.data <- indel.data.med.drug %>% select(`MC-1293`) %>%
  rownames_to_column("barcode")
barcode.order2 <- barcode.order %>% select(-order)
...
chip_data2 <- chip_data %>% select(barcode, HDAC1) 
chip_data2 <- merge(chip_data2, barcode.order2) %>% select(-barcode)
...
indel.data.cor <- indel.data[indel.data$Drug %in% names.high |
  ...
  select(barcode, Drug, mean.logratio.bc.drug, conc) %>% 
  unique() 
