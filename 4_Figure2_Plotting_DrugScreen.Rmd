...
eff.data.heatmap <- eff.data.heatmap %>%
  ...
  filter(min < 0.05) %>%
  distinct(drug, target, conc_char, freqCut_norm_mean, IPR)
...
corr_dmso <- merge(eff.data.heatmap, dmso_freq) %>%
  ...
  mutate(mean_effect = ave(freqCut_norm_mean, drug, conc_char, FUN = mean)) %>%
...
for (i in unique(corr_heatmap$drug_conc)) {
  corr_heatmap$cor_dmso[corr_heatmap$drug_conc == i] <- 
    cor(corr_heatmap$freqCut_norm_mean[corr_heatmap$drug_conc == i],
...
pheatmap(pc1%>%
         ...
         cluster_cols = F,
         breaks = breaks,
         color = colorRampPalette(c("black", "white", colors_diverse[4]))(palette_length))
...
for (i in unique(corr_dmso$drug_conc)) {
    model.correlation <- lm(formula = freqCut_norm_mean ~ dmso_eff, 
...
for (i in unique(corr_dmso$drug_conc)) {
    model.correlation <- lm(formula = freqCut_norm_mean ~ H3K27me3.zscore, 
                               data = corr_dmso[corr_dmso$drug_conc == i,]) %>% 
      tidy()
...
corr_hits <- corr_dmso %>% filter(drug_conc %in% c("PCI-24781 (Abexinostat)_100 nM", "Fluoro-SAHA_1 µM")) 

corr_h3k27me3_hits <- corr_dmso %>% filter(drug %in% c("EPZ-6438"))

ggplot(corr_hits,
       aes(y = freqCut_norm_mean, x = dmso_eff, 
           #color = ifelse(sign == 1, "red", "black")
           )) +
  geom_point() +
  ...
  facet_wrap(~drug_conc, scales = "free_y")
