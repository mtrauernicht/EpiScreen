...
eff.data <- indel.data %>%
  ...
  filter(abs(zscore_TIF_global_comb_rep) >= 2.58 & 
           !(viab_mean < .60 & zscore_TIF_global_comb_rep < -2.54)) %>%  # we can choose to display all drugs that have significant global hits
...
plt1 = pheatmap(eff.data.matrix[ , clone5_map_ipr$labels] %>% as.matrix() %>% t(),
...
## 2B Ranking 
```{r 2B Top TIF boosting, fig.width = 8, fig.height = 8}
rank_dt = indel.data %>% 
...
```{r S2D PCI-24781 example, fig.width = 6, fig.height = 6}
chrom <- chromatin %>%
...
ggplot(bc_mut_tib_mean, 
  ...
  scale_fill_manual("legend", values = c(colors_drugs[2], "white")) + 
