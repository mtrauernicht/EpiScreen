...
mean_viab_table = indel.data %>% distinct(drug, conc_char, replicate, viab_mean, target ) %>%
  group_by(conc_char, drug, target) %>% summarise(mean_viab = mean(viab_mean))
...
rank_dt %>% 
  ...
  mutate(# n = ifelse(is.na(n), 1, 2),
    col = ifelse(!between(rank, 15, maxrank-20) & 
                   filter_zscore_TIF_global_comb_rep & 
...
## S2B Plot regulatory effects on different chromatin types

```{r S2B chromatin effect per cluster, fig.width = 10, fig.height = 6}
chromatin_ipr <- chromatin %>%
  rownames_to_column("IPR")
...
## S2C TIF decreasing compounds

```{r S2C lowered TIF, fig.width = 8, fig.height = 5}
...
```{r S2D Check replicates, fig.width= 6, fig.height=4, echo = FALSE, eval=FALSE}
