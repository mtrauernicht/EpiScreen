...
ggplot_custom(indel.data %>%
              ...
              aes(x = conc_char, y = mean_eff_zscore_global_comb, color = ifelse(Mod(mean_eff_zscore_global_comb) > 2.58, "red", "black"))) +
  geom_quasirandom() +
...
ggplot_custom(indel.data %>%
  ...
  geom_quasirandom(aes(color = ifelse(Mod(mean_eff_zscore_global_comb) > 2.58, "red", "black"))) +
  geom_hline(linetype = "dashed", yintercept = 2.58) +
  geom_hline(linetype = "dashed", yintercept = -2.58) +
...
eff.data$sign <- "Yes"
eff.data$sign[Mod(eff.data$mean_eff_zscore_global_comb) < 2.58] <- "No"
...
eff.data.heatmap <- indel.data %>%
  ...
  #filter(Mod(mean_eff_zscore_comb) >= 2.58) %>%  # we could also choose to display all drugs that have significant global hits
...
breaks <- quantile(unlist(eff.data.heatmap2 %>% dplyr::select(contains('IPR'))), c(.05,.95)) # change these numbers according to where you want to set the cutoff
palette_length <- 100
