...
indel.data <- readRDS("/DATA/projects/DSBrepair/data/R/rs20220118_episcreen/rs20220118_episcreen_ratios.RDS")
```
...
eff.data <- indel.data %>%
  mutate(well = gsub("E1.*([A-Z]{1}[0-9]{2}$)", "\\1", exp)) %>%
  filter(sample == "drug" | drug == "DMSO") %>%
                dplyr::distinct(drug, freqCut_norm, conc_char, sample, target,
                                tech, replicate, IPR, well) %>%
  mutate(freqCut_norm = ave(freqCut_norm, drug, conc_char, replicate, well, tech, IPR, FUN = mean)) %>%
  distinct() %>%
  mutate(conc_drug = paste(conc_char, drug, sep = "_"))

for (i in unique(eff.data$conc_drug)) {
  for (j in unique(eff.data$IPR)) {
    if (nrow(eff.data[eff.data$conc_drug == i & eff.data$IPR == j,]) >= 2) {
    eff.data$pval[eff.data$conc_drug == i & eff.data$IPR == j] <- 
      t.test(eff.data$freqCut_norm[eff.data$conc_drug == i & eff.data$IPR == j], 
                            eff.data$freqCut_norm[eff.data$drug == "DMSO" & eff.data$IPR == j])$p.value
    }
  }
}
eff.data$pval_adj <- p.adjust(eff.data$pval, method = "fdr")


ggplot_custom(eff.data %>%
                filter(sample == "drug") %>%
                mutate(freqCut_norm = ave(freqCut_norm, drug, conc_char, IPR, FUN = mean)) %>%
                dplyr::select(-tech, -replicate) %>%
                distinct(),
              aes(x = log2(freqCut_norm), y = -log10(pval_adj), color = ifelse(pval_adj < 0.05, "red", "black"))) +
  geom_point() +
...
eff.data.2 <- eff.data %>%
  filter(sample == "drug", conc_char == "100 nM", IPR == "IPR14") %>%
  dplyr::select(freqCut_norm, pval, sample, drug, conc_char, target) %>%
  mutate(freqCut_norm = ave(freqCut_norm, drug, conc_char, FUN = mean)) %>%
  unique() %>%
  mutate(mean_eff_target = ave(freqCut_norm, target, FUN = median),
         pval = -log10(pval),
         freqCut_norm = log2(freqCut_norm))

ggplot_custom(data = eff.data.2, 
              aes(y = freqCut_norm, x = reorder(target, -mean_eff_target, FUN = median))) + 
