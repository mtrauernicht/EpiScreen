...
eff.data.heatmap2.clusters <- eff.data.heatmap2 %>% 
  as_tibble() %>%
...
target_set_all <- target_set_drugs %>%
  ...
  na.omit() %>%
  filter(drug_conc %in% eff.data.heatmap2.clusters$drug_conc)
...
## S2B-C Zoom on rank
```{r S2BC Zoom on rank, fig.width = 8, fig.height = 4}
# Boosting effect
p1 = rank_dt %>% 
  filter(rank <= 75) %>%
  # left_join(near_all_TIF) %>%
