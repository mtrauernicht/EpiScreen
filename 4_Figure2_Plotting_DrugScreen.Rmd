...
filter(rank > (maxrank-20) & 
  ...
  !(viab_mean < .50 & zscore_TIF_global_comb_rep < -2.54)) %>%
distinct(drug, conc_char, zscore_TIF_global_comb_rep , target)### Prepare the annotations
...
```{r S2 chromatin effect per cluster all}
...
rank_dt %>% 
  mutate(col = ifelse(!between(rank, 15, maxrank-20) & 
                        filter_zscore_TIF_global_comb_rep & 
                        !(viab_mean < .60 & zscore_TIF_global_comb_rep < -2.54),
                      target, "black")) %>%
  # filter(rank < 100 ) %>%
  ggplot(., aes(rank, zscore_TIF_global_comb_rep, color = col)) +
  geom_point()  +
...
breaks <- quantile(unlist(tophit_matrix %>% 
                            dplyr::select(contains('IPR'))), c(.01,.99), na.rm = T) # change these numbers according to where you want to set the cutoff
palette_length <- 100
