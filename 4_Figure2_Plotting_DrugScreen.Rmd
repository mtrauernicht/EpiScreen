...
eff.data <- indel.data %>%
  filter(sample == "drug" | drug == "DMSO", replicate == "E1504") %>%
                dplyr::distinct(drug, freqCut_norm_split, conc_char, sample, target,
                                tech, replicate) %>%
  mutate(freqCut_norm_split = ave(freqCut_norm_split, drug, conc_char, replicate, tech, FUN = mean),
         freqCut_norm_mean_drug = ave(freqCut_norm_split, drug, conc_char, replicate, FUN = mean)) %>%
  unique()
...
eff.data.2 <- eff.data %>%
                filter(sample == "drug", conc_char == "100 nM") %>%
                  ...
                  unique() %>%
                mutate(mean_eff_target = ave(freqCut_norm_mean_drug, target, FUN = median),
                       pval = -log10(pval),
                       freqCut_norm_mean_drug = log2(freqCut_norm_mean_drug))

ggplot_custom(data = eff.data.2, 
              aes(y = freqCut_norm_mean_drug, x = reorder(target, -mean_eff_target, FUN = median))) + 
    geom_boxplot(position = position_dodge(0.75), alpha = 0.4, outlier.shape = NA) +
  geom_quasirandom(aes(color = ifelse(pval > 1.3, "red", "black"))) +
...
# C: Bar plot
eff.data.2 <- eff.data %>%
  filter(sample != "DNA-PKi (NHEJ control)", sample != "Mirin (MMEJ control)", sample != "DMSO (neg control)", conc_char == "100 nM", replicate == "E1504") %>%
  distinct(drug, conc_char, target, pval) 

eff.data.2$sign <- "Yes"
eff.data.2$sign[eff.data.2$pval >= 0.05] <- "No"

eff.data.2 <- eff.data.2 %>%
  mutate(number = ave(target, target, conc_char, sign, FUN = length)) %>%
...
eff.data.heatmap <- indel.data %>%
  filter(sample == "drug" | drug == "DMSO", replicate == "E1504") %>%
  distinct(conc_char, drug, IPR, freqCut_norm_mean, freqCut_norm_split)


eff.data.heatmap2 <- eff.data.heatmap %>%
  dplyr::select(-freqCut_norm_mean) %>%
  mutate(freqCut_norm_split = ave(freqCut_norm_split, conc_char, drug, IPR, FUN = mean)) %>%
  unique() %>%
  mutate(freqCut_norm_split = log2(freqCut_norm_split)) %>%
  mutate(mod = Mod(freqCut_norm_split)) %>%
  mutate(max = ave(mod, drug, conc_char, FUN = max)) %>%
  dplyr::select(-mod) %>%
  spread(IPR, freqCut_norm_split) %>%
  group_by(conc_char) %>%
  top_n(15, max) %>%
  ungroup() %>%
  dplyr::select(-max)
...
# Alternative: only HDACs:
eff.data.heatmap <- indel.data %>%
  filter(target == "HDAC" & replicate == "E1504") %>%
  distinct(conc_char, drug, IPR, freqCut_norm_mean)%>%
  mutate(freqCut_norm_mean = log2(freqCut_norm_mean)) %>%
  mutate(mod = Mod(freqCut_norm_mean)) %>%
  mutate(max = ave(mod, drug, conc_char, FUN = max)) %>%
  filter(max > 0.15 | max < -0.15) %>%
  dplyr::select(-max, -mod) %>%
  spread(IPR, freqCut_norm_mean) %>%
  mutate(drug = paste(drug, conc_char, sep = ""))


pheatmap(as.matrix(eff.data.heatmap %>%
                       column_to_rownames('drug') %>%
                       dplyr::select(contains('IPR')) %>%
                       t()),
           border_color = "white",
