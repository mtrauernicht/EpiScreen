...
eff.data <- indel.data %>%
  ...
  mutate(freqCut_norm_split = ave(freqCut_norm_split, drug, conc_char, replicate, tech, FUN = mean),
         freqCut_norm_mean_drug = ave(freqCut_norm_split, drug, conc_char, FUN = mean)) %>%
  unique()
...
eff.data.heatmap <- indel.data %>%
  filter(sample == "drug" | drug == "DMSO", replicate == "E177") %>%
...
eff.data.heatmap <- indel.data %>%
  ...
  distinct(conc_char, drug, IPR, freqCut_norm)%>%
  mutate(freqCut_norm_mean = ave(freqCut_norm, drug, conc_char, IPR, FUN = mean)) %>%
  mutate(freqCut_norm_mean = log2(freqCut_norm_mean)) %>%
  mutate(mod = Mod(freqCut_norm_mean)) %>%
  mutate(max = ave(mod, drug, conc_char, FUN = max)) %>%
  filter(max > 0.15 | max < -0.15) %>%
  dplyr::select(-max, -mod, -freqCut_norm) %>%
