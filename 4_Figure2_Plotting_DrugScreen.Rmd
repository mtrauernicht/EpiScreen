...
eff.data.heatmap <- indel.data %>%
  ...
  distinct(conc_char, drug, target, IPR, mean_eff_zscore_comb, freqCut)
...
eff.data.heatmap <- merge(eff.data.heatmap, dmso_eff) %>%
  mutate(freqCut_dif = freqCut - dmso_freq) %>%
  mutate(freqCut_dif = ave(freqCut_dif, drug, conc_char, IPR, FUN = mean))
...
p1<- pheatmap(eff.data.heatmap2[eff.data.heatmap2$conc_char == "100 nM",] %>%
                       arrange(target) %>%
           ...
           angle_col = 90,
           na_col = "white",
           main = paste('efficiency change, concentration =', "100 nM"),
...
p2 <- pheatmap(eff.data.heatmap2[eff.data.heatmap2$conc_char == "1 µM",] %>%
                       arrange(target) %>%
                       column_to_rownames('drug') %>%
                       dplyr::select(contains('IPR')) %>%
                       t(),
           ...
           angle_col = 90,
           na_col = "white",
           main = paste('efficiency change, concentration =', "1 µM"),
           annotation_col = target,
           annotation_row = chromatin,
           color = colorRampPalette(c("#e07a5f", "#e5e5e5", "#3d405b"))(palette_length),
           annotation_colors = annotation_colors_scr, 
...
p3 <- pheatmap(eff.data.heatmap2[eff.data.heatmap2$conc_char == "10 µM",] %>%
                       arrange(target) %>%
                       column_to_rownames('drug') %>%
                       dplyr::select(contains('IPR')) %>%
                       t(),
           ...
           angle_col = 90,
           na_col = "white",
           main = paste('efficiency change, concentration =', "10 µM"),
           annotation_col = target,
           annotation_row = chromatin,
           color = colorRampPalette(c("#e07a5f", "#e5e5e5", "#3d405b"))(palette_length),
           annotation_colors = annotation_colors_scr, 
...
for (i in unique(corr_heatmap$drug_conc)) {
  ...
  corr_heatmap$cor_h3k27me3[corr_heatmap$drug_conc == i] <- 
    cor(corr_heatmap$freqCut_dif[corr_heatmap$drug_conc == i],
        corr_heatmap$H3K27me3.zscore[corr_heatmap$drug_conc == i])
...
for (i in unique(corr_dmso$drug_conc)) {
    model.correlation <- lm(formula = freqCut_dif ~ H3K27me3.zscore, 
                               data = corr_dmso[corr_dmso$drug_conc == i,]) %>% 
      tidy()
...
corr_hits <- corr_dmso %>% filter(drug_conc %in% c("PCI-24781 (Abexinostat)_100 nM", "Fluoro-SAHA_1 µM", "Belinostat (PXD101)_100 nM", "Droxinostat_10 µM")) 

corr_h3k27me3_hits <- corr_dmso %>% filter(drug %in% c("EPZ-6438"))

corr_hits <- merge(corr_hits, chromatin_2)
...
ggplot(corr_hits %>%
  ...
  geom_point(aes(y = freqCut*100, x = dmso_eff*100, color = chromatin)) +
  #scale_color_manual(values = c("#e07a5f", "#3d405b")) +
...
corr_hits <- corr_hits %>%
  mutate(drug_eff = dmso_eff + freqCut_dif) %>%
  distinct(drug_eff, drug_conc, IPR)

dmso_freq2 <- dmso_freq %>%
  mutate(drug_conc = "DMSO") %>%
  dplyr::select("drug_eff" = dmso_eff, drug_conc, IPR)

corr_hits <- rbind(corr_hits, dmso_freq2)

ggplot_custom(corr_hits,
       aes(x = drug_eff, y = IPR, fill = drug_conc)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_x_continuous(labels = scales::percent) +
  geom_vline(xintercept = max(dmso_freq$dmso_eff))


ggplot(corr_h3k27me3_hits,
       aes(y = freqCut_norm_mean, x = H3K27me3.zscore, color = ifelse(sign == 1, "red", "black"))) +
  geom_point() +
...
# Can the different HDAC inhibitor phenotypes be explained by which HDAC they target?
hdac_class <- chrom_corr %>%
  filter(target == "HDAC") %>%
  distinct(drug, global, conc_char)
```
