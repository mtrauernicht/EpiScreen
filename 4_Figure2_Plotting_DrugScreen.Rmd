...
ggplot_custom(indel.data %>%
              ...
              aes(x = conc_char, y = mean_eff_zscore_global_comb, color = ifelse(Mod(mean_eff_zscore_global_comb) > 1.96, "red", "black"))) +
  geom_quasirandom() +
...
ggplot_custom(indel.data %>%
  ...
  geom_quasirandom(aes(color = ifelse(Mod(mean_eff_zscore_global_comb) > 1.96, "red", "black"))) +
  geom_hline(linetype = "dashed", yintercept = 1.96) +
  geom_hline(linetype = "dashed", yintercept = -1.96) +
...
eff.data$sign <- "Yes"
eff.data$sign[Mod(eff.data$mean_eff_zscore_global_comb) < 1.96] <- "No"
...
myBreaks <- c(seq(breaks[1], -2, length.out=ceiling(palette_length/2) + 1), 
              seq(2, breaks[2], length.out=floor(palette_length/2)))
