...
for (i in unique(corr_dmso$drug_conc)) {
  corr_dmso$corr[corr_dmso$drug_conc == i] <- cor(corr_dmso$freqCut_norm_mean[corr_dmso$drug_conc == i], 
                                                  corr_dmso$dmso_eff[corr_dmso$drug_conc == i])
}
...
ggplot(corr_dmso,
  ...
  geom_point() +
  facet_wrap(target~sign)

ggplot(corr_dmso %>%
         filter(abs(mean_effect) > 0.02) %>%
         #filter(sign == 1) %>%
         distinct(corr, drug_conc, sign, target, conc_char),
       aes(y = corr, x = target, color = as.character(sign))) +
  geom_quasirandom() +
  coord_flip() +
...
ggplot(corr_dmso %>% filter(drug_conc %in% c("Givinostat (ITF2357)_100 nM", "Fluoro-SAHA_1 µM")),
              aes(x = freqCut_norm_mean, y = mean_eff)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
  stat_cor(method = "pearson") +
  facet_wrap(~drug_conc, scales = "free_x")
