...
eff.data <- indel.data %>%
  ...
  distinct(conc_char, drug, target, IPR, zscore_TIF_comb_rep, freqCut, replicate, tech, filter_zscore_TIF_comb_rep)
...
## 2B: Cutting efficiency change per drug
...
eff.data <- indel.data.freq %>%
  ...
  mutate(mean_eff_pval_global_comb = 2*pnorm(q = abs(zscore_TIF_global_comb_rep ), lower.tail = F),
         mean_eff_pval_adj_global_comb = p.adjust(mean_eff_pval_global_comb, method = 'fdr'),
         sign_global = ifelse((mean_eff_pval_adj_global_comb < 0.01 & 
                                 filter_zscore_TIF_global_comb_rep &
                                 zscore_TIF_global_comb_rep > 2.58), "TIF_Increase", 
                              ifelse((mean_eff_pval_adj_global_comb < 0.01 &
                                        filter_zscore_TIF_global_comb_rep &
                                 zscore_TIF_global_comb_rep < -2.58), "TIF_Decrease", 
                                     "No_Change")),
         mean_eff_pval_comb = 2*pnorm(q = abs(zscore_TIF_comb_rep ), lower.tail = F),
...
local.effects = eff.data %>% 
  ...
  pivot_longer(cols = No_Change:Local_TIF_Decrease, values_to = "n", names_to = "sign_global")
...
PJ34_df %>% 
  pivot_wider(names_from = replicate, values_from = c(freqCut_diff, viab_mean, freqCut)) %>%
  ggplot_custom(.,
                aes(x = freqCut_diff_E1504*100, y = freqCut_diff_E177*100)) +
  geom_point() +
  geom_abline(lty = "twodash") +
  geom_smooth(method = "lm", color = "red", lty = "dashed") +
  stat_cor(method = "pearson") +
  xlab("TIF difference to Control E1504") +
  ylab("TIF difference to Contro E177")  +
  facet_grid(. ~ conc_char)
