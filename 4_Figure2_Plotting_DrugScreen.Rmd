...
ggplot(corr_dmso %>%
       ...
       aes(y = -log10(p.value_dmso), x = target, color = as.character(sign), 
...
corr_hits <- corr_dmso %>% filter(drug_conc %in% c("PCI-24781 (Abexinostat)_100 nM", "Fluoro-SAHA_1 µM",
                                             "B2_10 µM", "Aurora A Inhibitor I_100 nM")) 

corr_h3k27me3_hits <- corr_dmso %>% filter(drug %in% c("EPZ-6438"))

ggplot(corr_hits,
       aes(y = freqCut_norm_mean, x = dmso_eff, color = ifelse(sign == 1, "red", "black"))) +
  geom_point() +
...
drugs_sel <- corr_dmso %>%
  filter(abs(mean_effect) > 0.025) %>%
  filter(sign == 1) %>%
  distinct(drug_conc, p.value_dmso, p.value_h3k27me3, slope_h3k27me3)
...
ggplot(chrom_corr %>%
         distinct(drug_conc, p.value_dmso, slope_h3k27me3, target, global),
       aes(x = global, y = slope_h3k27me3)) +
