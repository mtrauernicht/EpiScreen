...
eff.data <- indel.data.freq %>%
  ...
  mutate(freqCut_diff_global_mean = ave(freqCut_diff_global_mean, 
                                        drug, conc_char, FUN = function(x) mean(x, na.rm = TRUE))) %>%
  distinct(mean_eff_zscore_global_comb, drug, target, conc_char, freqCut_diff_global_mean) %>%
  mutate(mean_eff_pval_global_comb = 2*pnorm(q = abs(mean_eff_zscore_global_comb), lower.tail = F)) %>%
  mutate(mean_eff_pval_adj_global_comb = p.adjust(mean_eff_pval_global_comb, method = 'fdr'),
         sign = ifelse((freqCut_diff_global_mean > 0.02 & mean_eff_pval_adj_global_comb < 0.05), "TIF_Increase", 
                       ifelse((freqCut_diff_global_mean < -0.02 & mean_eff_pval_adj_global_comb < 0.05), "TIF_Decrease", 
                       "No_Change")),
...
eff.data %>% filter(sign != "No_Change") %>% distinct(target, conc_char, number) %>% group_by(conc_char) %>% summarise(count = sum(number))
```
...
```{r 2C Top TIF boosting, }
indel.data %>% 
    filter(conc_char != "control" & indel_freq_filter == T & viab_reproducibility == TRUE) %>%
    distinct(drug, conc_char, mean_eff_zscore_global_comb, target) %>% 
    arrange(desc(mean_eff_zscore_global_comb)) %>%
    mutate(rank = seq(1:n())) %>%
  filter(rank < 100) %>%
  ggplot(., aes(rank, mean_eff_zscore_global_comb, color = target)) +
  geom_point()  +
  scale_color_manual(values = colors_targets) +
  gghighlight(rank <= 10, label_key = drug) +
...
top_hits = indel.data  %>% 
    filter(conc_char != "control" & indel_freq_filter == T & viab_reproducibility == TRUE) %>% 
  distinct(drug, conc_char, mean_eff_zscore_global_comb, target) %>%
    arrange(desc(mean_eff_zscore_global_comb)) %>% slice_head(n = 10) %>% distinct(drug, conc_char, target, mean_eff_zscore_global_comb) %>% 
  mutate(rank = seq(1:n()),
         drug = gsub(" [(].*[)]", "", drug),
...
```{r 2I AurA vs AurB inhibitors, fig.width= 3, fig.height= 3}
