...
rank_dt %>% 
  ...
  mutate(# n = ifelse(is.na(n), 1, 2),
    col = ifelse(!between(rank, 8, maxrank-17) & 
...
## 2E Indel spectra
```{r 2E Indel Plots top hits, eval = FALSE}
label_ratio <- filter(indel.data, drug %in% c("DMSO", "PCI-24781 (Abexinostat)") & conc_char %in% c("control", "100 nM")) %>% 
  group_by(IPR, drug) %>%
...
bc_mut_tib_mean <- mutation.data %>% 
  distinct(IPR, drug, freq_mean,freq_sd, color, mutation, type, conc_char) %>%
  filter(drug %in% c("DMSO", "PCI-24781 (Abexinostat)"), type %in% c("core", "grouped_indel") & conc_char %in% c("control", "100 nM")) %>% 
  left_join(label_ratio)
...
p2 = ggplot(bc_mut_tib_mean, 
            aes(mutation, freq_mean)) + 
  geom_quasirandom(stat = "identity", aes(fill = color)) + 
  # geom_quasirandom(data = bc_mut_tib, aes(mutation, freq_split, fill = color),  color = "black", shape = 21, ) + 
  scale_fill_manual("legend", values = colors_pathways) + 
  scale_color_manual("legend", values = colors_pathways) + 
  # scale_y_continuous(name="frequency", labels = percent) +
...
label_ratio <- indel.data %>%
  mutate(drug_conc = paste(drug, conc_char, sep = " ")) %>%
  filter(drug_conc %in% c("DMSO control", "PCI-24781 100 nM")) %>% 
...
bc_mut_tib_mean <- mutation.data %>% 
  mutate(drug = gsub(" [(].*[)]", "", drug),
         drug_conc = paste(drug, conc_char, sep = " ")) %>%
  distinct(IPR, drug_conc, freq_mean,freq_sd, color, mutation, type, replicate) %>%
  filter(drug_conc %in% c("DMSO control", "PCI-24781 100 nM"), type %in% c("core", "grouped_indel")) %>% 
...
ggplot(bc_mut_tib_mean, 
  ...
  scale_color_manual("legend", values = colors_pathways[1:4]) + 
  #scale_y_continuous(name="frequency", labels = percent) +
  scale_x_discrete(name="indel size", breaks = c("<-14", -7, 0, 1, ">2")) + 
...
## S2EFG TIF decreasing compounds

```{r S2EFG lowered TIF, fig.width = 9, fig.height = 6}
...
## S2H Plot regulatory effects on different chromatin types

```{r S2H chromatin effect per cluster, fig.width = 10, fig.height = 6}
chromatin_ipr <- chromatin %>%
  rownames_to_column("IPR")
