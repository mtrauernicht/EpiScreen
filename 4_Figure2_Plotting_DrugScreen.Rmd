...
indel.data <- readRDS("/DATA/projects/DSBrepair/data/R/rs20220211_episcreen/rs20220211_episcreen_ratios.RDS")
```
...
target_set <- indel.data %>% 
  ...
  distinct(drug, drug_conc, conc_char, target) %>% 
...
clusters_drugs <- cutree(tree = as.dendrogram(clusters_drugs), k = 3)
...
pdf(paste0(figure_out, "rs20220214_indelfrequency_heatmap_allconc.pdf"), width=16, height=8)
...
pdf(paste0(figure_out, "rs20220214_indelfrequency_heatmap_DMSO.pdf"), width=3, height=8)
...
cluster1 = target %>% filter(clusters_drugs == 1) %>% rownames()
cluster1 = cluster1[!grepl("Nullscript", cluster1)]


eff.data.heatmap.cluster3 = eff.data.heatmap2 %>% 
  mutate(drug_conc = paste(drug, conc_char, sep = " ")) %>% 
  filter(drug_conc %in% cluster1)


breaks <- quantile(unlist(eff.data.heatmap.cluster3 %>% 
...
# Add cluster annotation
clusters_drugs <- hclust(dist(eff.data.heatmap.cluster3), method = "complete")
clusters_drugs <- cutree(tree = as.dendrogram(clusters_drugs), k = 4)
clusters_drugs <- data.frame(clusters_drugs)

eff.data.heatmap.cluster3 <- cbind(eff.data.heatmap.cluster3, clusters_drugs) %>% 
  as_tibble()

clusters_drugs <- eff.data.heatmap.cluster3 %>%
  distinct(drug_conc, clusters_drugs)
target.cluster3 <- target_set %>%
  rownames_to_column("drug_conc") %>%
  left_join(clusters_drugs) %>%
  na.omit()
rownames(target.cluster3) <- target.cluster3$drug_conc
target.cluster3 <- target.cluster3 %>%
  dplyr::select(target, clusters_drugs, conc_char)


pdf(paste0(figure_out, "rs20220214_indelfrequency_heatmap_cluster3.pdf"), width=16, height=8)
pheatmap(eff.data.heatmap.cluster3 %>%
                       arrange(target.cluster3) %>%
                       column_to_rownames('drug_conc') %>%
                       dplyr::select(contains('IPR')) %>%
                       t(),
           border_color = F,
           ...
           na_col = "white",
           main = 'efficiency change, all concentrations',
           annotation_col = target.cluster3,
           annotation_row = chromatin,
           color = colorRampPalette(c("#e07a5f", "#e5e5e5", "#3d405b"))(palette_length),
           annotation_colors = annotation_colors_scr, 
           annotation_legend = T)
...
pdf(paste0(figure_out, "rs20220214_indelfrequency_3clusters_chromatin.pdf"), width=8, height=4)
...
```{r}
drug_clusters <- eff.data.heatmap2 %>%
  mutate(drug_conc = paste(drug, conc_char, sep = "_")) %>%
  distinct(drug_conc, clusters_drugs)
...
ggplot(epistasis_cluster %>%
         filter(clusters_drugs %in% c('4', '5','7')) %>%
         filter(feature %in% c("LMNB1.zscore", "")),
...
epistasis_cluster <- epistasis_cluster[epistasis_cluster$drug_conc %in% drug_clusters$drug_conc,]
...
pheatmap(epistasis_cluster %>%
         ...
         border_color = NA,
         color = colorRampPalette(c("blue", "white", "red"))(palette_length))
