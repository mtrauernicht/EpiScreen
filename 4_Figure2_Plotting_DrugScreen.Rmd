...
ggplot_custom(indel.data %>%
              ...
              aes(x = conc_char, y = eff_zscore_pooled_global_comb, color = ifelse(Mod(eff_zscore_pooled_global_comb) > 1.96, "red", "black"))) +
  geom_quasirandom() +
...
ggplot_custom(indel.data %>%
  ...
  geom_quasirandom(aes(color = ifelse(Mod(eff_zscore_pooled_global_comb) > 1.96, "red", "black"))) +
  geom_hline(linetype = "dashed", yintercept = 1.96) +
  geom_hline(linetype = "dashed", yintercept = -1.96) +
...
eff.data <- indel.data %>%
  filter(sample == "drug") %>%
  distinct(eff_zscore_pooled_global_comb, drug, target, conc_char)

eff.data$sign <- "Yes"
eff.data$sign[Mod(eff.data$eff_zscore_pooled_global_comb) < 1.96] <- "No"
