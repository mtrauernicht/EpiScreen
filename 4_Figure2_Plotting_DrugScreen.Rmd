...
top_hits = indel.data  %>% 
  ...
  arrange(desc(zscore_TIF_global_comb_rep )) %>% eff.data.heatmap.top
  slice_head(n = 8) %>% 
...
```
## 2C Plot postive regulator effects on different chromatin types (clusters 3, 4 and 5)
```{r S2 chromatin effect per cluster, fig.width = 10, fig.height = 6}
chromatin_ipr <- chromatin %>%
  rownames_to_column("IPR")
...
## S2C
```{r S2D lowered TIF, fig.width = 8, fig.height = 5}
...
```{r S2D PJ34 replicates, fig.width= 6, fig.height=4, echo = FALSE, eval=FALSE}
