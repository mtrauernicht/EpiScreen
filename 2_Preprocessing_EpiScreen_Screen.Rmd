...
```{r outdir screen}
## Select outdir
out.dir = list.dirs(path = "/DATA/projects/DSBrepair/data/R") %>% .[grepl("episcreen_master", .)] %>% tail(n = 1)
in.dir = out.dir
...
df.list = mclapply(df.list, CallTrueBarcodes, mc.cores = 20)
...
mean.TIF.zscore.comb = TIF.zscore %>% distinct(drug, conc_char, barcode, replicate, zscore_TIF) %>%
  ...
  mutate(zscore_TIF_comb_tech = ave(zscore_TIF, drug, conc_char, barcode, replicate,
         ...
         sum_tech = ave(binary_zscore, drug, conc_char, barcode, replicate,
                        FUN = sum),
         filter_zscore_TIF_comb_tech = ifelse(sum_tech >= 2, TRUE, FALSE), 
         zscore_TIF_comb_rep = ave(zscore_TIF, drug, conc_char, barcode,
                                   FUN = function(x) sum(x)/sqrt(length(x))),
         # Make filter for replicate, here both tech filters need to be true.
         sum_rep = ave(as.integer(filter_zscore_TIF_comb_tech), drug, conc_char, barcode,
                             FUN = sum),
         filter_zscore_TIF_comb_rep = ifelse(sum_rep >= 4, TRUE, FALSE),         
         sum_sign = ave(zscore_TIF_comb_tech, drug, conc_char, barcode,
                             FUN = function(x) length(table(sign(x)))),
         filter_zscore_TIF_comb_rep = ifelse(sum_sign == 1, filter_zscore_TIF_comb_rep, FALSE)) %>%
  dplyr::select(-sum_tech, -sum_rep, -binary_zscore, -sum_sign)
...
# Combine the z-scores of both screens
TIF.zscore.global.comb = TIF.zscore.global %>% distinct(drug, conc_char, replicate, zscore_TIF_global) %>%
  filter(!is.na(zscore_TIF_global)) %>%
 mutate(zscore_TIF_global_comb_tech = ave(zscore_TIF_global, drug, conc_char, replicate,
                                    FUN = function(x) sum(x)/sqrt(length(x))),
         # Filter the z-scores for at least 2 significant hits per screen. 
         binary_zscore = ifelse(abs(zscore_TIF_global) >= 1.96, 1, 0),
         sum_tech = ave(binary_zscore, drug, conc_char, replicate,
                        FUN = sum),
         filter_zscore_TIF_global_comb_tech = ifelse(sum_tech >= 2, TRUE, FALSE), 
         zscore_TIF_global_comb_rep = ave(zscore_TIF_global, drug, conc_char, 
                                   FUN = function(x) sum(x)/sqrt(length(x))),
         # Make filter for replicate, here both tech filters need to be true.
         sum_rep = ave(as.integer(filter_zscore_TIF_global_comb_tech), drug, conc_char, 
                       FUN = sum),
         filter_zscore_TIF_global_comb_rep = ifelse(sum_rep >= 4, TRUE, FALSE),         
         sum_sign = ave(zscore_TIF_global_comb_tech, drug, conc_char,
                             FUN = function(x) length(table(sign(x)))),
         filter_zscore_TIF_global_comb_rep = ifelse(sum_sign == 1, filter_zscore_TIF_global_comb_rep, FALSE)) %>%
  dplyr::select(-sum_tech, -sum_rep, -binary_zscore, -sum_sign)
...
# Remove -Inf
bal.zscore = bal.zscore %>%
  group_by(drug, conc_char, barcode) %>%
  mutate(min_MMEJ_FC = min(MMEJ_log2_FC[is.finite(MMEJ_log2_FC)])) %>%
  mutate(MMEJ_log2_FC = ifelse(!is.finite(MMEJ_log2_FC), min_MMEJ_FC, MMEJ_log2_FC)) %>%
  ungroup() %>%
  dplyr::select(-min_MMEJ_FC)
...
mean.bal.zscore.comb = bal.zscore %>% distinct(drug, conc_char, barcode, replicate, zscore_bal) %>%
  ...
  mutate(zscore_bal_comb_tech = ave(zscore_bal, drug, conc_char, barcode, replicate,
         ...
         sum_tech = ave(binary_zscore, drug, conc_char, barcode, replicate,
                        FUN = sum),
         filter_zscore_bal_comb_tech = ifelse(sum_tech >= 2, TRUE, FALSE), 
         zscore_bal_comb_rep = ave(zscore_bal, drug, conc_char, barcode,
                                   FUN = function(x) sum(x)/sqrt(length(x))),
         # Make filter for replicate, here both tech filters need to be true.
         sum_rep = ave(as.integer(filter_zscore_bal_comb_tech), drug, conc_char, barcode,
                             FUN = sum),
         filter_zscore_bal_comb_rep = ifelse(sum_rep >= 4, TRUE, FALSE),         
         sum_sign = ave(zscore_bal_comb_tech, drug, conc_char, barcode,
                             FUN = function(x) length(table(sign(x)))),
...
# Combine the z-scores of both screens
bal.zscore.global.comb = bal.zscore.global %>% distinct(drug, conc_char, replicate, zscore_bal_global) %>%
  filter(!is.na(zscore_bal_global)) %>%
  mutate(zscore_bal_global_comb_tech = ave(zscore_bal_global, drug, conc_char, replicate,
                                    FUN = function(x) sum(x)/sqrt(length(x))),
         # Filter the z-scores for at least 2 significant hits per screen. 
         binary_zscore = ifelse(abs(zscore_bal_global) >= 1.96, 1, 0),
         sum_tech = ave(binary_zscore, drug, conc_char, replicate,
                        FUN = sum),
         filter_zscore_bal_global_comb_tech = ifelse(sum_tech >= 2, TRUE, FALSE), 
         zscore_bal_global_comb_rep = ave(zscore_bal_global, drug, conc_char, 
                                   FUN = function(x) sum(x)/sqrt(length(x))),
         # Make filter for replicate, here both tech filters need to be true.
         sum_rep = ave(as.integer(filter_zscore_bal_global_comb_tech), drug, conc_char, 
                             FUN = sum),
         filter_zscore_bal_global_comb_rep = ifelse(sum_rep >= 4, TRUE, FALSE),         
         sum_sign = ave(zscore_bal_global_comb_tech, drug, conc_char,
                             FUN = function(x) length(table(sign(x)))),
         filter_zscore_bal_global_comb_rep = ifelse(sum_sign == 1, filter_zscore_bal_global_comb_rep, FALSE)) %>%
  dplyr::select(-sum_tech, -sum_rep, -binary_zscore, -sum_sign)
