...
```{r merge metadata and filtering}

screen_indels_tib %>%
  # Join the reads per bc
  left_join(sum_bc_reads, by = c("barcode", "ID")) %>%
  # Join the summed tech reads
  left_join(count_rep, by = c("barcode", "ID", "mutation")) %>%
  filter(sum_bc_reads > 200) # Filter for samples with at least 200 total reads. 
```
...
indels_tib <- screen_indels_tib %>%
  dplyr::select(exp, barcode, mutation, count) %>% 
  pivot_wider(names_from = mutation, 
              values_from = count, 
              values_fill = 0)
  mutate(
    freqMMEJ = X.7 / all.indels,
