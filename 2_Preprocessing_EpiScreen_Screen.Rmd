...
mutations.large.del.dt = mutations.large.del.dt[, lapply(.SD, sum, na.rm=TRUE), by=c("exp", "barcode"),
                                                .SDcols=c("count_rep", "pooled_counts", "count",  
                                                          "norm_split", "freq_split", "sum_bc_tech_reads",
...
base.dt = copy(screen.indel.dt)
base.dt[, (colslargedels) := NULL ]
base.dt = unique(base.dt)

mutations.large.del.dt = base.dt %>% left_join(mutations.large.del.dt)
...
mutations.large.ins.dt = base.dt %>% left_join(mutations.large.ins.dt)
