...
mean.TIF.zscore.comb = TIF.zscore %>% distinct(drug, conc_char, barcode, replicate, zscore_TIF) %>%
  ...
  mutate(zscore_TIF_comb_tech = ave(zscore_TIF, drug, conc_char, barcode, replicate,
         ...
         filter_zscore_comb_tech = ifelse(sum_sign == 1, TRUE, FALSE),
         zscore_TIF_comb_rep = ave(zscore_TIF, drug, conc_char, barcode,
                                    FUN = function(x) sum(x)/sqrt(length(x))),
         sum_sign_comb = ave(zscore_TIF, drug, conc_char, barcode,
                                    FUN = function(x) length(table(sign(x)))),
         filter_zscore_comb_rep = ifelse(sum_sign_comb == 1, TRUE, FALSE),
         zscore_TIF_comb_tech = ifelse(filter_zscore_comb_tech, zscore_TIF_comb_tech, NA),
         zscore_TIF_comb_rep = ifelse(filter_zscore_comb_rep, zscore_TIF_comb_rep, NA)) %>%
...
TIF.zscore.global.comb = TIF.zscore %>% distinct(drug, conc_char, barcode, replicate, zscore_TIF_global) %>%
  ...
  mutate(zscore_TIF_global_comb_tech = ave(zscore_TIF_global, drug, conc_char, barcode, replicate,
         ...
         sum_sign = ave(zscore_TIF_global, drug, conc_char, barcode, replicate,
                                    FUN = function(x) length(table(sign(x)))),
         filter_global_zscore_comb_tech = ifelse(sum_sign == 1, TRUE, FALSE),
         ...
         sum_sign_comb = ave(zscore_TIF_global, drug, conc_char, barcode,
                                    FUN = function(x) length(table(sign(x)))),
         filter_global_zscore_comb_rep = ifelse(sum_sign_comb == 1, TRUE, FALSE),
         zscore_TIF_global_comb_tech = ifelse(filter_global_zscore_comb_tech, zscore_TIF_global_comb_tech, NA),
         zscore_TIF_global_comb_rep = ifelse(filter_global_zscore_comb_rep, zscore_TIF_global_comb_rep, NA))
...
TIF.zscore %<>% left_join(TIF.zscore.global) %>% 
  dplyr::select(barcode, replicate, plate, tech, well, drug, conc_char, 
                zscore_TIF, zscore_TIF_comb_tech, zscore_TIF_comb_rep,
                zscore_TIF_global, zscore_TIF_global_comb_tech, zscore_TIF_global_comb_rep)
```
...
bal.zscore = bal_zscore_calc %>%
  # And calculate the z-scores
  mutate(MMEJratio_norm = MMEJratio / mean_DMSO,
...
mean.bal.zscore.comb = bal.zscore %>% distinct(drug, conc_char, barcode, replicate, zscore_bal) %>%
  ...
  mutate(zscore_bal_comb_tech = ave(zscore_bal, drug, conc_char, barcode, replicate,
         ...
         sum_sign_comb = ave(zscore_bal, drug, conc_char, barcode,
                                    FUN = function(x) length(table(sign(x)))),
         filter_zscore_comb_rep = ifelse(sum_sign_comb == 1, TRUE, FALSE),
         zscore_bal_comb_tech = ifelse(filter_zscore_comb_tech, zscore_bal_comb_tech, NA),
         zscore_bal_comb_rep = ifelse(filter_zscore_comb_rep, zscore_bal_comb_rep, NA)) %>%
  dplyr::select(-sum_sign, -sum_sign_comb)
...
bal.zscore.global.comb = bal.zscore %>% distinct(drug, conc_char, barcode, replicate, zscore_bal_global) %>%
  ...
  mutate(zscore_bal_global_comb_tech = ave(zscore_bal_global, drug, conc_char, barcode, replicate,
         ...
         sum_sign_comb = ave(zscore_bal_global, drug, conc_char, barcode,
                                    FUN = function(x) length(table(sign(x)))),
         filter_global_zscore_comb_rep = ifelse(sum_sign_comb == 1, TRUE, FALSE),
         zscore_bal_global_comb_tech = ifelse(filter_global_zscore_comb_tech, zscore_bal_global_comb_tech, NA),
         zscore_bal_global_comb_rep = ifelse(filter_global_zscore_comb_rep, zscore_bal_global_comb_rep, NA))

bal.zscore.global %<>% left_join(bal.zscore.global.comb)

bal.zscore %<>% left_join(bal.zscore.global) %>% 
  dplyr::select(barcode, replicate, plate, tech, well, drug, conc_char, MMEJratio_norm,
                zscore_bal, zscore_bal_comb_tech, zscore_bal_comb_rep,
                zscore_bal_global, zscore_bal_global_comb_tech, zscore_bal_global_comb_rep)
```
...
dmso.mmej.score <- screen.ratio.dt %>%
  ...
  mutate(mean_DMSO = ave(DMSO, barcode, replicate, tech, FUN = function(x) mean(x, na.rm=T))) %>%
  dplyr::select(-DMSO) %>%
  distinct()

screen.ratio.dt %<>% left_join(dmso.mmej.score) %>% mutate(MMEJ_log2_FC = log2(MMEJratio/mean_DMSO)) %>% dplyr::select(-mean_DMSO)

# Double check correlation between replicates
cormat <- dmso.mmej.score %>% pivot_wider(names_from = c(replicate, tech), values_from = mean_DMSO) %>%
...
cor_filter = screen.ratio.dt %>% dplyr::select(barcode, replicate, tech, drug, conc_char, freqCut, MMEJscore, well, plate) %>%
  ...
  summarise(cor_TIF = cor(freqCut_E1504, freqCut_E177),
            cor_bal = cor(MMEJscore_E1504, MMEJscore_E177)) %>%
  mutate(cor_TIF_filter = ifelse(cor_TIF < TIF_cor, FALSE, TRUE),
         cor_bal_filter = ifelse(cor_bal < bal_cor, FALSE, TRUE))
...
ratio.cols = c("replicate",
               ...
               "zscore_TIF", 
               "zscore_TIF_comb_tech", 
               "zscore_TIF_comb_rep",
               "zscore_TIF_global", 
               "zscore_TIF_global_comb_tech",
               "zscore_TIF_global_comb_rep",
               "zscore_bal", 
               "zscore_bal_comb_tech", 
               "zscore_bal_comb_rep",
               "zscore_bal_global", 
