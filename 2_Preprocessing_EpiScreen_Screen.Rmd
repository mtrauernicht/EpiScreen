...
eff.zscore.pooled.global.comb = eff.zscore.global %>% 
  distinct(drug, conc_char, replicate, eff_zscore_pooled_global, mean_eff_zscore_global) %>%
...
dmso.mmej.score <- screen.ratio.dt %>%
  ...
  dplyr::select('DMSO' = MMEJratio, barcode, replicate) %>%
...
screen.ratio.dt <- screen.ratio.dt %>%
  ...
  # And calculate the z-scores
  mutate(MMEJ_zscore = (MMEJ_ratio_mean - mean_DMSO)/ sd_DMSO) %>%
...
dmso.mmej.score <- screen.ratio.dt %>%
  ...
  dplyr::select('DMSO' = MMEJratio, replicate) %>%
...
screen.ratio.dt <- screen.ratio.dt %>%
  mutate(MMEJ_ratio_mean_global = ave(MMEJratio, drug, concentration, replicate, FUN = mean)) %>%
  mutate(MMEJ_zscore_global = (MMEJ_ratio_mean_global - mean_DMSO)/ sd_DMSO) %>%
  dplyr::select(-mean_DMSO, -sd_DMSO)
```
