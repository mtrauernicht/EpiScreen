...
```{r outdir screen}
## Select outdir
out.dir = list.dirs(path = "/DATA/projects/DSBrepair/data/R") %>% .[grepl("episcreen_Bas_way", .)] %>% tail(n = 1)
in.dir = out.dir
...
mean.TIF.zscore.comb = TIF.zscore %>% distinct(drug, conc_char, barcode, replicate, zscore_TIF, zscore_TIF_global) %>%
  filter(!is.na(zscore_TIF)) %>%
  mutate(score_TIF_comb_tech = ave(zscore_TIF, drug, conc_char, barcode, replicate,
                                    FUN = mean),
         # Create filter for the z-scores for at least 2 significant hits per screen. 
         binary_zscore = ifelse(abs(zscore_TIF) >= 1.96, 1, 0),
         sum_tech = ave(binary_zscore, drug, conc_char, barcode, replicate,
                        FUN = sum),
         filter_score_TIF_comb_tech = ifelse(sum_tech >= 2, TRUE, FALSE),
         # Global z-scor: 
         score_TIF_global_comb_tech = ave(zscore_TIF_global, drug, conc_char, barcode, replicate,
                                    FUN = mean),
         # Create filter for the z-scores for at least 2 significant hits per screen. 
         binary_zscore_global = ifelse(abs(zscore_TIF_global) >= 1.96, 1, 0),
         sum_tech_global = ave(binary_zscore_global, drug, conc_char, replicate, barcode,
                        FUN = sum),
         filter_score_TIF_global_comb_tech = ifelse(sum_tech_global >= 2, TRUE, FALSE))
```

### DMSO null ditribution after averaging the z-scores of the technical replicates
```{r DMSO null distribution of mean replicates}
#data frame to save data
mean_sd_fits_score <- tibble(term = NA, estimate = NA, std.error = NA, barcode = NA, replicate = NA)

# First prepare a table with TIF in DMSO, well, barcode, replicate, tech and plate. Then group by tech and replicate.
dmso.TIF.score.comb <- mean.TIF.zscore.comb %>%
  filter(grepl("DMSO", drug)) %>%
  dplyr::select('DMSO' = score_TIF_comb_tech, 'DMSO_global' = score_TIF_global_comb_tech,  barcode, replicate) %>%
  distinct(barcode, replicate, DMSO, DMSO_global)

#Function to fit normal distribution through data
for (i in unique(dmso.TIF.score.comb$replicate)) {
  for (x in unique(dmso.TIF.score.comb$barcode)) {
    single_wt_ratio <- filter(dmso.TIF.score.comb, replicate == i & barcode == x) %>% pull(DMSO)
    mean_sd_fits_row <- fitdistr(single_wt_ratio, "normal") %>% tidy()
    mean_sd_fits_score <- mean_sd_fits_score %>% add_row(mean_sd_fits_row %>% mutate(barcode = x, replicate = i))
  }
  # Same for the global scores
  single_wt_ratio_global <- filter(dmso.TIF.score.comb, replicate == i) %>% distinct(DMSO_global) %>% pull(DMSO_global)
  mean_sd_fits_row_global <- fitdistr(single_wt_ratio_global, "normal") %>% tidy()
  mean_sd_fits_score <- mean_sd_fits_score %>% add_row(mean_sd_fits_row_global %>% mutate(barcode = "global", replicate = i))
}

#Transform the fitted values 
dmso.TIF.score.rep <- mean_sd_fits_score %>% filter(!is.na(term)) %>%
  dplyr::select(-std.error) %>%
  pivot_wider(names_from = term, values_from = estimate) %>% 
  dplyr::select(barcode, replicate, mean_DMSO = mean, sd_DMSO = sd)

# Same for the global scores to merge.
dmso.TIF.score.rep.global = dmso.TIF.score.rep %>% 
  filter(barcode == "global") %>% 
  dplyr::rename(mean_DMSO_global = mean_DMSO, sd_DMSO_global = sd_DMSO) %>% 
  dplyr::select(-barcode)

dmso.TIF.score.rep %<>% left_join(dmso.TIF.score.rep.global)
```

### Calculate the TIF z-scores per barcode combined reps
```{r calculate final efficiency z-scores}
# Calculate IPR z-scores per technical replicate
TIF.zscore.reps = mean.TIF.zscore.comb %>%
  left_join(dmso.TIF.score.rep) %>%
  # And calculate the z-scores
  mutate(zscore_TIF_tech = (score_TIF_comb_tech - mean_DMSO) / sd_DMSO,
         zscore_TIF_global_tech = (score_TIF_global_comb_tech - mean_DMSO_global) / sd_DMSO_global)

# Combine the z-scores of both screens
TIF.zscore.comb = TIF.zscore.reps %>% distinct(drug, conc_char, barcode, replicate, zscore_TIF, zscore_TIF_global, zscore_TIF_tech, zscore_TIF_global_tech) %>%
  filter(!is.na(zscore_TIF_tech)) %>%
  mutate(zscore_TIF_comb_rep = ave(zscore_TIF_tech, drug, conc_char, barcode,
                                    FUN = function(x) sum(x)/sqrt(length(x))),
         # Filter the z-scores for at least 2 significant hits per screen. 
         binary_zscore_rep = ifelse(abs(zscore_TIF_tech) >= 1.96, 1, 0),
         sum_tech_rep = ave(binary_zscore_rep, drug, conc_char, barcode, FUN = sum),
         filter_zscore_TIF_comb_rep = ifelse(sum_tech_rep >= 2, TRUE, FALSE),  
         # Filter for same effect. 
         sum_sign = ave(zscore_TIF_tech, drug, conc_char, barcode,
                             FUN = function(x) length(table(sign(x)))),
         filter_zscore_TIF_comb_rep = ifelse(sum_sign == 1, filter_zscore_TIF_comb_rep, FALSE),
######## Global z-score: 
         zscore_TIF_global_comb_rep = ave(zscore_TIF_global_tech, drug, conc_char, barcode,
                                    FUN =  function(x) sum(x)/sqrt(length(x))),
         # Create filter for the z-scores for at least 2 significant hits per screen. 
         binary_zscore_global_rep = ifelse(abs(zscore_TIF_global_tech) >= 1.96, 1, 0),
         sum_tech_global_rep = ave(binary_zscore_global_rep, drug, conc_char, barcode,
                        FUN = sum),
         filter_zscore_TIF_global_comb_rep = ifelse(sum_tech_global_rep >= 2, TRUE, FALSE),     
         # Filter for same effect.     
         sum_sign_global = ave(zscore_TIF_global_tech, drug, conc_char, barcode,
                             FUN = function(x) length(table(sign(x)))),
         filter_zscore_TIF_global_comb_rep = ifelse(sum_sign_global == 1, filter_zscore_TIF_global_comb_rep, FALSE))


TIF.zscore %<>% left_join(mean.TIF.zscore.comb) %>% left_join(TIF.zscore.comb) %>% 
  dplyr::select(barcode, replicate, plate, tech, well, drug, conc_char,
                zscore_TIF, score_TIF_comb_tech, 
                zscore_TIF_tech, filter_score_TIF_comb_tech,
                zscore_TIF_comb_rep, filter_zscore_TIF_comb_rep,
                zscore_TIF_global, score_TIF_global_comb_tech,
                zscore_TIF_global_tech, filter_score_TIF_global_comb_tech,
...
bal.zscore = bal_zscore_calc %>%
  ...
  mutate(min_MMEJratio = min(MMEJratio[MMEJratio != 0 & !is.na(MMEJratio)], na.rm = TRUE),
         MMEJratio = ifelse(MMEJratio == 0, min_MMEJratio, MMEJratio)) %>%
  ungroup() %>%
...
# Combine the z-scores of both screens
mean.bal.zscore.comb = bal.zscore %>% distinct(drug, conc_char, barcode, replicate, zscore_bal, zscore_bal_global) %>%
  filter(!is.na(zscore_bal)) %>%
  mutate(score_bal_comb_tech = ave(zscore_bal, drug, conc_char, barcode, replicate,
                                    FUN = mean),
         # Create filter for the z-scores for at least 2 significant hits per screen. 
         binary_zscore = ifelse(abs(zscore_bal) >= 1.96, 1, 0),
         sum_tech = ave(binary_zscore, drug, conc_char, barcode, replicate,
                        FUN = sum),
         filter_score_bal_comb_tech = ifelse(sum_tech >= 2, TRUE, FALSE),
         # Global z-scor: 
         score_bal_global_comb_tech = ave(zscore_bal_global, drug, conc_char, barcode, replicate,
                                    FUN = mean),
         # Create filter for the z-scores for at least 2 significant hits per screen. 
         binary_zscore_global = ifelse(abs(zscore_bal_global) >= 1.96, 1, 0),
         sum_tech_global = ave(binary_zscore_global, drug, conc_char, replicate, barcode,
                        FUN = sum),
         filter_score_bal_global_comb_tech = ifelse(sum_tech_global >= 2, TRUE, FALSE))
```

## DMSO null ditribution after averaging the z-scores of the technical replicates
```{r DMSO null distribution of mean replicates for the balance}
#data frame to save data
mean_sd_fits_score <- tibble(term = NA, estimate = NA, std.error = NA, barcode = NA, replicate = NA)

# First prepare a table with bal in DMSO, well, barcode, replicate, tech and plate. Then group by tech and replicate.
dmso.bal.score.comb <- mean.bal.zscore.comb %>%
  filter(grepl("DMSO", drug)) %>%
  dplyr::select('DMSO' = score_bal_comb_tech, 'DMSO_global' = score_bal_global_comb_tech,  barcode, replicate) %>%
  distinct(barcode, replicate, DMSO, DMSO_global)

#Function to fit normal distribution through data
for (i in unique(dmso.bal.score.comb$replicate)) {
  for (x in unique(dmso.bal.score.comb$barcode)) {
    single_wt_ratio <- filter(dmso.bal.score.comb, replicate == i & barcode == x) %>% pull(DMSO)
    mean_sd_fits_row <- fitdistr(single_wt_ratio, "normal") %>% tidy()
    mean_sd_fits_score <- mean_sd_fits_score %>% add_row(mean_sd_fits_row %>% mutate(barcode = x, replicate = i))
  }
  # Same for the global scores
  single_wt_ratio_global <- filter(dmso.bal.score.comb, replicate == i) %>% distinct(DMSO_global) %>% pull(DMSO_global)
  mean_sd_fits_row_global <- fitdistr(single_wt_ratio_global, "normal") %>% tidy()
  mean_sd_fits_score <- mean_sd_fits_score %>% add_row(mean_sd_fits_row_global %>% mutate(barcode = "global", replicate = i))
}

#Transform the fitted values 
dmso.bal.score.rep <- mean_sd_fits_score %>% filter(!is.na(term)) %>%
  dplyr::select(-std.error) %>%
  pivot_wider(names_from = term, values_from = estimate) %>% 
  dplyr::select(barcode, replicate, mean_DMSO = mean, sd_DMSO = sd)

# Same for the global scores to merge.
dmso.bal.score.rep.global = dmso.bal.score.rep %>% 
  filter(barcode == "global") %>% 
  dplyr::rename(mean_DMSO_global = mean_DMSO, sd_DMSO_global = sd_DMSO) %>% 
  dplyr::select(-barcode)

dmso.bal.score.rep %<>% left_join(dmso.bal.score.rep.global)
```

## Calculate the bal z-scores per barcode combined reps
```{r calculate final balance z-scores}
# Calculate IPR z-scores per technical replicate
bal.zscore.reps = mean.bal.zscore.comb %>%
  left_join(dmso.bal.score.rep) %>%
  # And calculate the z-scores
  mutate(zscore_bal_tech = (score_bal_comb_tech - mean_DMSO) / sd_DMSO,
         zscore_bal_global_tech = (score_bal_global_comb_tech - mean_DMSO_global) / sd_DMSO_global)

# Combine the z-scores of both screens
bal.zscore.comb = bal.zscore.reps %>% distinct(drug, conc_char, barcode, replicate, zscore_bal, zscore_bal_global, zscore_bal_tech, zscore_bal_global_tech) %>%
  filter(!is.na(zscore_bal_tech)) %>%
  mutate(zscore_bal_comb_rep = ave(zscore_bal_tech, drug, conc_char, barcode,
                                    FUN = function(x) sum(x)/sqrt(length(x))),
         # Filter the z-scores for at least 2 significant hits per screen. 
         binary_zscore_rep = ifelse(abs(zscore_bal_tech) >= 1.96, 1, 0),
         sum_tech_rep = ave(binary_zscore_rep, drug, conc_char, barcode, FUN = sum),
         filter_zscore_bal_comb_rep = ifelse(sum_tech_rep >= 2, TRUE, FALSE),
         # Filter for same effect. 
         sum_sign = ave(zscore_bal_tech, drug, conc_char, barcode,
                             FUN = function(x) length(table(sign(x)))),
         filter_zscore_bal_comb_rep = ifelse(sum_sign == 1, filter_zscore_bal_comb_rep, FALSE), 
         # Global z-score: 
         zscore_bal_global_comb_rep = ave(zscore_bal_global_tech, drug, conc_char, barcode,
                                    FUN =  function(x) sum(x)/sqrt(length(x))),
         # Create filter for the z-scores for at least 2 significant hits per screen. 
         binary_zscore_global_rep = ifelse(abs(zscore_bal_global_tech) >= 1.96, 1, 0),
         sum_tech_global_rep = ave(binary_zscore_global_rep, drug, conc_char, barcode,
                        FUN = sum),
         filter_zscore_bal_global_comb_rep = ifelse(sum_tech_global_rep >= 2, TRUE, FALSE),     
         # Filter for same effect.     
         sum_sign_global = ave(zscore_bal_global_tech, drug, conc_char, barcode,
                             FUN = function(x) length(table(sign(x)))),
         filter_zscore_bal_global_comb_rep = ifelse(sum_sign_global == 1, filter_zscore_bal_global_comb_rep, FALSE))


bal.zscore %<>% left_join(mean.bal.zscore.comb) %>% left_join(bal.zscore.comb) %>% 
  dplyr::select(barcode, replicate, plate, tech, well, drug, conc_char,
                MMEJ_FC, MMEJ_log2_FC,
                zscore_bal, score_bal_comb_tech, 
                zscore_bal_tech, filter_score_bal_comb_tech,
                zscore_bal_comb_rep, filter_zscore_bal_comb_rep,
                zscore_bal_global, score_bal_global_comb_tech,
                zscore_bal_global_tech, filter_score_bal_global_comb_tech,
...
ratio.cols = c("replicate",
               ...
               "zscore_TIF", 
               "score_TIF_comb_tech", "filter_score_TIF_comb_tech","zscore_TIF_tech",
               "zscore_TIF_comb_rep", "filter_zscore_TIF_comb_rep",
               "zscore_TIF_global", 
               "score_TIF_global_comb_tech", "filter_score_TIF_global_comb_tech","zscore_TIF_global_tech",
               "zscore_TIF_global_comb_rep", "filter_zscore_TIF_global_comb_rep",
               "zscore_bal", 
               "score_bal_comb_tech", "filter_score_bal_comb_tech","zscore_bal_tech",
               "zscore_bal_comb_rep", "filter_zscore_bal_comb_rep",
               "zscore_bal_global", 
               "score_bal_global_comb_tech", "filter_score_bal_global_comb_tech","zscore_bal_global_tech",
               "zscore_bal_global_comb_rep" , "filter_zscore_bal_global_comb_rep")
