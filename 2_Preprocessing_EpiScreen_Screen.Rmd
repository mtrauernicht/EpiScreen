...
## Select outdir
out.dir = paste0("/DATA/projects/DSBrepair/data/R/rs", Date, "_episcreen2/")
...
dmso.efficiency <- dmso.efficiency %>%
  distinct(DMSO_global, replicate, tech, well, plate) %>%
  mutate(DMSO_global = ave(DMSO_global, replicate, tech, FUN = mean)) %>%
  distinct() %>%
  right_join(dmso.efficiency)
...
dmso.eff.score <- screen.ratio.dt %>%
  filter(drug == "DMSO") %>%
  dplyr::select('DMSO' = freqCut, well, barcode, replicate, tech) %>%
  mutate(DMSO = ave(DMSO, barcode, well, replicate, tech, FUN = mean)) %>%
  distinct(barcode, replicate, tech, well, DMSO) %>%
...
>>>>>>> dc71f957e0ede195b57ff95416099cee39d870e7
```
...
eff.zscore = screen.ratio.dt %>%
  distinct(barcode, replicate, tech, drug, well, freqCut, conc_char, 
           mean_DMSO, sd_DMSO, mean_DMSO_pooled, sd_DMSO_pooled) %>%
...
eff.zscore %<>% left_join(eff.zscore.pooled.comb)
```
...
eff.zscore.global = screen.ratio.dt %>%
  distinct(replicate, tech, drug, well, freqCut_global, conc_char, 
           mean_DMSO_global, sd_DMSO_global, mean_DMSO_global_pooled, sd_DMSO_global_pooled) %>%
  # Then calculate the mean over the technical replicates
...
eff.zscore.pooled.global.comb = eff.zscore.global %>% 
  distinct(drug, conc_char, replicate, eff_zscore_pooled_global) %>%
...
# Merge with main table
screen.ratio.dt <- left_join(screen.ratio.dt, dmso.mmej.score, by = c("barcode", "replicate"))
...
screen.ratio.dt <- left_join(screen.ratio.dt, dmso.mmej.score, by = "replicate")
...
ratio.cols = c("replicate",
         ...
         "mean_eff_zscore",
         "eff_zscore_comb",
         "eff_zscore_pooled_comb",
         "mean_eff_zscore_comb",
         "eff_score_global_mean",
         "eff_zscore_global",
         "eff_zscore_pooled_global",
         "eff_zscore_pooled_global_comb",
         "MMEJ_score_mean",
         "MMEJ_zscore",
