...
indel.statistics.dt = as.data.table(readRDS("/DATA/projects/DSBrepair/git/EpiScreen/files_scripts/rs20211007_Episcreen_Reads_Viab_Metadata.RDS"))
...
unique(complete.mutations.dt[sum_bc_reads_split < 100]$exp) 
E177_pre = sum(grepl("E177", unique(complete.mutations.dt$exp)))
E1504_pre = sum(grepl("E1504", unique(complete.mutations.dt$exp)))

mutations.dt = complete.mutations.dt[sum_bc_reads_split >= 100]
dim(mutations.dt)

# Samples are removed from quite some wells. 
length(unique(mutations.dt$exp))

E177_post = sum(grepl("E177", unique(mutations.dt$exp)))
# Number of wells with removed IPRs
E177_pre - E177_post

E1504_post = sum(grepl("E1504", unique(mutations.dt$exp)))
# Number of wells with removed IPRs
...
# Sum large dels
mutations.large.del.sum.dt = mutations.large.del.dt[, lapply(.SD, sum, na.rm=TRUE), by=c("exp", "barcode"),
