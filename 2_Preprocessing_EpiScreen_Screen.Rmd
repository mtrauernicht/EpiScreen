...
```{r outdir screen}
## Select outdir
out.dir = list.dirs(path = "/DATA/projects/DSBrepair/data/R") %>% .[grepl("episcreen_Bas_way_all_bcs", .)] %>% tail(n = 1)
in.dir = out.dir
...
mean.TIF.zscore.comb = TIF.zscore %>% 
  distinct(drug, conc_char, barcode, replicate, zscore_TIF, zscore_TIF_global) %>%
  filter(!is.na(zscore_TIF)) %>%
  mutate(
    # Local score
    score_TIF_comb_tech = ave(zscore_TIF, drug, conc_char, barcode, replicate,
                              FUN = mean),
    # Global score: 
...
# Combine the z-scores of both screens
TIF.zscore.comb = TIF.zscore.reps %>% 
  distinct(drug, conc_char, barcode, replicate, 
           zscore_TIF, zscore_TIF_global, zscore_TIF_tech, zscore_TIF_global_tech, 
           filter_zscore_TIF, filter_zscore_TIF_global) %>%
  filter(!is.na(zscore_TIF_tech)) %>%
  mutate(
    # Local z-score:
    zscore_TIF_comb_rep = ave(zscore_TIF_tech, drug, conc_char, barcode,
  ...
  filter_zscore_TIF_global_comb_rep = ifelse(sum_sign_global == 1 & 
...
mean.bal.zscore.comb = bal.zscore %>% 
  distinct(drug, conc_char, barcode, replicate, zscore_bal, zscore_bal_global, MMEJ_FC, MMEJ_log2_FC) %>%
  filter(!is.na(zscore_bal)) %>%
  mutate(
    # Local score
    score_bal_comb_tech = ave(zscore_bal, drug, conc_char, barcode, replicate,
                              FUN = mean),
    # Global score: 
...
# Combine the z-scores of both screens
bal.zscore.comb = bal.zscore.reps %>% 
  distinct(drug, conc_char, barcode, replicate, 
           zscore_bal, zscore_bal_global, zscore_bal_tech, zscore_bal_global_tech, 
           filter_zscore_bal, filter_zscore_bal_global) %>%
  filter(!is.na(zscore_bal_tech)) %>%
  mutate(
    # Local z-score:
    zscore_bal_comb_rep = ave(zscore_bal_tech, drug, conc_char, barcode,
                              FUN = function(x) sum(x)/sqrt(length(x))),
    # Global z-score: 
  ...
  filter_zscore_bal_global_comb_rep = ifelse(sum_sign_global == 1 & 
...
ratio.cols = c("replicate",
               ...
               "zscore_TIF", 
               "score_TIF_comb_tech", "filter_zscore_TIF","zscore_TIF_tech",
               "zscore_TIF_comb_rep", "filter_zscore_TIF_comb_rep",
               "zscore_TIF_global", 
               "score_TIF_global_comb_tech", "filter_zscore_TIF_global","zscore_TIF_global_tech",
               "zscore_TIF_global_comb_rep", "filter_zscore_TIF_global_comb_rep",
               "zscore_bal", 
               "score_bal_comb_tech", "filter_zscore_bal","zscore_bal_tech",
               "zscore_bal_comb_rep", "filter_zscore_bal_comb_rep",
               "zscore_bal_global", 
               "score_bal_global_comb_tech", "filter_zscore_bal_global","zscore_bal_global_tech",
               "zscore_bal_global_comb_rep" , "filter_zscore_bal_global_comb_rep")
