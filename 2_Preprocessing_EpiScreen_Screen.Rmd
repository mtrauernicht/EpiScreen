...
count_filter %>% group_by(exp) %>% 
  filter(sum) %>%
  count() %>% pull(n) %>% mean()
# How many barcodes per IPR on average after the filter without PAO control wells?
count_filter %>% group_by(exp) %>% 
  filter(sum, !grepl("A01|A12|E01|E12", exp)) %>%
  count() %>% pull(n) %>% mean()
...
# This filters out all drugs at the concentration set in the setup
# The drug concentration needs to be in 1/2 or 2/3 samples per screen
# AND in both screens.
keep_drugs <- mutations.stat.dt %>%
  ...
  mutate(conc_char_drug = paste(conc_char, drug, sep = " "),
         ...
         # count replicates in which this combination is found in
         tech_n = as.numeric(ave(conc_char_drug_rep, conc_char_drug_rep, FUN = length)),
         viab_sum_eff = ave(viab_norm_split > viab_limit_eff, conc_char_drug_rep, FUN = sum),
         viab_eff = case_when(viab_sum_eff >= 2 ~ TRUE, 
                              viab_sum_eff == 1 & tech_n <= 2 ~ TRUE,
                              T ~ FALSE),
         viab_sum_bal = ave(viab_norm_split > viab_limit_bal, conc_char_drug_rep, FUN = sum),
         viab_bal =  case_when(viab_sum_bal >= 2 ~ TRUE, 
                              viab_sum_bal == 1 & tech_n <= 2 ~ TRUE,
                              T ~ FALSE)) %>% 
  # These conditions need to be present in at least 1 out of 2 or 2 out of 3 replicates 
  # per screen
  distinct(conc_char_drug, conc_char, drug, replicate, viab_eff, viab_bal) %>% 
  mutate(rep_bal = ave(viab_bal, conc_char_drug, FUN = sum),
         pathway_bal_filter = ifelse(rep_bal == 2, TRUE, FALSE),
         rep_eff = ave(viab_eff, conc_char_drug, FUN = sum),
         indel_freq_filter = ifelse(rep_eff == 2, TRUE, FALSE)) %>% 
  # check if present in both replicates
  # And in both bio replicates
  distinct(conc_char, drug, pathway_bal_filter, indel_freq_filter)

keep_drugs %>% dplyr::select(pathway_bal_filter, indel_freq_filter) %>% colSums()
...
mutations.screen.dt <- mutations.stat.dt %>%
  ...
  left_join(keep_drugs) %>%
  # make filter for the conditions for which we do not have at least 2 technical replicates
  mutate(viab_norm_split_eff = ifelse(indel_freq_filter & viab_norm_split > viab_limit_eff,
                                      viab_norm_split, NA), 
         viab_norm_split_bal = ifelse(pathway_bal_filter & viab_norm_split > viab_limit_bal,
                                      viab_norm_split, NA)) %>%
  # Calculate new mean
  mutate(viab_mean_if = ave(viab_norm_split_eff, drug, conc_char, replicate, 
                            FUN = function(x) mean(x, na.rm=T)),
         viab_mean_pb = ave(viab_norm_split_bal, drug, conc_char, replicate, 
                            FUN = function(x) mean(x, na.rm=T)))
