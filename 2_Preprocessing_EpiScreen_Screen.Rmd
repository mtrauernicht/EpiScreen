...
```{r run QC}
if(render_QC) {
rmarkdown::render("1_ParsingQC_DrugScreen.Rmd")
}
...
screen.ratio.dt = left_join(screen.ratio.dt, dmso.eff) %>% left_join(dmso.global)
...
screen.ratio.dt <- screen.ratio.dt %>%
  ...
  mutate(freqCut_norm_mean = mean(freqCut_norm, na.rm = TRUE),
...
for (i in unique(dmso.eff.score$rep)) {
  for (x in unique(dmso.eff.score$barcode)) {
    single_wt_ratio <- filter(dmso.eff.score, rep == i & barcode == x) %>% pull(DMSO)
...
dmso.eff.score <- mean_sd_fits %>% filter(!is.na(term)) %>%
...
# First calculate means and sd of the DMSO efficiency per replicate and barcode
global.dmso.eff.score <- screen.ratio.dt %>%
  filter(drug == "DMSO") %>%
...
#Function to fit normal distribution through data
for (i in unique(global.dmso.eff.score$rep)) {
    single_wt_ratio <- filter(global.dmso.eff.score, rep == i) %>% pull(DMSO)
    mean_sd_fits_row <- fitdistr(single_wt_ratio, "normal") %>% tidy()
    global_mean_sd_fits <- global_mean_sd_fits %>% add_row(mean_sd_fits_row %>% mutate(rep = i))
}

#Transform the fitted values 
global.dmso.eff.score <- global_mean_sd_fits %>% filter(!is.na(term)) %>%
  dplyr::select(-std.error) %>%
  separate(rep, into = c("replicate", "tech"), sep = " ") %>%
  mutate(tech = as.integer(tech)) %>%
  pivot_wider(names_from = term, values_from = estimate) %>% 
...
screen.ratio.dt = left_join(screen.ratio.dt, dmso.eff.score) %>% left_join(global.dmso.eff.score)
```
...
eff.zscore = screen.ratio.dt %>%
  ...
  mutate(eff_zscore = (freqCut - mean_DMSO)/ sd_DMSO,
         mean_eff_zscore = ave(eff_zscore, drug, conc_char, barcode, replicate, FUN = function(x) mean(x, na.rm=T)))
...
eff.zscore.global = screen.ratio.dt %>%
  ...
  mutate(mean_eff_zscore_global = ave(eff_zscore_global, drug, conc_char, replicate, FUN = function(x) mean(x, na.rm=T)))
...
eff.zscore.pooled.global.comb = eff.zscore.global %>% 
  ...
  filter(!is.na(mean_eff_zscore_global)) %>%
...
screen.ratio.dt <- screen.ratio.dt %>%
  ...
  mutate(MMEJ_zscore_mean = ave(MMEJ_zscore, drug, conc_char, replicate, barcode, FUN = function(x) mean(x, na.rm=T))) %>%
...
dmso.mmej.score <- screen.ratio.dt %>%
  ...
  filter(drug == "DMSO") %>%
  mutate(MMEJratio = ave(MMEJratio, tech, drug, well, plate, FUN = function(x) mean(x, na.rm=T))) %>%
  dplyr::select('DMSO' = MMEJratio, replicate, tech) %>%
  mutate(mean_DMSO = ave(DMSO, replicate, tech, FUN = function(x) mean(x, na.rm=T)), 
...
screen.ratio.dt <- screen.ratio.dt %>%
  mutate(MMEJ_ratio_global = ave(MMEJratio, drug, concentration, replicate, tech, FUN = function(x) mean(x, na.rm=T))) %>%
  mutate(MMEJ_zscore_global = (MMEJ_ratio_global - mean_DMSO)/ sd_DMSO) %>%
  mutate(MMEJ_zscore_global_mean = ave(MMEJ_zscore_global, drug, conc_char, replicate, FUN = function(x) mean(x, na.rm=T))) %>%
  dplyr::select(-mean_DMSO, -sd_DMSO)
```
...
zscores_comb_global <- screen.ratio.dt %>%
  ...
  filter(!is.na(MMEJ_zscore_global_mean)) %>%
