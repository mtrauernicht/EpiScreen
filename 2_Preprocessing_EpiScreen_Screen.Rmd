...
screen.indels.dt = screen.indel.dt %>% select(all_of(indel.cols)) %>% distinct()
...
dmso.eff.score <- screen.ratio.dt %>%
  ...
  mutate(DMSO_global = ave(DMSO_global, replicate, tech, well, plate, FUN = function(x) mean(x, na.rm=T))) %>%
  distinct(replicate, tech, well, plate, DMSO_global) %>%
  mutate(mean_DMSO_global = ave(DMSO_global, replicate, tech, FUN = function(x) mean(x, na.rm=T)), 
...
screen.ratio.dt %<>% left_join(dmso.mmej.score) %>% mutate(MMEJ_log2_FC = log2(MMEJratio/mean_DMSO)) %>% select(-mean_DMSO)
