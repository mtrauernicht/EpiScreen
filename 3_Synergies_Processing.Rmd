...
filtered_drug_conc = filtered_drug_pvals %>%
  filter(adj_pval_TIF < 0.01 | adj_pval_bal < 0.01 | drug == "DMSO") %>%
...
dplyr::group_by(drug, conc_char) %>% 
  ...
  mutate(zscore_TIF_comb_rep_temp = ifelse(adj_pval_TIF >= 0.01, 0, zscore_TIF_comb_rep_temp),
         zscore_bal_comb_rep_temp = ifelse(adj_pval_bal >= 0.01, 0, zscore_bal_comb_rep_temp)) %>%
...
ratios_tib <- indel.data %>%
  ...
  distinct(MMEJratio_mean, freqCut_mean, drug, conc_char, drug_conc, target, IPR) %>%
  mutate(MMEJratio.log2 = log2(MMEJratio_mean),
         MMEJratio.log2 = ave(MMEJratio.log2, drug, conc_char, IPR, FUN = mean),
         freqCut.log2 = log2(freqCut_mean),
         freqCut.log2 = (ave(freqCut.log2, drug, conc_char, IPR, FUN = mean))) %>%
...
dmso_ratios_tib <- indel.data %>%
  ...
  mutate(DMSO.ratio.log2 = log2(MMEJratio_mean),
         DMSO.ratio.log2 = ave(DMSO.ratio.log2, IPR, FUN = mean),
         DMSO.freqCut.log2 = log2(freqCut_mean),
         DMSO.freqCut.log2 = (ave(DMSO.freqCut.log2, IPR, FUN = mean))) %>%
  dplyr::select(-MMEJratio_mean, -freqCut_mean) %>%
  distinct()
...
epistasis_tib %<>% 
  ...
  distinct(signif.ratio, signif.freqCut, # These are p values for a significant CCD
           ...
           effect_on_indelrate, effect_on_ratio # If the effect is significant (from above)
           ) %>% 
  mutate(ratio_slope_plot = case_when(
      ...
      effect_on_ratio & signif.freqCut & TIF_signif == "bal_both" ~ indelrate_slope,
      T ~ as.double(NA)),
