...
filtered_drug_pvals = indel.data %>%
  ...
  mutate(zscore_TIF_comb_rep_temp = ifelse(filter_zscore_TIF_comb_rep, zscore_TIF_comb_rep, 0),
         ...
         adj_pval_TIF = p.adjust(p_val_TIF, method = 'holm'),
...
filtered_drug_conc = filtered_drug_pvals %>%
  filter(adj_pval_TIF < pval_signif | adj_pval_bal < pval_signif | drug == "DMSO") %>%
  ...
  mutate(# Count of IPR per compound that have an effect. We want at least 2 (avoid outliers)
    ...
    keep = ifelse(count_TIF > 1 | count_bal > 1, 
                  TRUE, FALSE)) %>%
  filter(keep) %>%
...
dplyr::group_by(drug, conc_char) %>% 
  ...
  dplyr::summarise(MMEJ_signif = sum(zscore_bal_comb_rep_temp < -1.96), #Summarise number of significant barcodes per compound for balance
  ...
  mutate(bal_signif = case_when(MMEJ_signif <= 1 & NHEJ_signif > 1 ~ "NHEJ", #Annotate if a protein favors NHEJ, MMEJ or both
                                    NHEJ_signif <= 1 & MMEJ_signif > 1 ~ "MMEJ", 
                                    T ~ "both"),
         TIF_signif = case_when(TIF_up_signif <= 1 & TIF_down_signif > 1 ~ "TIF_down", #Annotate if a protein favors NHEJ, MMEJ or both
                                    TIF_down_signif <= 1 & TIF_up_signif > 1 ~ "TIF_up", 
                                    T ~ "TIF_both")) %>%
  mutate(drug_conc = paste(drug, conc_char, sep = " ")) %>%
  semi_join(filtered_drug_conc) # Semi-join the filtered druc conc to keep only the significant ones after p adjustment.
...
ratios_tib <- indel.data %>%
  ...
  distinct(MMEJratio, freqCut_mean, drug, conc_char, drug_conc, target, IPR, replicate) %>% 
  mutate(MMEJratio_mean = ave(MMEJratio, drug, conc_char, IPR, replicate, FUN = function(x) mean(x, na.rm = TRUE)),
         MMEJratio.log2 = log2(MMEJratio_mean),
         MMEJratio.log2 = ave(MMEJratio.log2, drug, conc_char, IPR, FUN = function(x) mean(x, na.rm = TRUE)),
         freqCut.log2 = log2(freqCut_mean),
         freqCut.log2 = (ave(freqCut.log2, drug, conc_char, IPR, FUN = function(x) mean(x, na.rm = TRUE)))) %>%
  dplyr::select(-MMEJratio_mean, -MMEJratio, -freqCut_mean, -replicate) %>%
  distinct()
...
dmso_ratios_tib <- indel.data %>%
  ...
  mutate(DMSO.ratio.log2 = log2(MMEJratio_mean),
         DMSO.ratio.log2 = ave(DMSO.ratio.log2, IPR, FUN = function(x) mean(x, na.rm = TRUE)),
         DMSO.freqCut.log2 = log2(freqCut_mean),
         DMSO.freqCut.log2 = (ave(DMSO.freqCut.log2, IPR, FUN = function(x) mean(x, na.rm = TRUE)))) %>%
...
signif_hits = adj_p.value_KO_model %>% filter(p.adj < pval_signif) %>% 
...
epistasis_tib %<>% 
  ...
  distinct(signif.ratio, signif.freqCut, # These are p values for a significant CCD
           drug_conc, feature, 
  ...
  mutate(ratio_slope_plot = case_when(
      effect_on_ratio & signif.ratio & bal_signif == "MMEJ" & ratio_slope < 0 ~ ratio_slope, 
      bal_signif == "MMEJ" & ratio_slope > 0 ~ as.double(NA), 
      effect_on_ratio & signif.ratio & bal_signif == "NHEJ" & ratio_slope > 0 ~ ratio_slope, 
      bal_signif == "NHEJ" & ratio_slope < 0 ~ as.double(NA), 
      effect_on_ratio & signif.ratio & bal_signif == "bal_both" ~ indelrate_slope,
      T ~ as.double(NA)),
    indelrate_slope_plot = case_when(
      effect_on_indelrate & signif.freqCut & TIF_signif == "TIF_down" & indelrate_slope < 0 ~ indelrate_slope, 
      TIF_signif == "TIF_down" & indelrate_slope > 0 ~ as.double(NA), 
      effect_on_indelrate & signif.freqCut & TIF_signif == "TIF_up" & indelrate_slope > 0 ~ indelrate_slope, 
      TIF_signif == "TIF_up" & indelrate_slope < 0 ~ as.double(NA), 
      effect_on_indelrate & signif.freqCut & TIF_signif == "TIF_both" ~ indelrate_slope,
      T ~ as.double(NA))) %>%
