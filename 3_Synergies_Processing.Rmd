...
filtered_drug_pvals = indel.data %>%
  ...
  mutate(zscore_TIF_comb_rep_temp = ifelse(!filter_zscore_TIF_comb_rep, 0, zscore_TIF_comb_rep),
         zscore_bal_comb_rep_temp = ifelse(!filter_zscore_bal_comb_rep, 0, zscore_bal_comb_rep),
...
epistasis_tib %<>% 
  ...
  distinct(signif.ratio, signif.freqCut, drug_conc, feature, indelrate_slope, ratio_slope, bal_signif, TIF_signif) %>% 
  mutate(ratio_slope_plot = case_when(signif.ratio & bal_signif == "MMEJ" & ratio_slope < 0 ~ ratio_slope, 
                               bal_signif == "MMEJ" & ratio_slope > 0 ~ as.double(NA), 
                               signif.ratio & bal_signif == "NHEJ" & ratio_slope > 0 ~ ratio_slope, 
                               bal_signif == "NHEJ" & ratio_slope < 0 ~ as.double(NA), 
                               signif.freqCut & TIF_signif == "bal_both" ~ indelrate_slope,
                               T ~ as.double(NA)),
         indelrate_slope_plot = case_when(signif.freqCut & TIF_signif == "TIF_down" & indelrate_slope < 0 ~ indelrate_slope, 
                               TIF_signif == "TIF_down" & indelrate_slope > 0 ~ as.double(NA), 
                               signif.freqCut & TIF_signif == "TIF_up" & indelrate_slope > 0 ~ indelrate_slope, 
                               TIF_signif == "TIF_up" & indelrate_slope < 0 ~ as.double(NA), 
                               signif.freqCut & TIF_signif == "TIF_both" ~ indelrate_slope,
                               T ~ as.double(NA)))
