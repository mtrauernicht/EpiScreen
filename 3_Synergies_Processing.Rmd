...
filtered_drug_conc = indel.data %>%
  ...
  mutate(drug_conc = paste(drug, conc_char,sep = " "),
         effect_on_indelrate = ifelse(abs(zscore_TIF_comb_rep) >= 2.58 & adj_pval_TIF < 0.01, TRUE, FALSE),
         effect_on_ratio = ifelse(abs(zscore_bal_comb_rep) >= 2.58 & adj_pval_bal < 0.01, TRUE, FALSE)) %>%
...
dmso_ratios_tib <- indel.data %>%
  filter(drug == "DMSO") %>%
  distinct(MMEJratio, freqCut, IPR) %>%
  mutate(DMSO.ratio = ave(MMEJratio, IPR, FUN = mean)) %>%
