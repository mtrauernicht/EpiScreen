...
indel.data <- merge(indel.data.rep4, indel.data.rep3, all = TRUE)
...
# Correlation efficiency vs ratio
ggplot(data = indel.data, aes(x=mean.ratio.new, y=mean.eff, colour = Target)) +
    geom_point() +
...
# Mean efficiency vs. mean ratio per target
ggplot(data = indel.data, aes(x=mean.ratio.new, y=mean.eff)) + 
  geom_point() + geom_smooth(method = lm) + 
...
correlation.plot <- indel.data.rep4[,c(17,20,21)]
pairs(correlation.plot)

# Correlation plot plate 1uM_plate1
correlation.plot.weird.plate <- indel.data.rep4[grep("1um_.*_plate1",indel.data.rep4$condition),]
correlation.plot.weird.plate <- correlation.plot.weird.plate[,c(17,20,21)]
pairs(correlation.plot.weird.plate)

# Correlation reads vs. viability
data <- indel.data[indel.data$plate.nr == c("5","9"), ]
ggplot(data = data, aes(x=read.count, y=viability, colour = plate.nr)) +
  geom_point()

# Outlier plot
ggplot(data = outlier.df, aes(x = barcode, y = logratio, colour = Target)) + 
  geom_point() + theme(axis.text.x = element_text(angle = 90, hjust = 1.0, vjust = 0))
```
...
>>>>>>> 17dae7db6a314af4012aac1f44689dc1ac4099a6
