...
label_ratio <- indel.data %>%
  ...
  filter(drug_conc %in% c("DMSO_control", "Decitabine_1 µM")) %>% 
...
bc_mut_tib_mean <- mutation.data %>% 
  ...
  filter(drug_conc %in% c("DMSO_control", "Decitabine_1 µM"), type %in% c("core", "grouped_indel")) %>% 
...
for (i in unique(all.samples$drug_conc_exp)) {
  if (length(all.samples$value[all.samples$drug_conc_exp == i & all.samples$variable == "G2"]) > 1) { 
...
all.samples.plot %>% 
  ggplot(., aes(y = mean_value, x = reorder(drug_conc, -g2_value))) +
  geom_linerange(aes(ymin = vadj, ymax = vadj+SD, color = target), size = 1) +
  geom_bar(stat="identity", aes(alpha = variable, fill = target)) +
