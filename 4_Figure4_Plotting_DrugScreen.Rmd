...
screen_ratios = readRDS("/DATA/projects/DSBrepair/data/R/rs20220211_episcreen/rs20220211_episcreen_ratios.RDS")
...
chrom_data = screen_ratios %>% dplyr::select(barcode, all_of(chromatin.features.zs)) %>% distinct()
...
annotation_colors_scr = list(
  ...
  group = c(insulator="#874000", repressive="#010001", polycomb ="#2B0504", euchromatin="#70F8BA",
                 transcribing="#CAFE48", HDAC="#BC5F04", accessibility='#F4442E',
                 methylation='#7CB4B8'))
...
clone5_map = hclust(FC(t(clone5.matrix)))
...
clone5_map_ipr_18 = hclust(FC(clone5.matrix))
...
pdf(paste0(figure_out, "rs20220211_synergy_pathwayratio_all.pdf"), width=10, height=13)
...
ratios_tib <- screen_ratios %>%
  mutate(drug_conc_char = paste(drug, conc_char, sep = "_")) %>%
  semi_join(filtered_drug_conc) %>%
  distinct(NHEJratio, drug, conc_char, target, barcode) %>%
  mutate(NHEJratio = ave(NHEJratio, drug, conc_char, barcode, FUN = mean)) %>%
  mutate(NHEJratio.log2 = log2(NHEJratio)) %>%
  distinct()
...
vorino_screen = ratios_tib %>% filter(drug == "Vorinostat (SAHA, MK0683)") %>% 
  ungroup() %>%
  select(all_of(chromatin.features.zs), barcode, log2.FC.ratio, NHEJratio, DMSO.ratio) %>% 
  dplyr::rename(screen_FC = log2.FC.ratio, DMSO_screen = DMSO.ratio) %>%
  distinct(barcode, screen_FC, H3K27me3.zscore, NHEJratio, DMSO_screen)
...
GSK_fc = GSK_BIX_molcell_tib %>% 
  ...
  select(barcode, logFC_GSK)
  
chrom_data_pool = GSK_BIX_molcell_tib %>% select(barcode, all_of(chromatin.features)) %>% distinct()
