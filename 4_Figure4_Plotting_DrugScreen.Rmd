...
# screen_indels = readRDS("/DATA/projects/DSBrepair/data/R/rs20220211_episcreen/rs20220211_episcreen_mutations.RDS")
screen_ratios = readRDS("/DATA/projects/DSBrepair/data/R/rs20220216_episcreen/rs20220216_episcreen_ratios.3.RDS")
...
# Chromatin info
chromatin.features.zs <- colnames(screen_ratios)[grepl("[.]zscore", colnames(screen_ratios))]
chrom_data = screen_ratios %>% dplyr::select(IPR, all_of(chromatin.features.zs)) %>% distinct()

chromatin.features.zs
...
filtered_drug_conc = screen_ratios %>%
  filter(abs(mean_eff_zscore_comb) > 2.58 | abs(MMEJ_zscore_comb) > 2.58 | drug == "DMSO") %>%
  mutate(drug_conc = paste(drug, conc_char, sep = "_"),
         effect_on_indelrate = ifelse(abs(mean_eff_zscore_comb) > 2.58, TRUE, FALSE),
         effect_on_ratio = ifelse(abs(MMEJ_zscore_comb) > 2.58, TRUE, FALSE)) %>%
...
ratios_tib <- screen_ratios %>%
  ...
  distinct(NHEJratio, freqCut, drug, conc_char, target, IPR, barcode) %>%
  mutate(NHEJratio = ave(NHEJratio, drug, conc_char, IPR, barcode, FUN = mean)) %>%
  mutate(NHEJratio.log2 = log2(NHEJratio)) %>%
  mutate(freqCut = (ave(freqCut, drug, conc_char, IPR, barcode, FUN = mean))) %>%
  mutate(freqCut.log2 = log2(freqCut)) %>%
  distinct()

dmso_ratios_tib <- screen_ratios %>%
  filter(drug == "DMSO") %>%
  distinct(NHEJratio, freqCut, IPR) %>%
  mutate(DMSO.ratio = ave(NHEJratio, IPR, FUN = mean)) %>%
  mutate(DMSO.ratio.log2 = log2(DMSO.ratio)) %>%
  mutate(DMSO.freqCut = ave(freqCut, IPR, FUN = mean)) %>%
  mutate(DMSO.freqCut.log2 = log2(DMSO.freqCut)) %>%
  dplyr::select(-NHEJratio, -freqCut) %>%
  distinct()

ratios_tib <- merge(ratios_tib, dmso_ratios_tib) %>% 
  dplyr::group_by(IPR, drug, conc_char) %>% 
  mutate(log2.FC.ratio = NHEJratio.log2 - DMSO.ratio.log2, 
         log2.FC.freqCut = freqCut.log2 - DMSO.freqCut.log2) %>%
  left_join(chrom_data)
```

The log2 FCance ratio in our case is positive if it promotes NHEJ and negative if it promotes NHEJ
...
synergy_calc_tib = ratios_tib %>% ungroup() %>% select(log2.FC.freqCut, log2.FC.ratio, IPR, drug_conc, all_of(chromatin.features.zs))

for (i in unique(synergy_calc_tib$drug_conc)) {
  for (j in chromatin.features.zs) {
    model_tib_if <- synergy_calc_tib %>% filter(!is.na(log2.FC.freqCut), drug_conc == i)
    model_tib_pf <- synergy_calc_tib %>% filter(!is.na(log2.FC.ratio), drug_conc == i)
    
    # Indel rates
    model.indelrate.log2 = tibble(term = c("(Intercept)", "slope"), estimate  = NA, p.value = NA)

    if (dim(model_tib_if)[1] != 0) {
      
    model.indelrate.log2 <- lm(formula = log2.FC.freqCut ~ unlist(model_tib_if[j]), 
                               data = model_tib_if) %>% 
      tidy()
    model.indelrate.log2[2, 1] <- "slope"
    }   
    
    
    # Pathway balance
    model.ratio.log2 = tibble(term = c("(Intercept)", "slope"), estimate  = NA, p.value = NA)
    if (dim(model_tib_pf)[1] != 0) {

    model.ratio.log2 <- lm(formula = log2.FC.ratio ~ unlist(model_tib_pf[j]), 
                           data = model_tib_pf) %>% 
      tidy()
    model.ratio.log2[2, 1] <- "slope"
    }    
...
synergy_tib %<>% left_join(effect_compounds)  %>% 
  left_join(p.vals) %>%
  mutate(
...
# colors <- brewer.pal(length(unique(IPR.annotation$cluster)), "Pastel2")
# names(colors) <- unique(IPR.annotation$cluster)
annotation_colors_scr = list(
  ...
  group = c(insulator="#BD263D", repressive="#8D1B60", polycomb ="#D21F8A", euchromatin="#F7941D",
                 transcribing="#8CC63F", HDAC="#BC5F04", accessibility='#FFFF00',
                 methylation='#7CB4B8'))
...
#   select(-IPR, -group, -pool, -binsize, -ID) %>%
#   column_to_rownames(var = "IPR") %>% 
#   as.matrix() %>%
#   FC() %>% 
#   hclust()
...
# 
pdf(paste0(figure_out, "rs20220217_zscorepathwayratio_all.pdf"), width=10, height=20)
...
# giveNAs = which(is.na(as.matrix(FC(all_compounds_pathway_synergy))),arr.ind=TRUE)
# unique(giveNAs)
# 
# tab = sort(table(c(giveNAs)),decreasing=TRUE)
# checkNA = sapply(1:length(tab),function(i){
# sum(is.na(as.matrix(FC(all_compounds_pathway_synergy[-as.numeric(names(tab[1:i])),]))))
# })
# rmv = names(tab)[1:min(which(checkNA==0))]

# screen_ratios %>% dis
all_compounds_pathway_synergy = all_compounds_pathway_synergy[!grepl("DNA-PK|Mirin|Null", rownames(all_compounds_pathway_synergy)), ]
...
pdf(paste0(figure_out, "rs20220217_synergy_pathwayratio_all.pdf"), width=10, height=13)
...
############################# INDEL FREQUENCY ########################
all_compounds_indelrate_synergy

all_compounds_indelrate_synergy = all_compounds_indelrate_synergy[!grepl("DNA-PK|Mirin|Null", rownames(all_compounds_indelrate_synergy)), ]

quant_breaks <- quantile(all_compounds_indelrate_synergy, c(.01,.99), na.rm = T) # change these numbers according to where you want to set the cutoff
palette_length <- 1000
breaks <- c(seq(quant_breaks[1], 0, length.out=ceiling(palette_length/2) + 1), 
            seq(quant_breaks[2]/palette_length, quant_breaks[2], length.out=floor(palette_length/2)))
colors = colorRampPalette(slope.colors %>% pull(color))(length(breaks))
...
clone5_map_ipr_18 = hclust(dist(t(clone5.matrix)))
clone5_map_ipr_18_labes =  clone5_map_ipr_18$labels[clone5_map_ipr_18$labels %in% colnames(all_compounds_indelrate_synergy)]

pdf(paste0(figure_out, "rs20220217_synergy_indelfreq_all.pdf"), width=22, height=12)

pheatmap(t(all_compounds_indelrate_synergy[, clone5_map_ipr_18_labes]),
         border_color = F,
         cellwidth = 8,
         cellheight = 8,
         cluster_rows = clone5_map,
         breaks = breaks,
         annotation_col = target,
         main = "Indel frequency synergy for all compounds with significant hit",
         color = colors,
         annotation_colors = annotation_colors_scr, annotation_legend = T,
...
# Data table and compute log2_ratio (This line needs to be adjusted)
filtered_drug_conc = screen_ratios %>%
  filter(abs(MMEJ_zscore) > 1.96 | drug == "DMSO") %>%
  mutate(drug_conc = paste(drug, conc_char, sep = "_"),
...
ratios_tib <- screen_ratios %>%
  mutate(drug_conc_char = paste(drug, conc_char, sep = "_")) %>%
  semi_join(filtered_drug_conc) %>%
  distinct(NHEJratio, drug, conc_char, target, IPR) %>%
  mutate(NHEJratio = ave(NHEJratio, drug, conc_char, IPR, FUN = mean)) %>%
  mutate(NHEJratio.log2 = log2(NHEJratio)) %>%
  distinct()
...
vorinostat_ratios = validations_ratios %>% 
  ...
  distinct(IPR, drug, NHEJ_MMEJ , efficiency) %>% 
...
vorinostat_ratios %<>% left_join(unique(validations_ratios[, c(1, 42:75)]), by = "IPR") %>% distinct(IPR, .keep_all = TRUE)

chromatin.features <- colnames(validations_ratios)[42:65]
...
vorino_screen = ratios_tib %>% filter(drug == "Vorinostat (SAHA, MK0683)") %>% 
  ungroup() %>%
  select(all_of(chromatin.features.zs), IPR, log2.FC.ratio, NHEJratio, DMSO.ratio) %>% 
  dplyr::rename(screen_FC = log2.FC.ratio, DMSO_screen = DMSO.ratio) %>%
  distinct(IPR, screen_FC, H3K27me3.zscore, NHEJratio, DMSO_screen)

vorino_pool_screen = vorinostat_ratios %>% 
  mutate(IPR = gsub(".[AB]$", "", IPR)) %>% 
  filter(IPR %in% vorino_screen$IPR) %>% 
  distinct(IPR, logFC_NHEJ_MMEJ, H3K27me3, NHEJ_MMEJ_Vorinostat, NHEJ_MMEJ_DMSO) %>%
  dplyr::rename(pool_FC = logFC_NHEJ_MMEJ, 
...
ggplot(vorino_pool_screen, aes(H3K27me3, H3K27me3.zscore)) + geom_point() + stat_cor() + geom_smooth(method = "lm") + theme_bw(base_size = 16) + xlab("Log2 FC pool") + ylab("Log2 FC screen")
...
```{r}
Vorino_rep_check = screen_ratios %>% 
...
```
```{r}
screen_ratios %>% pull(drug) %>% unique() %>% .[grepl("MC-1293", .)]
...
HDAC_followup = screen_ratios %>% 
  ...
  distinct(MMEJ_log2_FC, MMEJ_zscore_comb, IPR, chromatin, drug) %>% 
  mutate(mean_MMEJ_FC = ave(MMEJ_log2_FC, IPR, drug, FUN = mean)) %>%
...
GSK_fc = GSK_BIX_molcell_tib %>% 
  ...
  select(IPR, logFC_GSK)
  
chrom_data_pool = GSK_BIX_molcell_tib %>% select(IPR, all_of(chromatin.features)) %>% distinct()
