...
ggplot(chromatin_effect %>%
         filter(clusters_drugs %in% c(1,4)),
...
## 5B: Effect size of target group per chromatin group
```{r}
chromatin_ipr <- chromatin %>%
  rownames_to_column("IPR")
...
rank_dt = indel.data %>% 
  ...
  distinct(drug, conc_char, zscore_bal_global_comb_rep , target, cor_bal, cor_bal_filter, viab_mean) %>% 
...
rank_dt %>% 
  ...
  mutate(# n = ifelse(is.na(n), 1, 2),
    col = ifelse(!between(rank, 10, maxrank-6) & 
                   !(viab_mean < .60 & zscore_bal_global_comb_rep < -2.54),
                 target, "black")
  ) %>%
  # filter(rank < 100 ) %>%
...
ggplot(bc_mut_tib_mean %>%
         mutate(freq_mean_mean = ave(freq_mean, mutation, chromatin, drug_conc, FUN = mean)) %>%
         mutate(freq_sd = ave(freq_mean, mutation, chromatin, drug_conc, FUN = sd)) %>%
         distinct(freq_mean_mean, freq_sd, label_efficiency, label_ratio, mutation, drug_conc, chromatin, color), 
            aes(mutation, freq_mean_mean, color = color, fill = drug_conc)) + 
  geom_bar(stat = "identity", position = position_dodge(0.75), aes(alpha = 0.7), width = .7) +
  geom_errorbar(aes(ymin=freq_mean_mean-freq_sd, ymax=freq_mean_mean+freq_sd), width=.2,
...
## 4E: Decitabine combined with NU7441
...
ggplot(decitabine_nu7441_df %>%
  ...
  geom_errorbar(aes(ymin=percentage_mean-percentage_sd, ymax=percentage_mean+percentage_sd), width=.2,
                 position=position_dodge(.75))  +
  scale_fill_manual("legend", values = c(Decitabine_ssODN = "#D69C81", NU7441_ssODN = "#8CA0C4", Decitabine_NU7441_ssODN = "grey50", DMSO_ssODN = "grey70")) + 
  scale_color_manual("legend", values = c(Decitabine_ssODN = "#D69C81", NU7441_ssODN = "#8CA0C4", Decitabine_NU7441_ssODN = "grey50", DMSO_ssODN = "grey70")) + 
  #scale_y_continuous(name="frequency", labels = percent) +
...
p1 = ggplot(mock.slopes, aes(z_score,value,color = variable)) + theme_bw() + 
  ...
  ylab("log2(fold change TIF)") + xlab("Feature signal (z-score)") + 
...
for(i in unique(CCD_tib$feature)) {
  ...
  if(nrow(rank_dt) > 1) {
    ...
    p2 = rank_dt %>% 
      filter(between(rank, maxrank-20, maxrank)) %>%
      mutate(rank = seq(1:n())) %>% 
      ggplot(., aes(x = rank, y = ratio_slope_plot, fill = target)) +
      geom_bar(stat = "identity") + 
      scale_x_reverse() +
