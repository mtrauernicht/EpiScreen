...
all_balance = indel.data %>% 
  distinct(drug, conc_char, replicate, MMEJ_zscore_mean, target, 
           barcode, viab_reproducibility, pathway_bal_filter) %>% 
  filter(!is.na(MMEJ_zscore_mean) & 
           !between(MMEJ_zscore_mean, -1.96, 1.96) & 
...
indel.data %>% 
    filter(conc_char != "control" & pathway_bal_filter == TRUE & viab_reproducibility == TRUE) %>%
    distinct(drug, conc_char, MMEJ_zscore_global_mean, target) %>% 
    arrange(desc(MMEJ_zscore_global_mean)) %>%
    mutate(rank = seq(1:n())) %>%
  filter(rank < 100) %>%
  ggplot(., aes(rank, MMEJ_zscore_global_mean, color = target)) +
  geom_point()  +
...
top_hits = indel.data  %>% 
  ...
  distinct(drug, conc_char, MMEJ_zscore_global_mean, target) %>%
    arrange(desc(MMEJ_zscore_global_mean)) %>% slice_head(n = 10) %>% distinct(drug, conc_char, target, MMEJ_zscore_global_mean) %>% 
  mutate(rank = seq(1:n()),
         drug = gsub(" [(].*[)]", "", drug),
...
```{r}
p1 = indel.data %>% 
    filter(conc_char != "control" & pathway_bal_filter == TRUE & viab_reproducibility == TRUE , replicate == "E177") %>%
    distinct(drug, conc_char, MMEJ_zscore_global, target) %>% 
    arrange(desc(MMEJ_zscore_global)) %>%
    mutate(rank = seq(1:n()),
         drug = gsub(" [(].*[)]", "", drug),
         drug_conc = paste(drug, conc_char, sep = " ")) %>%
    filter(rank < 100) %>%
    ggplot(., aes(rank, MMEJ_zscore_global, color = target)) +
    geom_point()  +
    scale_color_manual(values = colors_targets) +
    gghighlight(rank <= 7, label_key = drug_conc) +
    geom_hline(linetype = "dashed", yintercept = 1.96) +
    scale_x_reverse(limits = c(100, 0)) +
    ylab("Global MMEJ ratio z-score screen 1") +
    theme_classic()

p2 = indel.data %>% 
    filter(conc_char != "control" & pathway_bal_filter == TRUE & viab_reproducibility == TRUE , replicate == "E1504") %>%
    distinct(drug, conc_char, MMEJ_zscore_global, target) %>% 
    arrange(desc(MMEJ_zscore_global)) %>%
    mutate(rank = seq(1:n()),
         drug = gsub(" [(].*[)]", "", drug),
    ...
    geom_hline(linetype = "dashed", yintercept = 1.96) +
    scale_x_reverse(limits = c(100, 0)) +
    ylab("Global MMEJ ratio z-score screen 2") +
    theme_classic()
...
top_hits = indel.data  %>% 
    filter(conc_char != "control" & pathway_bal_filter == T & viab_reproducibility == TRUE) %>% 
  distinct(drug, conc_char, MMEJ_zscore_global, target, replicate) %>%
    arrange(desc(MMEJ_zscore_global)) %>% slice_head(n = 10) %>% distinct(drug, conc_char, target, MMEJ_zscore_global, replicate) %>% 
  mutate(rank = seq(1:n()),
         drug = gsub(" [(].*[)]", "", drug),
         drug_conc = paste(drug, conc_char, sep = " "))
```
