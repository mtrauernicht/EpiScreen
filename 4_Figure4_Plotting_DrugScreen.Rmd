...
rank_dt = indel.data %>% 
  ...
  distinct(drug, conc_char, MMEJ_zscore_global_comb , target) %>% 
  arrange(desc(MMEJ_zscore_global_comb )) %>%
  mutate(rank = seq(1:n()),
         drug = gsub(" [(].*[)]", "", drug),
...
p1 = rank_dt %>% 
  # filter(rank < 100 ) %>%
  ggplot(., aes(rank, MMEJ_zscore_global_comb )) +
  geom_point()  +
...
p2 = rank_dt %>%
  ...
  ggplot(., aes(rank, MMEJ_zscore_global_comb , color = target)) +
  geom_point()  +
  scale_color_manual(values = colors_targets) +
...
top_hits = indel.data  %>% 
  ...
  distinct(drug, conc_char, MMEJ_zscore_global_comb , target) %>%
  arrange(desc(MMEJ_zscore_global_comb )) %>% slice_head(n = 10) %>% distinct(drug, conc_char, target, MMEJ_zscore_global_comb ) %>% 
  mutate(rank = seq(1:n()),
         drug = gsub(" [(].*[)]", "", drug),
         drug_conc = paste(drug, conc_char, sep = " "))
```
