...
ratios_tib <- indel.data %>%
  ...
  distinct(MMEJratio, freqCut, drug, conc_char, drug_conc, target, IPR, tech, replicate) %>%
...
ratios_tib_fc <- ratios_tib %>% 
  dplyr::group_by(IPR, drug, conc_char, tech, replicate) %>% 
...
ratios_tib_fc %<>% mutate(drug_conc = paste(drug, conc_char, sep = " ")) %>% filter(!is.na(MMEJratio.log2))
