...
## Select outdir
out.dir = paste0("/DATA/projects/DSBrepair/scratch/figures_episcreen/exploratory_", Date, "/")
dir.create(out.dir)
...
```{r}
validations_ratios = readRDS("/DATA/projects/DSBrepair/data/R/rs20220201_episcreen/rs20220201_episcreen_validation_ratios.RDS")
validations_indels = readRDS("/DATA/projects/DSBrepair/data/R/rs20220201_episcreen/rs20220201_episcreen_validation_indels.RDS")
screen_indels = readRDS("/DATA/projects/DSBrepair/data/R/rs20220201_episcreen/rs20220201_episcreen_mutations.RDS")
screen_ratios = readRDS("/DATA/projects/DSBrepair/data/R/rs20220201_episcreen/rs20220201_episcreen_ratios.RDS")
```
...
vorinostat_ratios = validations_ratios %>% 
  ...
  mutate(diff_efficiency = efficiency_Vorinostat - efficiency_DMSO,
         ...
         logFC_MMEJ_NHEJ = log2(MMEJ_NHEJ_Vorinostat / MMEJ_NHEJ_DMSO)) %>%
  filter(!is.na(logFC_MMEJ_NHEJ) & !is.na(ratio_efficiency) & !is.infinite(logFC_MMEJ_NHEJ))
  
vorinostat_ratios %<>% left_join(unique(validations_ratios[, c(1, 42:75)]), by = "barcode") %>% distinct(barcode, .keep_all = TRUE)
```

```{r ratio MMEJNHEJ vs chrom, fig.height=8, fig.width=9}
chrom.mods.cols <- seq(grep("logFC_MMEJ_NHEJ", names(vorinostat_ratios))+1, grep("H3K27me3_domain", names(vorinostat_ratios))-1)

vorinostat.hist.corr <- CorrelationDFMean(vorinostat_ratios, "logFC_MMEJ_NHEJ", targets = chrom.mods.cols) %>%
  arrange(corr) %>% pull(rowname) %>% factor(.,levels = .)

lims = 1

CorrelationPlotMean(vorinostat_ratios, "logFC_MMEJ_NHEJ", targets = vorinostat.hist.corr)
```

```{r ratio efficiency vs chrom, fig.height=8, fig.width=9}
chrom.mods.cols <- seq(grep("logFC_MMEJ_NHEJ", names(vorinostat_ratios))+1, grep("H3K27me3_domain", names(vorinostat_ratios))-1)

vorinostat.hist.corr <- CorrelationDFMean(vorinostat_ratios, "diff_efficiency", targets = chrom.mods.cols) %>%
  arrange(corr) %>% pull(rowname) %>% factor(.,levels = .)

lims = 1

CorrelationPlotMean(vorinostat_ratios, "diff_efficiency", targets = vorinostat.hist.corr)
```

```{r}
# vorinostat_ratios %>% ggplot(., aes(vorinostat_ratios)) + geom_density()
vorinostat_ratios %>% ggplot(., aes(logFC_MMEJ_NHEJ)) + geom_density()
vorinostat_ratios %>% ggplot(., aes(diff_efficiency)) + geom_density()
vorinostat_ratios %>% ggplot(., aes(ratio_efficiency)) + geom_density()

vorinostat_ratios %>% ggplot(., aes(diff_efficiency, logFC_MMEJ_NHEJ)) + geom_point() + geom_smooth(method= "lm")
vorinostat_ratios %>% filter(efficiency_DMSO > 0.7) %>% ggplot(., aes(log2(ratio_efficiency), efficiency_DMSO, color = H3K27me3_domain)) + geom_point() + geom_smooth(method= "lm")
vorinostat_ratios %>% filter(efficiency_DMSO > 0.7) %>% ggplot(., aes(log2(ratio_efficiency), efficiency_DMSO, color = late_replicating_domain)) + geom_point() + geom_smooth(method= "lm")
vorinostat_ratios %>% filter(efficiency_DMSO > 0.7) %>% ggplot(., aes(log2(ratio_efficiency), efficiency_DMSO, color = LAD_domain)) + geom_point() + geom_smooth(method= "lm")


vorinostat_ratios %>% 
  # filter(efficiency_DMSO > 0.7) %>% 
  ggplot(., aes(logFC_MMEJ_NHEJ, MMEJ_NHEJ_DMSO)) + geom_point() + geom_smooth(method= "lm")

vorinostat_ratios %>% 
  # filter(efficiency_DMSO > 0.7) %>% 
  ggplot(., aes(MMEJ_NHEJ_Vorinostat, MMEJ_NHEJ_DMSO)) + geom_point() + geom_smooth(method= "lm") + geom_abline(slope = 1, type = 2)

vorinostat_ratios %>% 
  # filter(efficiency_DMSO > 0.7) %>% 
  ggplot(., aes(efficiency_Vorinostat, efficiency_DMSO)) + geom_point() + geom_smooth(method= "lm") + geom_abline(slope = 1, type = 2)
```

```{r}
vorinostat_ratios %>%
  dplyr::select(barcode, logFC_MMEJ_NHEJ, diff_efficiency, ratio_efficiency, LMNB1, H3K9me2, POL2AS2, EZH2, H3K27ac, H3K27me3) %>%
  group_by(barcode) %>%
  gather(logFC_MMEJ_NHEJ, diff_efficiency, ratio_efficiency, key = "pathway", value = "ratio") %>%
  gather(LMNB1, H3K9me2, POL2AS2, EZH2, H3K27ac, H3K27me3, key = "feature", value = "value") %>%
  ggplot(., aes(value, ratio))  +  
  geom_point() +
  geom_smooth(method= "lm") +
  theme_bw(base_size = 16) +
  stat_cor(method = "spearman") +
  facet_grid(pathway ~ feature, scales = "free")
```


```{r indel patterns in changing sites}
barcodes.vori = vorinostat_ratios %>% filter(logFC_MMEJ_NHEJ > 1) %>% pull(barcode)
...
barcodes.vori = vorinostat_ratios %>% filter(diff_efficiency > 0.1) %>% pull(barcode)
...
signif_bcs = signif_IPRs_full_tib %>% filter(p < 0.01) %>% pull(barcode)
...
indel.dt = vorinostat_ratios %>%
  filter(!is.na(logFC_MMEJ_NHEJ )) %>%
  dplyr::select(barcode, logFC_MMEJ_NHEJ , domains) %>%
...
plot_comb_grid_beeswarm(indel.dt, opt_df,
                        domain_levels, 'logFC_MMEJ_NHEJ', min_count = 10, y_plabel=c(-2, 2))

k27me3 = indel.dt %>% filter(late_replicating == 0, 
                             LAD == 0,
                             H3K9me2 == 0,
                             H3K27me3 == 1) %>% 
  pull(logFC_MMEJ_NHEJ)
