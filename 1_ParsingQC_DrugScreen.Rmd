...
out.dir = "/DATA/projects/DSBrepair/git/EpiScreen/files_scripts/"
```
...
cell.viab.data = fread(paste0(out.dir, "mt20200618_viability_rep1-2.csv"), 
...
indel.statistics.tib %>% 
  ...
  write_rds(., 
            file = paste0(out.dir, 
                          "rs20210626_Episcreen_Reads_Viab_Metadata.RDS"))
```
...
well.statistics <- indel.statistics.tib %>% 
  filter(replicate == "E177" & drug_plate == 1) %>%
  group_by(well) %>% 
  dplyr::summarise(r_written_pct = mean(r_written_pct))
...
viab_plot_tib = indel.statistics.tib %>%
  distinct(drug, well, conc_char,
           drug_plate, tech, plate,
           replicate, target, viab_norm) %>%
  dplyr::group_by(drug, conc_char, replicate) %>%
  dplyr::mutate(viab_sd = sd(viab_norm),
                viab_mean = mean(viab_norm)) %>%
  dplyr::select(-viab_norm) %>%
  pivot_wider(names_from = replicate, values_from = c(viab_mean, viab_sd)) %>%
  mutate(viab_ratio = viab_mean_E1504/viab_mean_E177) %>%
  distinct(drug,
           target,
           conc_char,
           viab_ratio,
...
odd_data = viab_plot_tib %>% 
  filter(!between(viab_ratio, 0.5, 2))

# QQ plot of the viability ratios.. 
p <- viab_plot_tib %>% 
  mutate(viab_ratio = viab_mean_E1504/viab_mean_E177) %>% 
  ggplot(., aes(sample = viab_ratio))

p + stat_qq() + stat_qq_line()

# Scatter plot of both replicates. 
# Highlighting the controls, concentration and outlyers. 
ggplot(viab_plot_tib, 
       aes(viab_mean_E1504, viab_mean_E177, color = viab_sd_E1504)) + 
  geom_point() +
...
for (i in unique(viab_plot_tib$target)) {
  p = filter(viab_plot_tib, target == i) %>%
    ...
    geom_point() +
    gghighlight(!(between(viab_ratio, 1/1.5, 1.5)) &
                  (viab_mean_E1504 > .5 |
                     viab_mean_E177 > .5),
                label_key = drug,
                max_highlight = 100L) +
    geom_abline(slope = 1.5/1, intercept = 0, 
                linetype = "dashed", alpha = 0.3) +
    geom_abline(slope = 1/1.5, intercept = 0, 
                linetype = "dashed", alpha = 0.3) +
...
indel.statistics.tib %>% ggplot(., aes(viab_norm, log10(reads))) +  geom_point() + facet_grid(replicate ~ conc_char) 
```
