...
indel.statistics.df$well <- gsub(".*_.*_(.*?)_[0-9]*", "\\1", indel.statistics.df$sample.well)
indel.statistics.df$row <- gsub("[0-9]*", "\\1", indel.statistics.df$well)
...
for (i in 1:18) {
  ...
  limits <- quantile(data$reads, probs = c(0.05, 0.95))
  margin <- (limits[2] - limits[1]) * 0.75
  limits[1] <- limits[1] - margin
  limits[2] <- limits[2] + margin

  limits[1] <- 2000
  # plate$mean_score[which(plate$mean_score > limits[2])] <- limits[2]

  # Plot this, first the PI staining with a custom color-coded density
  # y <- density(data$PI_area, n = 2^10)
  ...
  plt2
  # grid.arrange(plt2, ncol = 2, widths = c(1, 1.3))
