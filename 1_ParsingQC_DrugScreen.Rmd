...
setwd("/home/r.schep/mydata/projects/CRISPR_TRIP/20180918_TRIP_5106/data/processed_20181107/parsed/")
...
indel.statistics.df$concentration <- gsub("^(.*)_rep.*", "\\1", rownames(indel.statistics.df))
indel.statistics.df$replicate <- gsub("^.*_(.*?)_.*", "\\1", rownames(indel.statistics.df))
```
...
indel.statistics.df$sample <- rownames(indel.statistics.df)
b <- ggplot(indel.statistics.df, aes(sample, reads)) + geom_bar(stat = "identity", aes(fill = concentration))
b
b <- ggplot(indel.statistics.df, aes(sample, reads)) + geom_bar(stat = "identity", aes(fill = replicate))
b
...
nrow(dupli.df)
# Remove the set of intergrations that are both in the A and B pools.  #why???
...
# Yes, the data looks good like this. There are only 2 reads that are mapped exactly to the
# same location, one in A & B with different barcodes, and one with similar looking BC.
...
tib_dist <- tib_distance_mapping[, c(".id", "barcode", "start_pos")] %>% 
  group_by(barcode) %>% 
...
ggplot(tib_dist, aes(barcode, sort(log10(dist))), fill = trans) + 
...
setwd("/DATA/projects/DSBrepair/data/")
filename <- SetFileName("_Mapping_RSTP2_2000_broad.bed", "rs")
