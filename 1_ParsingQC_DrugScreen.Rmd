...
The two replicates correlate quite well except for some of these outlyers in E1504 that were less deadly to the cells.


```{r data export}
viab_plot_tib %<>% 
  pivot_longer(., c("viab_mean_E1504", "viab_mean_E177"), 
               names_to = "replicate", 
               values_to = "viab_mean") %>% 
  mutate(replicate = gsub("viab_mean_", "", replicate)) %>%
  pivot_longer(., c("viab_var_E1504", "viab_var_E177"), 
               names_to = "replicate2", 
               values_to = "viab_var") %>%
  select(-replicate2) %>%
  unique()
...
indel.statistics.tib %>%
  ...
  write_rds(.,
            file = paste0(out.dir,
                          "rs20211221_Episcreen_Reads_Viab_Metadata.RDS"))
```
