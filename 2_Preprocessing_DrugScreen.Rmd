...
# Add plate specifier
RSTP2_2000_indels.df$plate <- "P1"
RSTP2_2000_indels.df[grep("10um_.*_plate2", RSTP2_2000_indels.df$.id),]$plate <- "P2"
RSTP2_2000_indels.df[grep("1um_.*_plate1", RSTP2_2000_indels.df$.id),]$plate <- "P3"
RSTP2_2000_indels.df[grep("1um_.*_plate2", RSTP2_2000_indels.df$.id),]$plate <- "P4"
RSTP2_2000_indels.df[grep("100nm_.*_plate1", RSTP2_2000_indels.df$.id),]$plate <- "P5"
RSTP2_2000_indels.df[grep("100nm_.*_plate2", RSTP2_2000_indels.df$.id),]$plate <- "P6"
...
ggplot(RSTP2_2000_indels.df, aes(read.count)) + geom_density(fill = "#3D9F83") + theme_bw() + 
  labs(title = "read distribution per drug")
ggplot(RSTP2_2000_indels.df, aes(read.count)) + geom_density(fill = "#3D9F83") + theme_bw() + 
  labs(title = "read distribution per drug") + 
  facet_wrap(~Drug)
ggplot(RSTP2_2000_indels.df, aes(read.count)) + geom_density(fill = "#3D9F83") + theme_bw() + 
...
RSTP2_2000_indels.df <- RSTP2_2000_indels.df[RSTP2_2000_indels.df$read.count1.7 > 30,] %>% 
...
RSTP2_2000_indels.df <- RSTP2_2000_indels.df %>%
  select(Drug, Target, well, V2, rep, conc, plate, freqMMEJ, freqNHEJ,
         NHEJMMEJratio, MMEJscore, NHEJscore, freqCut, .id)
setnames(RSTP2_2000_indels.df, c("Drug", "Target", "V2", ".id"), c("drug", "target", "barcode", "id"))




# Add viability data to RSTP2_2000_indels.df
RSTP2_2000_indels.df <- merge(viability.data, RSTP2_2000_indels.df)
RSTP2_2000_indels.df <- RSTP2_2000_indels.df %>% select(-id)

# Remove barcodes with abnormal counts from analysis
RSTP2_2000_indels.df <- RSTP2_2000_indels.df[RSTP2_2000_indels.df$barcode != "ACCCCTAAAGGCGCTG" &
                           RSTP2_2000_indels.df$barcode != "CTCTTAATCGCTGCC",]


# We want to exclude reads from cells that have reduced viability (set cut-off at 45% reduced viability)
RSTP2_2000_indels.df <- RSTP2_2000_indels.df %>% 
  mutate(viability = ave(viability, plate, FUN = function(x) x/max(x))) # relative viability
RSTP2_2000_indels.df <- RSTP2_2000_indels.df[RSTP2_2000_indels.df$viability > 0.45,]
...
RSTP2_2000_indels.df$sample[RSTP2_2000_indels.df$drug == "DMSO"] <- "WT"
