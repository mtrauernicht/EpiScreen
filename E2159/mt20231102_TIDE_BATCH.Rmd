...
```{r}
ref.file <- "/DATA/usr/m.trauernicht/projects/EpiScreen/E2159/mt20230306_seq_LBR.txt"
output.file <- "/DATA/usr/m.trauernicht/projects/EpiScreen/E2159/mt20230329_TIDE_LBR.pdf"
folder <- ""
seqstart = 65
seqend = 225
maxshift = 10
...
Define the arguments: 
```{r}
ref.file <- "/DATA/usr/m.trauernicht/projects/EpiScreen/E2159/mt20230306_seq_selected.txt"
output.file <- "/DATA/usr/m.trauernicht/projects/EpiScreen/E2159/mt20230329_TIDE.pdf"
folder <- ""
seqstart = 200
seqend = 450
maxshift = 10
...
## Export percentages
```{r}
percentage_df2 <- data.frame("sample" = unique(table$name), "freq" = "", x = 1:length(table$name))
percentage_df2 <- merge(percentage_df2, data.frame('y' = 1:21, 'mutation' = -maxshift:maxshift))


for (i in 1:length(plot_list)) {
  for (j in 1:21) {
  percentage_df2$freq[percentage_df2$x == i & percentage_df2$y == j] <- plot_list[[i]]$NNFIT$x[j]/sum(plot_list[[i]]$NNFIT$x)
  }
}

percentage_df2 <- percentage_df2 %>%
  distinct(sample, freq, mutation)

percentage_df <- rbind(percentage_df, percentage_df2)
```
...
## Annotate dataframe
percentage_df <- percentage_df %>%
  mutate(condition = gsub("_r[1-6]", "", sample)) %>%
  mutate(replicate = gsub(".*(r[1-6])", "\\1", sample)) %>%
  mutate(gRNA = gsub("_.*", "", condition)) %>%
  mutate(drug = gsub(".*_", "", condition)) %>%
  filter(str_detect(condition, "ssODN", negate = T)) %>%
  filter(replicate != "r6")
...
TIF_df <- percentage_df %>%
  ...
  mutate(TIF = 1 - `0`) %>%
  mutate(mean_TIF = ave(TIF, condition, FUN = mean)) %>%
  distinct(sample, condition, replicate, drug, gRNA, TIF, mean_TIF)

dmso_tif <- TIF_df %>%
  filter(drug == "DMSO") %>%
  dplyr::select("dmso_tif" = TIF, replicate, gRNA)

TIF_df <- merge(TIF_df, dmso_tif) %>%
  mutate(difTIF =  TIF - dmso_tif) %>%
  mutate(mean_difTIF = ave(difTIF, condition, FUN = mean))  
