...
mean.log2.distance.mmej %<>% mutate(drug_conc = paste(drug, conc_char, sep = "_")) %>% pivot_longer()
...
hits = slope.protein.features %>% filter(term == "unlist(model.dt[j])" & abs(slope.log2) > 0.15) %>% pull(drug_conc) %>% unique()
